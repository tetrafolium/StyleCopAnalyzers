...
namespace StyleCop.Analyzers.LayoutRules
{
        ...
        /// <summary>
        /// A C# statement containing opening and closing braces is written completely on a single
        /// line.
        /// </summary>
        /// <remarks>
        /// <para>A violation of this rule occurs when a statement that is wrapped in opening and
        /// closing braces is written on a single line. For example:</para>
        ///
        ...
        /// </code>
        ///
        /// <para>When StyleCop checks this code, a violation of this rule will occur because the
        /// entire lock statement is written on one line. The statement should be written across
        /// multiple lines, with the opening and closing braces each on their own line, as
        /// follows:</para>
        ///
        /// <code language="csharp">
        /// public object Method()
        ...
        internal class SA1501StatementMustNotBeOnASingleLine : DiagnosticAnalyzer
        {
                ...
                /// cref="SA1501StatementMustNotBeOnASingleLine"/> analyzer.
                /// </summary>
                ...
                private static readonly LocalizableString Description
                    = new LocalizableResourceString (nameof (LayoutResources.SA1501Description),
                                                     LayoutResources.ResourceManager,
                                                     typeof (LayoutResources));
                        ...
                        if (context.IsAnalyzerSuppressed (SA1503BracesMustNotBeOmitted.Descriptor))
                                {
                                        ...
                                        context.RegisterSyntaxNodeAction (
                                            ctx => CheckChildStatement (
                                                ctx, ((UsingStatementSyntax) ctx.Node).Statement),
                ...
                HandleBlock (SyntaxNodeAnalysisContext context)
                {
                        ...
                        if (!block.OpenBraceToken.IsMissing && !block.CloseBraceToken.IsMissing
                            && IsPartOfStatement (block))
                                {
                                        var openBraceLineNumber
                                            = block.SyntaxTree
                                                  .GetLineSpan (block.OpenBraceToken.Span)
                                                  .StartLinePosition.Line;
                                        var closeBraceLineNumber
                                            = block.SyntaxTree
                                                  .GetLineSpan (block.CloseBraceToken.Span)
                                                  .StartLinePosition.Line;
                ...
                HandleIfStatement (SyntaxNodeAnalysisContext context)
                {
                        ...
                        if (ifStatement.Parent.IsKind (SyntaxKind.ElseClause))
                                {
                                        // this will be analyzed as a clause of the outer if
                                        // statement
                                        return;
                                }
                        ...
                        if (!context.IsAnalyzerSuppressed (SA1520UseBracesConsistently.Descriptor))
                                {
                                        // inconsistencies will be reported as SA1520, as long as
                                        // it's not suppressed
                                     ...
                                     StatementSyntax childStatement)
                {
                        ...
                        // We are only interested in the case where statement and childStatement
                        // start on the same line. Use IsFirstInLine to detect this condition
                        // easily.
                        SyntaxToken firstChildToken = childStatement.GetFirstToken ();
                        if (firstChildToken.IsMissingOrDefault ()
                            || firstChildToken.IsFirstInLine ())
                                {
                                        return;
                                }
                        ...
                        if (!context.IsAnalyzerSuppressed (
                                ...
                                {
                                        // diagnostics for multi-line statements is handled by
                                        // SA1519, as long as it's not suppressed
                                        FileLinePositionSpan lineSpan
                                            = childStatement.GetLineSpan ();
                ...
                private static bool
                IsSingleLineExpression (ExpressionSyntax containingExpression)
                {
                        ...
                        var lineSpan = containingExpression.SyntaxTree.GetLineSpan (
                            containingExpression.Span);
                        return lineSpan.StartLinePosition.Line == lineSpan.EndLinePosition.Line;
                }

                private static bool
                IsPartOfStatement (BlockSyntax block)
                {
                        return block.Parent.FirstAncestorOrSelf<StatementSyntax> () != null;
                }

                private static ExpressionSyntax
                GetContainingExpression (SyntaxNode node)
                {
                        return node.FirstAncestorOrSelf<ExpressionSyntax> ();
                }

                private static void
                ReportDiagnostic (SyntaxNodeAnalysisContext context, Location location,
                                  bool reportAsHidden = false)
                {
                        ...
                        if (reportAsHidden)
                                {
                                        diagnostic = Diagnostic.Create (
                                            ...
                                            Descriptor.MessageFormat, DiagnosticSeverity.Hidden,
                                            Descriptor.DefaultSeverity,
                                            Descriptor.IsEnabledByDefault, 1, Descriptor.Title,
                                            Descriptor.Description, Descriptor.HelpLinkUri,
                                            location, properties
