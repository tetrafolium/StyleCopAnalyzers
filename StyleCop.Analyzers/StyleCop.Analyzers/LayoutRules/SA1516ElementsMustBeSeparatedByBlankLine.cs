...
namespace StyleCop.Analyzers.LayoutRules
{
        ...
        /// </summary>
        /// <remarks>
        /// <para>To improve the readability of the code, StyleCop requires blank lines in certain
        /// situations, and prohibits blank lines in other situations. This results in a consistent
        /// visual pattern across the code, which can improve recognition and readability of
        /// unfamiliar code.</para>
        ///
        /// <para>A violation of this rule occurs when two adjacent element are not separated by a
        ...
        /// </code>
        ///
        /// <para>In the example above, the method and property are not separated by a blank line,
        /// so a violation of this rule would occur.</para>
        ///
        /// <code language="csharp">
        ...
        /// </code>
        ///
        /// <para>In the example above, the add and remove of the event need to be separated by a
        ...
        internal class SA1516ElementsMustBeSeparatedByBlankLine : DiagnosticAnalyzer
        {
                ...
                private static readonly LocalizableString MessageFormatOmit
                    = new LocalizableResourceString (
                        nameof (LayoutResources.SA1516MessageFormatOmit),
                        LayoutResources.ResourceManager, typeof (LayoutResources));
                ...
                /// diagnostics generated by this analyzer.
                /// </summary>
                        ...
                        if (propertyDeclaration.AccessorList?.Accessors != null)
                                {
                                        ...
                                        // We are not interested in properties with only one
                                        // accessor
                                        if (accessors.Count == 2)
                                                {
                                                        if (accessors[0].Body != null
                                                            && accessors[1].Body != null)
                                                                {
                                                                        // Don't report a diagnostic
                                                                        // if all accessors are
                                                                        // single line. Example:
                                                                        //// public string Foo
                                                                        //// {
                                                                        ////    get { return "bar";
                ...
                private static void
                HandleUsings (SyntaxNodeAnalysisContext context,
                        ...
                        for (var i = 1; i < usings.Count; i++)
                                {
                                        ...
                                        if (partOfSameGroup)
                                                {
                                                        // if the using statements are part of the
                                                        // same group, there is no need to check.
                                                        continue;
                                                }
                                        ...
                                        else if (blankLinesBetweenUsingGroups == OptionSetting.Omit)
                                                {
                                                        ...
                                                        var separatingTrivia
                                                            = TriviaHelper.MergeTriviaLists (
                                                                ...
                                                                usings [i]
                                                                    .GetLeadingTrivia ());
                                                        if (!separatingTrivia.ContainsBlankLines (
                                                                false))
                                                                {
                                                                        continue;
                                                                }
                ...
                private static void
                HandleMemberList (SyntaxNodeAnalysisContext context,
                        ...
                        for (int i = 1; i < members.Count; i++)
                                {
                                        if (!members[i - 1].ContainsDiagnostics
                                            && !members[i].ContainsDiagnostics)
                                                {
                                                        // Report if
                                                        // the current declaration is not a field
                                                        // declaration or the previous declaration
                                                        // is of different type or the previous
                                                        // declaration spans across multiple lines
                                                        //
                                                        // Note that the order of checking is
                                                        // important, as the call to IsMultiLine
                                                        // requires a FieldDeclarationSyntax,
                                                        // something that can only be guaranteed if
                                                        ...
                                                        if (!members [i]
                                                            ...
                                                            || !members [i - 1]
                                                                    .IsKind (members [i]
                                                                                 .Kind ())
                                                            || IsMultiline ((FieldDeclarationSyntax)
                                                                                members[i - 1]))
                                                                {
                                                                        ReportIfThereIsNoBlankLine (
                                                                            context, members[i - 1],
                ...
                private static bool
                IsMultiline (FieldDeclarationSyntax fieldDeclaration)
                {
                        ...
                        if (attributeLists.Count > 0)
                                {
                                        var lastAttributeSpan
                                            = fieldDeclaration.SyntaxTree.GetLineSpan (
                ...
                private static bool
                IsMultiline (AccessorDeclarationSyntax accessorDeclaration)
                {
                ...
                private static void
                ReportIfThereIsNoBlankLine (SyntaxNodeAnalysisContext context, SyntaxNode firstNode,
                        ...
                        if (!HasEmptyLine (allTrivia))
                                {
                                        context.ReportDiagnostic (Diagnostic.Create (
                                            Descriptor, GetDiagnosticLocation (secondNode),
                                            DiagnosticProperties));
                                }
                }

                private static Location
                GetDiagnosticLocation (SyntaxNode node)
                {
                ...
                private static bool
                HasEmptyLine (IEnumerable<SyntaxTrivia> allTrivia)
                {
