...
namespace StyleCop.Analyzers.Helpers
{
        ...
        internal static class DocumentationCommentExtensions
        {
                ...
                public static T
                ReplaceExteriorTrivia<T> (this T node, SyntaxTrivia trivia) where T : XmlNodeSyntax
                {
                        ...
                        return node.ReplaceTrivia (
                            node.DescendantTrivia (descendIntoTrivia
                                                   : true)
                                .Where (
                                    i => i.IsKind (SyntaxKind.DocumentationCommentExteriorTrivia)),
                ...
                WithoutFirstAndLastNewlines (this SyntaxList<XmlNodeSyntax> summaryContent)
                {
                        ...
                        if (IsXmlNewLine (lastSyntaxTokens[lastSyntaxTokens.Count - 1]))
                        ...
                        else
                                {
                                        ...
                                        if (!IsXmlNewLine (
                                                lastSyntaxTokens[lastSyntaxTokens.Count - 2]))
                                                {
                                                        return summaryContent;
                                                }
                        ...
                        for (int i = 0; i < removeFromEnd; i++)
                                {
                                        ...
                                        lastSyntaxTokens = lastSyntaxTokens.RemoveAt (
                                            lastSyntaxTokens.Count - 1);
                                }

                        summaryContent = summaryContent.RemoveAt (summaryContent.Count - 1);
                        if (lastSyntaxTokens.Count != 0)
                                {
                                        summaryContent = summaryContent.Add (
                                            lastSyntax.WithTextTokens (lastSyntaxTokens));
                                }
                        ...
                        if (summaryContent.Count > 0)
                                {
                                        ...
                                        summaryContent = summaryContent.Replace (
                                            summaryContent[0], summaryContent [0]
                                                                   .WithLeadingTrivia ());

                                        // Remove leading spaces (between the <para> start tag and
                                        // the start of the paragraph content)
                                        if (summaryContent[0] is XmlTextSyntax firstTextSyntax
                                            && firstTextSyntax.TextTokens.Count > 0)
                                                {
                                                        SyntaxToken firstTextToken
                                                            = firstTextSyntax.TextTokens[0];
                                                        string firstTokenText = firstTextToken.Text;
                                                        string trimmed
                                                            = firstTokenText.TrimStart ();
                                                        if (trimmed != firstTokenText)
                                                                {
                                                                        SyntaxToken newFirstToken
                                                                            = SyntaxFactory.Token (
                                                                                firstTextToken
                                                                                    .LeadingTrivia,
                                                                                firstTextToken
                ...
                public static bool
                IsXmlNewLine (this SyntaxToken node)
                {
                        return node.IsKind (SyntaxKind.XmlTextLiteralNewLineToken);
                }

                public static bool
                IsXmlWhitespace (this SyntaxToken node)
                {
                ...
                /// <summary>
                /// Adjust the leading and trailing trivia associated with <see
                /// cref="SyntaxKind.XmlTextLiteralNewLineToken"/> tokens to ensure the formatter
                /// properly indents the exterior trivia.
                /// </summary>
                /// <typeparam name="T">The type of syntax node.</typeparam>
                /// <param name="node">The syntax node to adjust tokens.</param>
                /// <returns>A <see cref="SyntaxNode"/> equivalent to the input <paramref
                /// name="node"/>, adjusted by moving any trailing trivia from <see
                /// cref="SyntaxKind.XmlTextLiteralNewLineToken"/> tokens to be leading trivia of
                /// the following token.</returns>
                public static T
                AdjustDocumentationCommentNewLineTrivia<T> (this T node) where T : SyntaxNode
                {
                        var tokensForAdjustment
                            = from token in node.DescendantTokens () where token
                            ...
                            = token
                                  .GetNextToken (includeZeroWidth
                                      ...
                                      select new KeyValuePair<SyntaxToken, SyntaxToken> (token,
                                                                                         next);
                        ...
                        foreach (var pair in tokensForAdjustment)
                                {
                                        ...
                                        replacements[pair.Value] = pair.Value.WithLeadingTrivia (
                                            pair.Value.LeadingTrivia.InsertRange (
                                                0, pair.Key.TrailingTrivia));
                                }
                ...
                public static XmlNameSyntax
                GetName (this XmlNodeSyntax element)
                {
                ...
                SelectExteriorTrivia (SyntaxTrivia rewrittenTrivia, SyntaxTrivia trivia,
                                      SyntaxTrivia triviaWithSpace)
                {
                        ...
                        // otherwise the space is part of the leading trivia of the following token,
                        // so don't add an extra one to the exterior trivia
                        return trivia;
                }
        }
}
