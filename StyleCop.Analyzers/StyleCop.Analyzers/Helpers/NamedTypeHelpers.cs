...
namespace StyleCop.Analyzers.Helpers
{
        ...
        internal static class NamedTypeHelpers
        {
                ...
                IsNativeMethodsClass (INamedTypeSymbol type)
                {
                ...
                internal static bool
                IsNativeMethodsClass (ClassDeclarationSyntax syntax)
                {
                ...
                internal static bool
                IsContainedInNativeMethodsClass (INamedTypeSymbol type)
                {
                        if (type == null)
                                {
                                        return false;
                ...
                internal static bool
                IsContainedInNativeMethodsClass (SyntaxNode syntax)
                {
                        while (syntax != null)
                                {
                ...
                GetNameOrIdentifier (MemberDeclarationSyntax member)
                {
                        switch (member.Kind ())
                                {
                ...
                GetNameOrIdentifierLocation (SyntaxNode member)
                {
                        ...
                        location = location ?? (member as BaseTypeDeclarationSyntax)
                            ?.Identifier.GetLocation ();
                        location = location ?? (member as NamespaceDeclarationSyntax)
                            ?.Name.GetLocation ();
                        location
                            = location ?? (member as UsingDirectiveSyntax) ?.Name.GetLocation ();
                ...
                internal static bool
                IsPartialDeclaration (MemberDeclarationSyntax declaration)
                {
                ...
                /// </summary>
                /// <remarks>
                /// <para>This method does only check the interfaces the containing type is
                /// implementing directly. If a derived class is implementing an interface and this
                /// member is required for it this method will still return false.</para>
                /// </remarks>
                /// <param name="memberSymbol">The member symbol that should be analyzed.</param>
                /// <returns>true if the member is implementing an interface member, otherwise
                /// false.</returns>
                internal static bool
                IsImplementingAnInterfaceMember (ISymbol memberSymbol)
                {
                        ...
                        if (memberSymbol is IMethodSymbol methodSymbol)
                                {
                                        // Check if the member is implementing an interface
                                        // explicitly
