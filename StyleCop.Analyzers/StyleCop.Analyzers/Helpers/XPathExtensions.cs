...
namespace StyleCop.Analyzers.Helpers
{
        ...
        {
                // This class borrows heavily from src/Compilers/Core/Portable/PortableShim.cs of
                // Roslyn
                ...
                internal static IEnumerable<XElement>
                XPathSelectElements (this XNode node, string expression)
                {
                        var xpathSelectElements
                            = XPathExtensionsType.GetTypeInfo ().GetDeclaredMethod (
                                nameof (XPathSelectElements),
                                new[]{ typeof (XNode), typeof (string) });

                        return xpathSelectElements.Invoke (null, new object[]{ node, expression })
                            as IEnumerable<XElement>;
                }

                private static Type
                GetTypeFromEither (string contractName, string desktopName)
                {
                ...
                private static Type
                TryGetType (string assemblyQualifiedName)
                {
                        try
                                {
                                        // Note that throwOnError=false only suppresses some
                                        // exceptions, not all.
                ...
                GetDeclaredMethod (this TypeInfo typeInfo, string name, params Type[] paramTypes)
                {
                        return FindItem (typeInfo.GetDeclaredMethods (name), paramTypes);
                }

                private static T
                FindItem<T> (IEnumerable<T> collection, params Type[] paramTypes) where T
                    : MethodBase
                {
