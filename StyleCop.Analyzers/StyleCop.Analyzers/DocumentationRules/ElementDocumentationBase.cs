...
namespace StyleCop.Analyzers.DocumentationRules
{
        ...
        /// <summary>
        /// This is the base class for analyzers which examine the <c>&lt;param&gt;</c> text of a
        /// documentation comment on an element declaration.
        /// </summary>
        internal abstract class ElementDocumentationBase : DiagnosticAnalyzer {
                ...
                private readonly Action<SyntaxNodeAnalysisContext, StyleCopSettings>
                    operatorDeclarationAction;
                private readonly Action<SyntaxNodeAnalysisContext, StyleCopSettings>
                    conversionOperatorDeclarationAction;
                private readonly Action<SyntaxNodeAnalysisContext, StyleCopSettings>
                    baseTypeDeclarationAction;
                private readonly Action<SyntaxNodeAnalysisContext, StyleCopSettings>
                    fieldDeclarationAction;
                private readonly Action<SyntaxNodeAnalysisContext, StyleCopSettings>
                    propertyDeclarationAction;
                private readonly Action<SyntaxNodeAnalysisContext, StyleCopSettings>
                    enumMemberDeclarationAction;

                protected ElementDocumentationBase(
                    bool inheritDocSuppressesWarnings, string matchElementName = null)
                {
                        ...
                        this.conversionOperatorDeclarationAction
                            = this.HandleConversionOperatorDeclaration;
                ...
                public override void Initialize(AnalysisContext context)
                {
                        ...
                        context.RegisterSyntaxNodeAction(
                            this.methodDeclarationAction, SyntaxKind.MethodDeclaration);
                        context.RegisterSyntaxNodeAction(
                            this.constructorDeclarationAction, SyntaxKind.ConstructorDeclaration);
                        context.RegisterSyntaxNodeAction(
                            this.delegateDeclarationAction, SyntaxKind.DelegateDeclaration);
                        context.RegisterSyntaxNodeAction(
                            this.indexerDeclarationAction, SyntaxKind.IndexerDeclaration);
                        context.RegisterSyntaxNodeAction(
                            this.operatorDeclarationAction, SyntaxKind.OperatorDeclaration);
                        context.RegisterSyntaxNodeAction(this.conversionOperatorDeclarationAction,
                            SyntaxKind.ConversionOperatorDeclaration);
                        context.RegisterSyntaxNodeAction(
                            this.baseTypeDeclarationAction, SyntaxKinds.BaseTypeDeclaration);
                        context.RegisterSyntaxNodeAction(
                            this.fieldDeclarationAction, SyntaxKind.FieldDeclaration);
                        context.RegisterSyntaxNodeAction(
                            this.propertyDeclarationAction, SyntaxKind.PropertyDeclaration);
                        context.RegisterSyntaxNodeAction(
                            this.enumMemberDeclarationAction, SyntaxKind.EnumMemberDeclaration);
                }
                ...
                /// <param name="needsComment"><see langword="true"/> if the current documentation
                /// settings indicate that the element should be documented; otherwise, <see
                /// langword="false"/>.</param> <param name="syntaxList">The <see
                /// cref="XmlElementSyntax"/> or <see cref="XmlEmptyElementSyntax"/> of the node to
                /// examine.</param> <param name="diagnosticLocations">The location(s) where
                /// diagnostics, if any, should be reported.</param>
                ...
                /// <param name="needsComment"><see langword="true"/> if the current documentation
                /// settings indicate that the element should be documented; otherwise, <see
                /// langword="false"/>.</param> <param name="completeDocumentation">The complete
                /// documentation for the declared symbol, with any <c>&lt;include&gt;</c> elements
                /// expanded. If the XML documentation comment included a <c>&lt;param&gt;</c>
                /// element, this value will be <see langword="null"/>, even if the XML
                    ...
                    SyntaxNodeAnalysisContext context, StyleCopSettings settings)
                {
                        ...
                        this.HandleDeclaration(
                            context, settings, needsComment, node, node.Identifier.GetLocation());
                }

                private void HandleConstructorDeclaration(
                    SyntaxNodeAnalysisContext context, StyleCopSettings settings)
                {
                ...
                private void HandleDelegateDeclaration(
                    SyntaxNodeAnalysisContext context, StyleCopSettings settings)
                {
                ...
                private void HandleIndexerDeclaration(
                    SyntaxNodeAnalysisContext context, StyleCopSettings settings)
                {
                        ...
                        bool needsComment = SA1600ElementsMustBeDocumented.NeedsComment(
                            ...
                            declaredAccessibility, effectiveAccessibility);
                        this.HandleDeclaration(
                            context, settings, needsComment, node, node.ThisKeyword.GetLocation());
                }

                private void HandleOperatorDeclaration(
                    SyntaxNodeAnalysisContext context, StyleCopSettings settings)
                {
                        ...
                        bool needsComment = SA1600ElementsMustBeDocumented.NeedsComment(
                            settings.DocumentationRules, node.Kind(), node.Parent.Kind(),
                            declaredAccessibility, effectiveAccessibility);
                        this.HandleDeclaration(context, settings, needsComment, node,
                            node.OperatorToken.GetLocation());
                }

                private void HandleConversionOperatorDeclaration(
                    SyntaxNodeAnalysisContext context, StyleCopSettings settings)
                {
                        ...
                        bool needsComment = SA1600ElementsMustBeDocumented.NeedsComment(
                            ...
                            declaredAccessibility, effectiveAccessibility);
                        this.HandleDeclaration(
                            context, settings, needsComment, node, node.GetLocation());
                }

                private void HandleBaseTypeDeclaration(
                    SyntaxNodeAnalysisContext context, StyleCopSettings settings)
                {
                    ...
                    SyntaxNodeAnalysisContext context, StyleCopSettings settings)
                {
                        ...
                        bool needsComment = SA1600ElementsMustBeDocumented.NeedsComment(
                            ...
                            declaredAccessibility, effectiveAccessibility);
                        this.HandleDeclaration(
                            context, settings, needsComment, node, node.Declaration.GetLocation());
                }

                private void HandlePropertyDeclaration(
                    SyntaxNodeAnalysisContext context, StyleCopSettings settings)
                {
                ...
                private void HandleEnumMemberDeclaration(
                    SyntaxNodeAnalysisContext context, StyleCopSettings settings)
                {
                ...
                private void HandleDeclaration(SyntaxNodeAnalysisContext context,
                    StyleCopSettings settings, bool needsComment, SyntaxNode node,
                    params Location[] locations)
                {
                        ...
                        var hasIncludedDocumentation = documentation.Content.GetFirstXmlElement(
                            XmlCommentHelper.IncludeXmlTag) is object;
                        ...
                        if (hasIncludedDocumentation) {
                                ...
                                this.HandleCompleteDocumentation(
                                    context, needsComment, completeDocumentation, locations);
                        } else {
                                IEnumerable<XmlNodeSyntax> matchingXmlElements
                                    = string.IsNullOrEmpty(this.matchElementName)
                                    ? documentation.Content
                                          .Where(x => x is XmlElementSyntax
                                                  || x is XmlEmptyElementSyntax)
                                          .Where(x => !string.Equals(x.GetName() ?.ToString(),
                                                     XmlCommentHelper.IncludeXmlTag,
                                                     StringComparison.Ordinal))
                                      : documentation.Content.GetXmlElements(this.matchElementName);

                                this.HandleXmlElement(context, settings, needsComment,
                                    matchingXmlElements, locations);
                        }
                }
        }
}
