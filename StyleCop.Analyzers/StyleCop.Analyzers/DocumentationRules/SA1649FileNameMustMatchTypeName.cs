...
namespace StyleCop.Analyzers.DocumentationRules
{
    ...
    internal class SA1649FileNameMustMatchTypeName : DiagnosticAnalyzer
    {
        ...
        private static readonly LocalizableString MessageFormat =
            new LocalizableResourceString(nameof(DocumentationResources.SA1649MessageFormat),
                                          DocumentationResources.ResourceManager, typeof(DocumentationResources));
        ...
        private static readonly Action<SyntaxTreeAnalysisContext, StyleCopSettings> SyntaxTreeAction =
            Analyzer.HandleSyntaxTree;
        ...
        private static class Analyzer
        {
            public static void HandleSyntaxTree(SyntaxTreeAnalysisContext context, StyleCopSettings settings)
            {
                ...
                var modifiers =
                    (firstTypeDeclaration as BaseTypeDeclarationSyntax) ?.Modifiers ?? SyntaxFactory.TokenList();
                ...
                var expectedFileName = FileNameHelpers.GetConventionalFileName(
                    firstTypeDeclaration, settings.DocumentationRules.FileNamingConvention);
                ...
                if (string.Compare(fileName, expectedFileName, StringComparison.OrdinalIgnoreCase) != 0)
                {
                    if (settings.DocumentationRules.FileNamingConvention == FileNamingConvention.StyleCop &&
                        string.Compare(fileName, FileNameHelpers.GetSimpleFileName(firstTypeDeclaration),
                                       StringComparison.OrdinalIgnoreCase) == 0)
                    {
                        return;
                    }

                    var properties = ImmutableDictionary.Create<string, string>().Add(ExpectedFileNameKey,
                                                                                      expectedFileName + suffix);

                    var identifier = (firstTypeDeclaration as BaseTypeDeclarationSyntax)
                        ?.Identifier
