...
namespace StyleCop.Analyzers.DocumentationRules
{
        ...
        /// <summary>
        /// This is the base class for analyzers which examine the <c>&lt;summary&gt;</c> or
        /// <c>&lt;content&gt;</c> text of the documentation comment associated with a
        /// <c>partial</c> element.
        /// </summary>
        internal abstract class PartialElementDocumentationSummaryBase : DiagnosticAnalyzer
        {
                ...
                Initialize (AnalysisContext context)
                {
                        ...
                        context.RegisterSyntaxNodeAction (this.typeDeclarationAction,
                ...
                /// <summary>
                /// Analyzes the top-level <c>&lt;summary&gt;</c> or <c>&lt;content&gt;</c> element
                ...
                /// langword="false"/>.</param> <param name="syntax">The <see
                /// cref="XmlElementSyntax"/> or <see cref="XmlEmptyElementSyntax"/> of the node to
                /// examine.</param> <param name="completeDocumentation">The complete documentation
                /// for the declared symbol, with any <c>&lt;include&gt;</c> elements expanded. If
                /// the XML documentation comment included a <c>&lt;summary&gt;</c> element, this
                /// value will be <see langword="null"/>, even if the XML documentation comment also
                ...
                private static bool
                IsPartialMethodDefinition (SyntaxNode node)
                {
                ...
                {
                        // We handle TypeDeclarationSyntax instead of BaseTypeDeclarationSyntax
                        // because enums are not allowed to be partial.
                                         ...
                                         StyleCopSettings settings)
                {
                        ...
                        if (!node.Modifiers.Any (SyntaxKind.PartialKeyword))
                                {
                                        // non-partial elements are handled by
                                        // ElementDocumentationSummaryBase
                                        return;
                                }
                ...
                private void
                HandleDeclaration (SyntaxNodeAnalysisContext context, bool needsComment,
                                   SyntaxNode node, params Location[] locations)
                {
                                        ...
                                        if (relevantXmlElement != null)
                                                {
                                                        ...
                                                        if (IsPartialMethodDefinition (node))
                                                                {
                                                                        // Workaround: Roslyn does
                                                                        // not support expanding
                                                                        // include directives for
                                                                        // partial method
                                                                        // definitions.
                                                                        //             (see
                                                                        //             src/Compilers/CSharp/Portable/Compiler/DocumentationCommentCompiler.cs#L315)
                                                                        rawDocumentation
                                                        ...
                                                        completeDocumentation = XElement.Parse (
                        ...
                        this.HandleXmlElement (context, needsComment, relevantXmlElement,
                        ...
                        foreach (XmlNodeSyntax xmlNode in documentCommentTrivia.Content)
                                {
                                        if (xmlNode == includeTag)
                                                {
                                                        this.ExpandIncludeTag (compilation, sb,
                                                                               xmlNode);
                                                }
                                        else
                                                {
                ...
                private void
                ExpandIncludeTag (Compilation compilation, StringBuilder sb, XmlNodeSyntax xmlNode)
                {
                        try
                                {
                                        var includeElement = XElement.Parse (xmlNode.ToString (),
                                                                             LoadOptions.None);

                                        var fileAttribute = includeElement.Attribute (
                                            XName.Get (XmlCommentHelper.FileAttributeName));
                                        var pathAttribute = includeElement.Attribute (
                                            XName.Get (XmlCommentHelper.PathAttributeName));

                                        if ((fileAttribute != null) && (pathAttribute != null))
                                                {
                                                        var resolver = compilation.Options
