...
private static readonly LocalizableString SA1623MessageFormat =
	new LocalizableResourceString(nameof(DocumentationResources.SA1623MessageFormat),
	                              DocumentationResources.ResourceManager, typeof(DocumentationResources));
...
private static readonly LocalizableString SA1624MessageFormat =
	new LocalizableResourceString(nameof(DocumentationResources.SA1624MessageFormat),
	                              DocumentationResources.ResourceManager, typeof(DocumentationResources));
...
private static void AnalyzeSummaryElement(SyntaxNodeAnalysisContext context, XmlNodeSyntax syntax,
                                          Location diagnosticLocation,
                                          ...
                                          string startingTextGetsOrSets, string startingTextReturns)
{
	...
	bool prefixIsGets =
		!prefixIsGetsOrSets && text.StartsWith(startingTextGets, StringComparison.OrdinalIgnoreCase);
	...
	if (getter != null && setter != null)
	{
		...
		else
		{
			var propertyAccessibility =
				propertyDeclaration.GetEffectiveAccessibility(context.SemanticModel, context.CancellationToken);
			...
			else
			{
				var getterAccessibility =
					getter.GetEffectiveAccessibility(context.SemanticModel, context.CancellationToken);
				var setterAccessibility =
					setter.GetEffectiveAccessibility(context.SemanticModel, context.CancellationToken);
	...
	if (getterVisible)
	{
		if (setterVisible)
		{
			...
			{
				ReportSA1623(context, diagnosticLocation, diagnosticProperties, text, expectedStartingText
				             : startingTextGetsOrSets, unexpectedStartingText1
		...
		else if (setter != null)
		{
			...
			if (!prefixIsGets)
			{
				if (prefixIsGetsOrSets)
				{
					ReportSA1624(context, diagnosticLocation, diagnosticProperties, accessor
		...
		else
		{
			...
			if (!prefixIsGets)
			{
				ReportSA1623(context, diagnosticLocation, diagnosticProperties, text, expectedStartingText
				             : startingTextGets, unexpectedStartingText1
				             : startingTextSets, unexpectedStartingText2
				             : startingTextGetsOrSets, unexpectedStartingText3
				             : startingTextReturns);
			}
		}
	}
	else if (setterVisible)
	{
		...
		else
		{
			...
			if (!prefixIsSets)
			{
				ReportSA1623(context, diagnosticLocation, diagnosticProperties, text, expectedStartingText
				             : startingTextSets, unexpectedStartingText1
				             : startingTextGetsOrSets, unexpectedStartingText2
...
private static void ReportSA1623(SyntaxNodeAnalysisContext context, Location diagnosticLocation,
                                 ...
                                 string unexpectedStartingText2 = null, string unexpectedStartingText3 = null)
{
	...
	if (text.StartsWith(unexpectedStartingText1, StringComparison.OrdinalIgnoreCase))
	{
		diagnosticProperties.Add(TextToRemoveKey, text.Substring(0, unexpectedStartingText1.Length));
	         ...
	         text.StartsWith(unexpectedStartingText2, StringComparison.OrdinalIgnoreCase))
	{
		diagnosticProperties.Add(TextToRemoveKey, text.Substring(0, unexpectedStartingText2.Length));
	         ...
	         text.StartsWith(unexpectedStartingText3, StringComparison.OrdinalIgnoreCase))
	{
		diagnosticProperties.Add(TextToRemoveKey, text.Substring(0, unexpectedStartingText3.Length));
	}

	context.ReportDiagnostic(Diagnostic.Create(SA1623Descriptor, diagnosticLocation,
	                                           diagnosticProperties.ToImmutable(), expectedStartingText));
}

private static void ReportSA1624(SyntaxNodeAnalysisContext context, Location diagnosticLocation,
                                 ...
                                 string accessor, string expectedStartingText, string startingTextToRemove)
{
	...
	context.ReportDiagnostic(Diagnostic.Create(SA1624Descriptor, diagnosticLocation,
	                                           diagnosticProperties.ToImmutable(), accessor,
