...
namespace StyleCop.Analyzers.DocumentationRules
{
    ...
    internal class SA1626SingleLineCommentsMustNotUseDocumentationStyleSlashes : DiagnosticAnalyzer
    {
        ...
        private static readonly Action<SyntaxNodeAnalysisContext> SingleLineDocumentationTriviaAction =
            HandleSingleLineDocumentationTrivia;
            ...
            if (node.Content.All(x => x.IsKind(SyntaxKind.XmlText)))
            {
                foreach (var trivia in node.DescendantTrivia(descendIntoTrivia : true))
                {
                    ...
                    context.ReportDiagnostic(
                        Diagnostic.Create(Descriptor, Location.Create(trivia.SyntaxTree, slashes)));
                }
            }
        }
    }
