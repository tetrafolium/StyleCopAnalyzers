...
namespace StyleCop.Analyzers.NamingRules
{
        ...
        /// </summary>
        /// <remarks>
        /// <para>A violation of this rule occurs when the name of a parameter does not begin with a
        /// lower-case letter.</para>
        ///
        /// <para>An exception to this rule is made for lambda parameters named <c>_</c> and
        /// <c>__</c>. These parameters are often used to designate a placeholder parameter which is
        /// not actually used in the body of the lambda expression.</para>
        ///
        /// <para>If the parameter name is intended to match the name of an item associated with
        /// Win32 or COM, and thus needs to begin with an upper-case letter, place the parameter
        ...
        internal class SA1313ParameterNamesMustBeginWithLowerCaseLetter : DiagnosticAnalyzer
        {
                ...
                HandleParameter (SyntaxNodeAnalysisContext context)
                {
                        ...
                        if (syntax.Parent.Parent.IsKind (SyntaxKindEx.RecordDeclaration))
                                {
                                        // Positional parameters of a record are treated as
                ...
                private static bool
                IsInLambda (ParameterSyntax syntax)
                {
                ...
                private static bool
                NameMatchesAbstraction (ParameterSyntax syntax, SemanticModel semanticModel)
                {
                        if (!(syntax.Parent is ParameterListSyntax parameterList))
                                {
                                        // This occurs for simple lambda expressions (without
                                        // parentheses)
                                        return false;
                                }
                        ...
                        if (methodSymbol.IsOverride)
                                {
                                        ...
                                        // declaration -> exit because there is no meaningful
                                        // analysis to be done.
