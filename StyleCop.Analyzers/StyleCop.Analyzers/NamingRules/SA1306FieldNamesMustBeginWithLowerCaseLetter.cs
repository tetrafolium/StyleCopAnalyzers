...
namespace StyleCop.Analyzers.NamingRules
{
        ...
        /// </summary>
        /// <remarks>
        /// <para>A violation of this rule occurs when the name of a field begins with an upper-case
        /// letter. Constants, non-private readonly fields and static readonly fields should always
        /// start with an uppercase letter, whilst private readonly fields should start with a
        /// lowercase letter. Also, public or internal fields should always start with an uppercase
        /// letter.</para>
        ///
        /// <para>If the field name is intended to match the name of an item associated with Win32
        /// or COM, and thus needs to begin with an upper-case letter, place the field within a
        /// special <c>NativeMethods</c> class. A <c>NativeMethods</c> class is any class which
        /// contains a name ending in <c>NativeMethods</c>, and is intended as a placeholder for
        /// Win32 or COM wrappers. StyleCop will ignore this violation if the item is placed within
                ...
                HandleFieldDeclaration (SyntaxNodeAnalysisContext context)
                {
                        ...
                        if (syntax.Modifiers.Any (SyntaxKind.PublicKeyword)
                            || syntax.Modifiers.Any (SyntaxKind.InternalKeyword))
                                {
                                        // this diagnostic does not apply to public or internal read
                                        // only fields
                                        return;
                                }
                        ...
                        if (syntax.Modifiers.Any (SyntaxKind.ReadOnlyKeyword)
                            && syntax.Modifiers.Any (SyntaxKind.ProtectedKeyword))
                                {
                                        // this diagnostic does not apply to non-private read only
                                        // fields
                                        return;
                                }
