    ...
    internal class SA1134AttributesMustNotShareLine : DiagnosticAnalyzer
    {
        ...
        private static void HandleAttributeList(SyntaxNodeAnalysisContext context)
        {
            ...
            if (!nextToken.IsMissingOrDefault())
            {
                ...
                // do not report for trailing attribute lists, to prevent unnecessary diagnostics and issues with the
                // code fix
                if ((nextTokenLineSpan.EndLinePosition.Line == attributeListLineSpan.EndLinePosition.Line) &&
            ...
            if (violation)
            {
                context.ReportDiagnostic(Diagnostic.Create(Descriptor, attributeList.OpenBracketToken.GetLocation(),
