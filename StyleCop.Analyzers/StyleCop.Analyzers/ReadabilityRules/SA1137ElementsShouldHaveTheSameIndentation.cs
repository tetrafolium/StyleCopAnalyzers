...
private static readonly Action<SyntaxNodeAnalysisContext> InitializerExpressionAction =
	HandleInitializerExpression;
private static readonly Action<SyntaxNodeAnalysisContext> AnonymousObjectCreationExpressionAction =
...
private static void HandleSwitchStatement(SyntaxNodeAnalysisContext context)
{
	...
	foreach (SwitchSectionSyntax switchSection in switchStatement.Sections)
	{
		...
		if (switchSection.Statements.Count == 1 && switchSection
		    .Statements [0]
...
private static void AddMembersAndAttributes<T>(ImmutableList<SyntaxNode>.Builder elements,
...
private static void CheckElements<T>(SyntaxNodeAnalysisContext context, SyntaxList<T> elements) where T
: SyntaxNode
{
	if (elements.Count < 2)
	{
...
private static void CheckElements<T>(SyntaxNodeAnalysisContext context, SeparatedSyntaxList<T> elements) where T
: SyntaxNode
{
	if (elements.Count < 2)
	{
...
private static void CheckBlocks(SyntaxNodeAnalysisContext context, ImmutableList<BlockSyntax> elements)
{
	...
	foreach (BlockSyntax element in elements)
	{
		...
		string openBraceIndentation = openBraceIndentationTrivia.IsKind(SyntaxKind.WhitespaceTrivia)
		...
		string closeBraceIndentation = closeBraceIndentationTrivia.IsKind(SyntaxKind.WhitespaceTrivia)
		...
		{
			ReportDiagnostic(context, element.OpenBraceToken, openBraceIndentationTrivia, openBraceIndentation,
			                 expectedIndentation);
		}

		if (!string.Equals(expectedIndentation, closeBraceIndentation, StringComparison.Ordinal))
		{
			ReportDiagnostic(context, element.CloseBraceToken, closeBraceIndentationTrivia,
			                 closeBraceIndentation, expectedIndentation);
		}
	}
}

private static void CheckElements<T>(SyntaxNodeAnalysisContext context, ImmutableList<T> elements) where T
: SyntaxNode
{
	if (elements.Count < 2)
	{
	...
	foreach (T element in elements)
	{
		...
		string indentation =
			indentationTrivia.IsKind(SyntaxKind.WhitespaceTrivia) ? indentationTrivia.ToString() : string.Empty;
...
private static ImmutableList<T> CleanupElementsList<T>(ImmutableList<T> elements) where T : SyntaxNode
{
...
private static void CheckBraces(SyntaxNodeAnalysisContext context, SyntaxToken openBraceToken,
                                SyntaxToken closeBraceToken)
{
	...
	{
		ReportDiagnostic(context, closeBraceToken, closeBraceIndentationTrivia, closeBraceIndentation,
                                     ...
                                     string expectedIndentation)
{
	...
	ImmutableDictionary<string, string> properties =
		ImmutableDictionary.Create<string, string>().SetItem(ExpectedIndentationKey, expectedIndentation);
