...
namespace StyleCop.Analyzers.ReadabilityRules
{
        ...
        /// <summary>
        /// A call to an instance member of the local class or a base class is not prefixed with
        /// ‘this.’, within a C# code file.
        /// </summary>
        /// <remarks>
        /// <para>A violation of this rule occurs whenever the code contains a call to an instance
        /// member of the local class or a base class which is not prefixed with <c>this.</c>. An
        /// exception to this rule occurs when there is a local override of a base class member, and
        /// the code intends to call the base class member directly, bypassing the local override.
        /// In this case the call can be prefixed with <c>base.</c> rather than <c>this.</c>.</para>
        ///
        /// <para>By default, StyleCop disallows the use of underscores or <c>m_</c> to mark local
        /// class fields, in favor of the <c>this.</c> prefix. The advantage of using <c>this.</c>
        /// is that it applies equally to all element types including methods, properties, etc., and
        /// not just fields, making all calls to class members instantly recognizable, regardless of
        /// which editor is being used to view the code. Another advantage is that it creates a
        /// quick, recognizable differentiation between instance members and static members, which
        /// are not prefixed.</para>
        ///
        /// <para>A final advantage of using the <c>this.</c> prefix is that typing <c>this.</c>
        ...
        internal class SA1101PrefixLocalCallsWithThis : DiagnosticAnalyzer {
                ...
                public override void Initialize(AnalysisContext context)
                {
                        ...
                        context.RegisterSyntaxNodeAction(
                            MemberAccessExpressionAction, SyntaxKind.SimpleMemberAccessExpression);
                        context.RegisterSyntaxNodeAction(SimpleNameAction, SyntaxKinds.SimpleName);
                }

                /// <summary>
                /// <see cref="SyntaxKind.SimpleMemberAccessExpression"/> is handled separately so
                /// only <c>X</c> is evaluated in the expression <c>X.Y.Z.A.B.C</c>.
                /// </summary>
            ...
            case SyntaxKind.Argument when IsPartOfConstructorInitializer(
                (SimpleNameSyntax) context.Node):
            ...
            HandleIdentifierNameImpl(context, (SimpleNameSyntax) context.Node);
                ...
                private static void HandleIdentifierNameImpl(
                    SyntaxNodeAnalysisContext context, SimpleNameSyntax nameExpression)
                {
