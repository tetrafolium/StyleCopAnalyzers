...
namespace StyleCop.Analyzers.ReadabilityRules
{
        ...
        internal class SX1101DoNotPrefixLocalMembersWithThis : DiagnosticAnalyzer
        {
                ...
                /// cref="SX1101DoNotPrefixLocalMembersWithThis"/> analyzer.
                /// </summary>
                ...
                private static readonly LocalizableString Title = new LocalizableResourceString (
                    nameof (ReadabilityResources.SX1101Title), ReadabilityResources.ResourceManager,
                ...
                private static readonly Action<SyntaxNodeAnalysisContext> ThisExpressionAction
                ...
                Initialize (AnalysisContext context)
                {
                        ...
                        context.RegisterSyntaxNodeAction (ThisExpressionAction,
                                                          SyntaxKind.ThisExpression);
                }

                private static void
                HandleThisExpression (SyntaxNodeAnalysisContext context)
                {
                        ...
                        var memberAccessExpression
                            = (MemberAccessExpressionSyntax) context.Node.Parent;
                        var originalSymbolInfo = context.SemanticModel.GetSymbolInfo (
                            memberAccessExpression, context.CancellationToken);
                        ...
                        StatementSyntax statement
                            = context.Node.FirstAncestorOrSelf<StatementSyntax> ();
                        if (statement != null)
                                {
                                        var modifiedStatement = statement.ReplaceNode (
                                            memberAccessExpression,
                                            memberAccessExpression.Name.WithAdditionalAnnotations (
                                                annotation));
                                        speculationRoot = modifiedStatement;
                                        if (!context.SemanticModel.TryGetSpeculativeSemanticModel (
                                                statement.SpanStart, modifiedStatement,
                        ...
                        else
                                {
                                        ArrowExpressionClauseSyntax arrowExpressionClause
                                            = context.Node.FirstAncestorOrSelf<
                                        ...
                                        var modifiedArrowExpressionClause
                                            = arrowExpressionClause.ReplaceNode (
                                                memberAccessExpression,
                                                memberAccessExpression.Name
                                                    .WithAdditionalAnnotations (annotation));
                                        speculationRoot = modifiedArrowExpressionClause;
                                        if (!context.SemanticModel.TryGetSpeculativeSemanticModel (
                                                arrowExpressionClause.SpanStart,
                                                modifiedArrowExpressionClause,
                                                out speculativeModel))
                                                {
                                                        return;
                                                }
                        ...
                        SyntaxNode mappedNode
                            = speculationRoot.GetAnnotatedNodes (annotation).Single ();
                        var newSymbolInfo = speculativeModel.GetSymbolInfo (
                            mappedNode, context.CancellationToken);
