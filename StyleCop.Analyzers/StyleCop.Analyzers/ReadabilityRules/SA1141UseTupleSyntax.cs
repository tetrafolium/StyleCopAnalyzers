...
namespace StyleCop.Analyzers.ReadabilityRules
{
        ...
        {
                /// <summary>
                /// The ID for diagnostics produced by the <see cref="SA1141UseTupleSyntax"/>
                /// analyzer.
                /// </summary>
                ...
                private static readonly Action<SyntaxNodeAnalysisContext> DefaultExpressionAction
                ...
                Initialize (AnalysisContext context)
                {
                        ...
                        context.RegisterSyntaxNodeAction (ConversionOperatorAction,
                        ...
                        context.RegisterSyntaxNodeAction (DefaultExpressionAction,
                ...
                private static void
                HandleConversionOperator (SyntaxNodeAnalysisContext context)
                {
                        if (!context.SupportsTuples ())
                                {
                                        return;
                ...
                HandleObjectCreationExpression (SyntaxNodeAnalysisContext context)
                {
                        ...
                        CheckType (context, objectCreationExpression.Type,
                ...
                HandleInvocationExpression (SyntaxNodeAnalysisContext context)
                {
                        ...
                        if (invocationExpression.ArgumentList.Arguments.Count < 2)
                                {
                                        // Tuple creation with less than two elements cannot use the
                                        // language syntax
                                        return;
                                }

                        if (!invocationExpression.Expression.IsKind (
                        ...
                        if ((symbolInfo.Symbol is INamedTypeSymbol namedTypeSymbol)
                            && (namedTypeSymbol.ToDisplayString (DisplayFormat)
                                == "System.ValueTuple"))
                                {
                                        if (memberAccessExpression.Name.Identifier.ValueText
                                            == "Create")
                                                {
                                                        context.ReportDiagnostic (
                                                            Diagnostic.Create (
                                                                Descriptor, memberAccessExpression
                                                                                .GetLocation ()));
                                                }
                                }
                }
                ...
                private static void
                HandleDefaultExpression (SyntaxNodeAnalysisContext context)
                {
                        if (!context.SupportsTuples ())
                                {
                                        return;
                ...
                private static void
                CheckParameterList (SyntaxNodeAnalysisContext context,
                ...
                private static void
                CheckType (SyntaxNodeAnalysisContext context, TypeSyntax typeSyntax,
                ...
                private static void
                CheckTupleType (SyntaxNodeAnalysisContext context,
                                TupleTypeSyntaxWrapper tupleTypeSyntax, Location reportLocation)
                {
                ...
                private static void
                CheckGenericName (SyntaxNodeAnalysisContext context,
                                  GenericNameSyntax genericNameSyntax, Location reportLocation)
                {
                        if (IsValueTupleWithLanguageRepresentation (context, genericNameSyntax))
                                {
                                        var location
                                            = reportLocation ?? genericNameSyntax.GetLocation ();
                                        context.ReportDiagnostic (
                ...
                {
                        var symbolInfo = context.SemanticModel.GetSymbolInfo (
                            syntax, context.CancellationToken);
