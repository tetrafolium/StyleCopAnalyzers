...
namespace StyleCop.Analyzers.ReadabilityRules
{
        ...
        /// <summary>
        /// An anonymous method was declared using the form <c>delegate (parameters) { }</c>, when a
        /// lambda expression would provide equivalent behavior with the syntax <c>(parameters) => {
        /// }</c>.
        /// </summary>
        ...
        internal class SA1130UseLambdaSyntax : DiagnosticAnalyzer
        {
                ...
                private static readonly LocalizableString Title = new LocalizableResourceString (
                    nameof (ReadabilityResources.SA1130Title), ReadabilityResources.ResourceManager,
                    typeof (ReadabilityResources));
                private static readonly LocalizableString MessageFormat
                    = new LocalizableResourceString (
                        nameof (ReadabilityResources.SA1130MessageFormat),
                        ReadabilityResources.ResourceManager, typeof (ReadabilityResources));
                private static readonly LocalizableString Description
                    = new LocalizableResourceString (
                        nameof (ReadabilityResources.SA1130Description),
                        ReadabilityResources.ResourceManager, typeof (ReadabilityResources));
                ...
                /// </summary>
                /// <param name="symbol">The symbol containing information about the method
                /// invocation.</param> <param name="argumentIndex">The index of the argument
                /// containing the delegate.</param> <returns>A parameter list for the delegate
                /// parameters.</returns>
                internal static ParameterListSyntax
                GetDelegateParameterList (ISymbol symbol, int argumentIndex)
                {
                        ...
                        if (parameterList[realIndex].IsParams)
                                {
                                        if (type is IArrayTypeSymbol arrayTypeSymbol)
                                                ...
                                                {
                                                        // Just to make sure that we do not crash if
                                                        // in future versions of the compiler other
                                                        // types are allowed for params. This if
                                                        // statement should be extended if e.g. Span
                                                        // params are introduced into the language.
                                                        return null;
                                                }
                                }
                        ...
                        if (argumentSyntax?.Parent is BaseArgumentListSyntax argumentListSyntax)
                                {
                                        ...
                                        SymbolInfo originalSymbolInfo
                                            = semanticModel.GetSymbolInfo (
                                                originalInvocableExpression);
                                        Location location
                                            = originalInvocableExpression.GetLocation ();
                                        ...
                                        // Determine the parameter list from the method that is
                                        // invoked, as delegates without parameters are allowed, but
                                        // they cannot be replaced by a lambda without parameters.
                                        var parameterList = GetDelegateParameterList (
                                            originalSymbolInfo.Symbol, argumentIndex);

                                        if (parameterList == null)
                                                {
                                                        // This might happen if the call was using
                                                        // params witha type unknown to the
                                                        // analyzer, e.g. params Span<T>.
                                                        return false;
                                                }

                                        // In some cases passing a delegate as an argument to a
                                        // method is required to call the right overload When there
                                        // is an other overload that takes an expression.
                                        ...
                                        var invocationExpression
                                            = originalInvocableExpression.ReplaceNode (
                                        ...
                                        if (!originalSymbolInfo.Symbol.Equals (
                ...
                {
                        var result = ImmutableArray.CreateBuilder<ParameterSyntax> (
                            symbolParameters.Length);

                        foreach (var symbolParameter in symbolParameters)
                                {
                                        var syntaxParameter
                                            = GetParameterSyntaxFromParameterSymbol (
                                                symbolParameter);
