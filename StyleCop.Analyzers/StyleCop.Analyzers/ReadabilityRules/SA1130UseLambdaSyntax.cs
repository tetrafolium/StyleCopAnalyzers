...
namespace StyleCop.Analyzers.ReadabilityRules
{
        ...
        internal class SA1130UseLambdaSyntax : DiagnosticAnalyzer {
                ...
                /// parameters.</returns>
                internal static ParameterListSyntax GetDelegateParameterList(
                    ISymbol symbol, int argumentIndex)
                {
                        ...
                        if (parameterList[realIndex].IsParams) {
                                if (type is IArrayTypeSymbol arrayTypeSymbol) {
                                        ...
                                        // Just to make sure that we do not crash if in future
                                        // versions of the compiler other types are allowed for
                                        // params. This if statement should be extended if e.g. Span
                    ...
                    SyntaxNodeAnalysisContext context)
                {
                        ...
                        case SyntaxKind.Argument:
                                reportDiagnostic = HandleMethodInvocation(context.SemanticModel,
                                    anonymousMethod, (ArgumentSyntax) anonymousMethod.Parent);
                                break;
                ...
                private static bool HandleMethodInvocation(SemanticModel semanticModel,
                    AnonymousMethodExpressionSyntax anonymousMethod, ArgumentSyntax argumentSyntax)
                {
                        ...
                        if (argumentSyntax?.Parent is BaseArgumentListSyntax argumentListSyntax) {
                                ...
                                if (originalSymbolInfo.Symbol == null) {
                                        // The most likely cause for this is an overload resolution
                                        // failure.
                                        return false;
                                }
                                ...
                                if (parameterList == null) {
                                        ...
                                        // unknown to the analyzer, e.g. params Span<T>.
                                        return false;
                                }

                                // In some cases passing a delegate as an argument to a method is
