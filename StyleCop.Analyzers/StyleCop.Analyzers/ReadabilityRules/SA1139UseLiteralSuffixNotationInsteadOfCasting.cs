        ...
        internal class SA1139UseLiteralSuffixNotationInsteadOfCasting : DiagnosticAnalyzer {
                        ...
                        if (unaryExpressionSyntax != null) {
                                if (unaryExpressionSyntax.Kind() != SyntaxKind.UnaryPlusExpression
                                    && unaryExpressionSyntax.Kind()
                                        ...
                                        // don't report diagnostic if bit operations are performed
                                        // and for some invalid code (eg. "(long)++1")
                                        return;
                                }
                        }
                ...
                ? castExpressionSyntax.Expression
                        ...
                        if (!context.SemanticModel.GetConstantValue(context.Node).HasValue) {
                                // cast does not have a valid value (like "(ulong)-1") which is
                                // reported as error
                                return;
                        }

                        var speculativeExpression
                            = castExpressionSyntax.Expression.ReplaceNode(castedElementTypeSyntax,
