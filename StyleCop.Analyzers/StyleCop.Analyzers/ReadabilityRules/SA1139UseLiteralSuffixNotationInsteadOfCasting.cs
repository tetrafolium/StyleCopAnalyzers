...
namespace StyleCop.Analyzers.ReadabilityRules
{
        ...
        /// <summary>
        /// A cast is performed instead of using literal of a number. Use "U" suffix to create
        /// 32-bit unsigned integer, "L" for 64-bit integer, "UL" for 64-bit unsigned integer, "F"
        /// for 32-bit floating point number, "D" for 64-bit floating point number, and "M" for a
        /// decimal number.
        /// </summary>
        ...
        internal class SA1139UseLiteralSuffixNotationInsteadOfCasting : DiagnosticAnalyzer
        {
                ...
                /// cref="SA1139UseLiteralSuffixNotationInsteadOfCasting"/> analyzer.
                /// </summary>
                ...
                private static readonly LocalizableString MessageFormat
                    = new LocalizableResourceString (
                        nameof (ReadabilityResources.SA1139MessageFormat),
                        ReadabilityResources.ResourceManager, typeof (ReadabilityResources));
                private static readonly LocalizableString Description
                    = new LocalizableResourceString (
                        nameof (ReadabilityResources.SA1139Description),
                        ReadabilityResources.ResourceManager, typeof (ReadabilityResources));
                ...
                HandleCastExpression (SyntaxNodeAnalysisContext context)
                {
                        ...
                        if (!(castExpressionSyntax.Type is PredefinedTypeSyntax
                                  castingToTypeSyntax))
                                {
                                        return;
                                }
                        ...
                        var unaryExpressionSyntax
                            = castExpressionSyntax.Expression.WalkDownParentheses ()
                                  as PrefixUnaryExpressionSyntax;
                        if (unaryExpressionSyntax != null)
                                {
                                        if (unaryExpressionSyntax.Kind ()
                                                != SyntaxKind.UnaryPlusExpression
                                            && unaryExpressionSyntax.Kind ()
                                                   != SyntaxKind.UnaryMinusExpression)
                                                {
                                                        // don't report diagnostic if bit operations
                                                        // are performed and for some invalid code
                                                        // (eg. "(long)++1")
                                                        return;
                                                }
                                }

                        var castedElementTypeSyntax = unaryExpressionSyntax
                                                      == null
                ? castExpressionSyntax.Expression
                                                             .WalkDownParentheses ()
                                                                 as LiteralExpressionSyntax
                            : unaryExpressionSyntax.Operand.WalkDownParentheses ()
                                  as LiteralExpressionSyntax;
                        ...
                        if (targetType.Equals (
                                context.SemanticModel.GetTypeInfo (castedElementTypeSyntax).Type))
                                {
                                        // cast is redundant which is reported by another
                                        // diagnostic.
                                        return;
                                }
                        ...
                        if (!context.SemanticModel.GetConstantValue (context.Node).HasValue)
                                {
                                        // cast does not have a valid value (like "(ulong)-1") which
                                        // is reported as error
                                        return;
                                }

                        var speculativeExpression = castExpressionSyntax.Expression.ReplaceNode (
                            castedElementTypeSyntax,
                            castedElementTypeSyntax.WithLiteralSuffix (syntaxKindKeyword));
                        var speculativeTypeInfo = context.SemanticModel.GetSpeculativeTypeInfo (
                            castExpressionSyntax.SpanStart, speculativeExpression,
                        ...
                        context.ReportDiagnostic (
                            Diagnostic.Create (Descriptor, castExpressionSyntax.GetLocation ()));
                }
        }
}
