...
namespace StyleCop.Analyzers.ReadabilityRules
{
        ...
        internal class SA110xQueryClauses : DiagnosticAnalyzer
        {
                ...
                private static readonly LocalizableString SA1102Description
                    = new LocalizableResourceString (
                        nameof (ReadabilityResources.SA1102Description),
                        ReadabilityResources.ResourceManager, typeof (ReadabilityResources));
                ...
                private static readonly LocalizableString SA1103Description
                    = new LocalizableResourceString (
                        nameof (ReadabilityResources.SA1103Description),
                        ReadabilityResources.ResourceManager, typeof (ReadabilityResources));
                ...
                private static readonly LocalizableString SA1104Description
                    = new LocalizableResourceString (
                        nameof (ReadabilityResources.SA1104Description),
                        ReadabilityResources.ResourceManager, typeof (ReadabilityResources));
                ...
                private static readonly LocalizableString SA1105Description
                    = new LocalizableResourceString (
                        nameof (ReadabilityResources.SA1105Description),
                        ReadabilityResources.ResourceManager, typeof (ReadabilityResources));
                ...
                Initialize (AnalysisContext context)
                {
                        ...
                        context.RegisterSyntaxNodeAction (QueryExpressionAction,
                                                          SyntaxKind.QueryExpression);
                }

                private static void
                HandleQueryExpression (SyntaxNodeAnalysisContext context)
                {
                        ...
                        for (var i = 0; i < tokensToCheck.Count - 1; i++)
                                {
                                        ...
                                        // The node before the query clause may encompass the entire
                                        // query clause, so we need to use the token within that
                                        // node for the line determination.
                                        ...
                                        if (((location2.StartLinePosition.Line
                                             ...
                                             > 1)
                                            && isEnabledSA1102
                                            && !token2.LeadingTrivia.Any (
                                                trivia => trivia.IsDirective))
                                                {
                                            ...
                                            && parent1.SpansMultipleLines ())
                                                {
                                                        ...
                                                        // Make sure that SA1103 will not be
                                                        // reported, as there is a more specific
                                                        // diagnostic reported.
                                                        suppressSA1103 = true;
                                                }

                                        if (onSameLine && isEnabledSA1105
                ...
                private static void
                HandleQueryBody (QueryBodySyntax body, List<SyntaxToken> tokensToCheck)
                {
                ...
                private static void
                HandleQueryClause (QueryClauseSyntax queryClause, List<SyntaxToken> tokensToCheck)
                {
                ...
                private static void
                HandleSelectOrGroup (SelectOrGroupClauseSyntax selectOrGroup,
                ...
                private static void
                HandleContinuation (QueryContinuationSyntax continuation,
                                    List<SyntaxToken> tokensToCheck)
                {
