...
namespace StyleCop.Analyzers.ReadabilityRules
{
        ...
        /// </summary>
        /// <remarks>
        /// <para>A violation of this rule occurs when the parameters to a method or indexer span
        /// across multiple lines, but the first parameter does not start on the line after the
        /// opening bracket. For example:</para> <code language="csharp"> public string
        /// JoinName(string first,     string last)
        /// {
        /// }
        /// </code>
        /// <para>The parameters should begin on the line after the declaration, whenever the
        /// parameter span across multiple lines:</para> <code language="csharp"> public string
        /// JoinName(     string first,     string last)
        /// {
        /// }
        /// </code>
        ...
        internal class SA1116SplitParametersMustStartOnLineAfterDeclaration : DiagnosticAnalyzer {
                ...
                /// cref="SA1116SplitParametersMustStartOnLineAfterDeclaration"/> analyzer.
                /// </summary>
                ...
                private static readonly LocalizableString Title
                    = new LocalizableResourceString(nameof(ReadabilityResources.SA1116Title),
                        ReadabilityResources.ResourceManager, typeof(ReadabilityResources));
                private static readonly LocalizableString MessageFormat
                    = new LocalizableResourceString(
                        nameof(ReadabilityResources.SA1116MessageFormat),
                        ReadabilityResources.ResourceManager, typeof(ReadabilityResources));
                private static readonly LocalizableString Description
                    = new LocalizableResourceString(nameof(ReadabilityResources.SA1116Description),
                        ReadabilityResources.ResourceManager, typeof(ReadabilityResources));
                ...
                private static readonly Action<SyntaxNodeAnalysisContext>
                    ElementBindingExpressionAction = HandleElementBindingExpression;
                private static readonly Action<SyntaxNodeAnalysisContext>
                    ImplicitElementAccessAction = HandleImplicitElementAccess;
                private static readonly Action<SyntaxNodeAnalysisContext>
                ...
                public override void Initialize(AnalysisContext context)
                {
                        ...
                        context.RegisterSyntaxNodeAction(
                            ElementBindingExpressionAction, SyntaxKind.ElementBindingExpression);
                        context.RegisterSyntaxNodeAction(
                            ImplicitElementAccessAction, SyntaxKind.ImplicitElementAccess);
                        context.RegisterSyntaxNodeAction(
                ...
                private static void HandleIndexerDeclaration(SyntaxNodeAnalysisContext context)
                {
                        ...
                        BracketedParameterListSyntax argumentListSyntax
                            = indexerDeclaration.ParameterList;
                        SeparatedSyntaxList<ParameterSyntax> arguments
                            = argumentListSyntax.Parameters;

                        if (arguments.Count > 1) {
                                Analyze(context, argumentListSyntax.OpenBracketToken, arguments[0],
                                    arguments[1]);
                        }
                }
                ...
                private static void HandleArrayCreationExpression(SyntaxNodeAnalysisContext context)
                {
                        ...
                        foreach (var rankSpecifier in arrayCreation.Type.RankSpecifiers) {
                                ...
                                if (sizes.Count > 1) {
                                        Analyze(context, rankSpecifier.OpenBracketToken, sizes[0],
                                            sizes[1]);
                                }
                        }
                }
                ...
                private static void HandleAttribute(SyntaxNodeAnalysisContext context)
                {
                        ...
                        if (arguments.Count > 1) {
                                Analyze(context, argumentListSyntax.OpenParenToken, arguments[0],
                                    arguments[1]);
                        }
                }
                ...
                private static void HandleImplicitElementAccess(SyntaxNodeAnalysisContext context)
                {
                        ...
                        HandleBracketedArgumentListSyntax(
                            context, implicitElementAccess.ArgumentList);
                }

                private static void HandleArgumentListSyntax(
                    SyntaxNodeAnalysisContext context, ArgumentListSyntax argumentList)
                {
                        ...
                        if (parameters.Count > 1) {
                                Analyze(context, argumentList.OpenParenToken, parameters[0],
                                    parameters[1]);
                        }
                }
                    ...
                    SyntaxNodeAnalysisContext context, ParameterListSyntax parameterList)
                {
                        ...
                        if (parameters.Count > 1) {
                                Analyze(context, parameterList.OpenParenToken, parameters[0],
                                    parameters[1]);
                        }
                }

                private static void HandleBracketedArgumentListSyntax(
                    SyntaxNodeAnalysisContext context,
                    BracketedArgumentListSyntax bracketedArgumentList)
                {
                        ...
                        if (parameters.Count > 1) {
                                Analyze(context, bracketedArgumentList.OpenBracketToken,
                                    parameters[0], parameters[1]);
                        }
                }

                private static void Analyze(SyntaxNodeAnalysisContext context,
                    SyntaxToken openParenOrBracketToken, SyntaxNode firstParameter,
                    SyntaxNode secondParameter)
                {
                        ...
                        if (openParenOrBracketToken.GetLine() == firstParameterLine) {
                                if (firstParameterLine
                                    != secondParameter.GetLineSpan().StartLinePosition.Line) {
