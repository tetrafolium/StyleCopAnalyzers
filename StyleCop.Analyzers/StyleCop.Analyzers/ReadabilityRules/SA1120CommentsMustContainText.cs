...
namespace StyleCop.Analyzers.ReadabilityRules
{
        ...
        internal class SA1120CommentsMustContainText : DiagnosticAnalyzer
        {
                ...
                private static readonly LocalizableString Description
                    = new LocalizableResourceString (
                        nameof (ReadabilityResources.SA1120Description),
                        ReadabilityResources.ResourceManager, typeof (ReadabilityResources));
                ...
                private static void
                HandleMultiLineComment (SyntaxTreeAnalysisContext context,
                        ...
                        if (string.IsNullOrWhiteSpace (commentText))
                                {
                                        var diagnostic = Diagnostic.Create (
                                            Descriptor, multiLineComment.GetLocation ());
                                        context.ReportDiagnostic (diagnostic);
                                }
                }

                private static void
                HandleSingleLineComment (SyntaxTreeAnalysisContext context,
                                         SyntaxTrivia singleLineComment)
                {
                        ...
                        var firstNonWhiteSpace
                            = TriviaHelper.IndexOfFirstNonWhitespaceTrivia (list);
                        ...
                        if (!isFirst)
                                {
                                        // This is -2 because we need to go back past the end of
                                        // line trivia as well.
                ...
                private static bool
                IsNullOrWhiteSpace (string value, int startIndex)
                {
                        if (value == null)
                                {
                                        return true;
