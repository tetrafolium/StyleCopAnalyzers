...
namespace StyleCop.Analyzers.MaintainabilityRules
{
        ...
        /// </summary>
        /// <remarks>
        /// <para>It is possible in C# to insert parenthesis around virtually any type of
        /// expression, statement, or clause, and in many situations use of parenthesis can greatly
        /// improve the readability of the code. However, excessive use of parenthesis can have the
        /// opposite effect, making it more difficult to read and maintain the code.</para>
        ///
        /// <para>A violation of this rule occurs when parenthesis are used in situations where they
        /// provide no practical value. Typically, this happens anytime the parenthesis surround an
        /// expression which does not strictly require the use of parenthesis, and the parenthesis
        /// expression is located at the root of a statement. For example, the following lines of
        /// code all contain unnecessary parenthesis which will result in violations of this
        /// rule:</para>
        ///
        ...
        /// </code>
        ///
        /// <para>In each of these statements, the extra parenthesis can be removed without
        /// sacrificing the readability of the code:</para>
        ///
        /// <code language="csharp">
        /// int x = 5 + b;
        /// string y = this.Method().ToString();
        ...
        internal class SA1119StatementMustNotUseUnnecessaryParenthesis : DiagnosticAnalyzer
        {
                ...
                /// cref="SA1119StatementMustNotUseUnnecessaryParenthesis"/> analyzer.
                /// </summary>
                public const string DiagnosticId = "SA1119";

                /// <summary>
                /// The ID for the helper diagnostic used to mark the parentheses tokens surrounding
                /// the expression with <see cref="WellKnownDiagnosticTags.Unnecessary"/>.
                /// </summary>
                ...
                private static readonly LocalizableString Title = new LocalizableResourceString (
                    nameof (MaintainabilityResources.SA1119Title),
                ...
                private static readonly LocalizableString Description
                    = new LocalizableResourceString (
                        nameof (MaintainabilityResources.SA1119Description),
                        MaintainabilityResources.ResourceManager,
                        typeof (MaintainabilityResources));
...
#pragma warning restore RS2000 // Add analyzer diagnostic IDs to analyzer release.
                ...
                private static readonly Action<SyntaxNodeAnalysisContext>
                    ParenthesizedExpressionAction = HandleParenthesizedExpression;
                                            ...
                                            && !node.IsKind (SyntaxKind.ConstructorDeclaration))
                                                {
                                                        ...
                                                        if (IsSwitchOrWithExpressionPrecededByTypeCast (
                                                                node))
                                                                {
                                                                        return;
                                                                }

                                                        if (IsSwitchOrWithExpressionExpressionOfMemberAccess (
                                                                node))
                                                                {
                                                                        return;
                                                                }
                                        ...
                                        else
                                                {
                                                        if (node.Parent is InterpolationSyntax
                                                            && IsConditionalAccessInInterpolation (
                                                                node.Expression))
                                                                {
                                                                        // Parenthesis can't be
                                                                        // removed here
                                                                        return;
                                                                }
                ...
                private static bool
                IsConditionalAccessInInterpolation (ExpressionSyntax node)
                {
                        ...
                        while (expressionToCheck.Count > 0)
                                {
                                        ...
                                        if (currentNode.IsKind (SyntaxKind.ConditionalExpression))
                                                ...
                                                {
                                                        // We have to use parenthesis if the
                                                        // conditional access is in an interpolation
                                                        // inside an assignment.
                                                        var assignment = currentNode
                                                            as AssignmentExpressionSyntax;
                                                        expressionToCheck.Enqueue (assignment.Left);
                                                        expressionToCheck.Enqueue (
                                                            assignment.Right);
                                                }
                                }
                ...
                IsSwitchOrWithExpressionPrecededByTypeCast (ParenthesizedExpressionSyntax node)
                {
                        ...
                        while (previousToken.IsKind (SyntaxKind.OpenParenToken)
                               && previousToken.Parent.IsKind (SyntaxKind.ParenthesizedExpression))
                                {
                                        previousToken = previousToken.GetPreviousToken ();
                                }

                        return previousToken.IsKind (SyntaxKind.CloseParenToken)
                               && previousToken.Parent.IsKind (SyntaxKind.CastExpression);
                }

                private static bool
                IsSwitchOrWithExpressionExpressionOfMemberAccess (
                    ParenthesizedExpressionSyntax node)
                {
                        ...
                        return node.Parent switch {
                                MemberAccessExpressionSyntax
                                    memberAccessExpression => memberAccessExpression.Expression
                                                              == node,
                                ConditionalAccessExpressionSyntax
                                    conditionalAccessExpression => conditionalAccessExpression
                                                                       .Expression
                                                                   == node,
                                ElementAccessExpressionSyntax
                ...
                private static void
                ReportDiagnostic (SyntaxNodeAnalysisContext context,
                                  ParenthesizedExpressionSyntax node)
                {
                        context.ReportDiagnostic (
                            Diagnostic.Create (Descriptor, node.GetLocation ()));
                        context.ReportDiagnostic (Diagnostic.Create (
                            ParenthesisDescriptor, node.OpenParenToken.GetLocation ()));
                        context.ReportDiagnostic (Diagnostic.Create (
