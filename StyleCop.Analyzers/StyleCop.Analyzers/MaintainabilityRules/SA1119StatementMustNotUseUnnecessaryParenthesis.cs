...
private static readonly LocalizableString MessageFormat =
	new LocalizableResourceString(nameof(MaintainabilityResources.SA1119MessageFormat),
	                              MaintainabilityResources.ResourceManager, typeof(MaintainabilityResources));
...
#pragma warning disable RS2000 // Add analyzer diagnostic IDs to analyzer release.
	new DiagnosticDescriptor(
		ParenthesesDiagnosticId, Title, MessageFormat, AnalyzerCategory.MaintainabilityRules,
		DiagnosticSeverity.Hidden, AnalyzerConstants.EnabledByDefault, Description, HelpLink, customTags
...
private static readonly Action<SyntaxNodeAnalysisContext> ParenthesizedExpressionAction =
...
private static void HandleParenthesizedExpression(SyntaxNodeAnalysisContext context)
{
	...
	{
		if (!(node.Expression is BinaryExpressionSyntax) && !(node.Expression is AssignmentExpressionSyntax) &&
		    ...
		    !node.IsKind(SyntaxKind.ConstructorDeclaration))
		{
			if (node.Expression.IsKind(SyntaxKind.ConditionalAccessExpression) &&
			    (node.Parent is ElementAccessExpressionSyntax || node.Parent is MemberAccessExpressionSyntax ||
		...
		{
			if (node.Parent is InterpolationSyntax && IsConditionalAccessInInterpolation(node.Expression))
			{
				// Parenthesis can't be removed here
				return;
			}

			if (!(node.Parent is ExpressionSyntax) || node.Parent is CheckedExpressionSyntax ||
