...
/// It is used to share code in diagnostics <see cref="SA1405DebugAssertMustProvideMessageText"/> and <see
...
{
/// <summary>
/// Analyzes a <see cref="InvocationExpressionSyntax"/> node to add a diagnostic to static method calls in <see
/// cref="System.Diagnostics.Debug"/>. The diagnostic is added if the parameter count is lower than <paramref
/// name="parameterIndex"/> or the string given at the index can be evaluated to null, string.Empty or just
/// whitespaces.
/// </summary>
...
protected internal static void HandleInvocationExpression(SyntaxNodeAnalysisContext context, string methodName,
                                                          int parameterIndex, DiagnosticDescriptor descriptor)
{
	...
	var name =
		memberAccessExpressionSyntax?.Name?.Identifier.ValueText ?? identifierNameSyntax?.Identifier.ValueText;
	if (name == methodName)
	{
		if (context.SemanticModel.GetSymbolInfo(invocationExpressionSyntax).Symbol is IMethodSymbol symbolInfo)
		{
			...
			if (Equals(symbolInfo.ContainingType, debugType) && symbolInfo.Name == methodName)
			{
				if ((invocationExpressionSyntax.ArgumentList?.Arguments.Count ?? 0) <= parameterIndex)
				{
					...
					context.ReportDiagnostic(
						Diagnostic.Create(descriptor, invocationExpressionSyntax.GetLocation()));
				}
				else
				{
					...
					if (messageParameter?.Expression != null)
					{
						Optional<object> constantValue =
							context.SemanticModel.GetConstantValue(messageParameter.Expression);
