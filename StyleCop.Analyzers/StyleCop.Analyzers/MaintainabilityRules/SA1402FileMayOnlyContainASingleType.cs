...
private static readonly LocalizableString MessageFormat =
	new LocalizableResourceString(nameof(MaintainabilityResources.SA1402MessageFormat),
	                              MaintainabilityResources.ResourceManager, typeof(MaintainabilityResources));
...
private static void HandleSyntaxTree(SyntaxTreeAnalysisContext context, StyleCopSettings settings)
{
	...
	var preferredTypeNode =
		typeNodes.FirstOrDefault(n => FileNameHelpers.GetConventionalFileName(
	...
	foreach (var typeNode in typeNodes)
	{
		if (typeNode == preferredTypeNode ||
		    (isPartialType && foundTypeName == NamedTypeHelpers.GetNameOrIdentifier(typeNode)))
		{
			continue;
		}
...
private static IEnumerable<MemberDeclarationSyntax> GetTopLevelTypeDeclarations(SyntaxNode root,
