...
namespace StyleCop.Analyzers.Lightup
{
        ...
        internal static class LightupHelpers {
                private static readonly ConcurrentDictionary<Type,
                    ConcurrentDictionary<SyntaxKind, bool>> SupportedWrappers
                ...
                internal static bool CanWrapNode(SyntaxNode node, Type underlyingType)
                {
                        ...
                        if (!wrappedSyntax.TryGetValue(node.Kind(), out canCast)) {
                                canCast = wrappedSyntax.GetOrAdd(node.Kind(),
                ...
                CreateOperationPropertyAccessor<TOperation, TProperty>(
                    Type type, string propertyName)
                {
                        TProperty FallbackAccessor(TOperation syntax)
                        {
                                if (syntax == null) {
                                        ...
                                        // ArgumentNullException here, the light-up behavior needs
                                        // to match behavior of the underlying property.
                ...
                CreateSeparatedSyntaxListPropertyAccessor<TSyntax, TProperty>(
                    Type type, string propertyName)
                {
                ...
                CreateSyntaxWithPropertyAccessor<TSyntax, TProperty>(Type type, string propertyName)
                {
                        ...
                        Expression instance
                            ...
                            ? (Expression) syntaxParameter
                            : Expression.Convert(syntaxParameter, type);
                        Expression value = property.PropertyType.GetTypeInfo().IsAssignableFrom(
                ...
                CreateSeparatedSyntaxListWithPropertyAccessor<TSyntax, TProperty>(
                    Type type, string propertyName)
                {
                        TSyntax FallbackAccessor(
                            TSyntax syntax, SeparatedSyntaxListWrapper<TProperty> newValue)
                        {
                                if (syntax == null) {
                                        // Unlike an extension method which would throw
                                        // ArgumentNullException here, the light-up behavior needs
                        ...
                        var underlyingListProperty
                            = typeof(SeparatedSyntaxListWrapper<TProperty>)
                                  .GetTypeInfo()
                                  .GetDeclaredProperty(
                                      nameof(SeparatedSyntaxListWrapper<TProperty>.UnderlyingList));
