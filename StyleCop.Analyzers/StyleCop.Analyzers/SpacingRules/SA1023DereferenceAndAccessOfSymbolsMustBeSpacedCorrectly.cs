...
namespace StyleCop.Analyzers.SpacingRules
{
        ...
        /// </summary>
        /// <remarks>
        /// <para>A violation of this rule occurs when the spacing around a dereference or access-of
        /// symbol is not correct.</para>
        ///
        /// <para>The spacing around the symbol depends upon whether the symbol is used within a
        /// type declaration. If so, the symbol should always be followed by a single space, unless
        /// it is the last character on the line, or is followed by an opening square bracket or a
        /// parenthesis. In addition, the symbol should not be preceded by whitespace, and should
        /// not be the first character on the line. An example of a properly spaced dereference
        ...
        /// </code>
        ///
        /// <para>When a dereference or access-of symbol is used outside of a type declaration, the
        /// opposite rule applies. In this case, the symbol should always be preceded by a single
        /// space, unless it is the first character on the line, or is preceded by an opening square
        /// bracket, a parenthesis or a symbol of the same type i.e. an equals. The symbol should
        /// not be followed by whitespace, and should not be the last character on the line. For
        /// example:</para>
        ///
        /// <code language="cs">
        /// y = *x;
        ...
        internal class SA1023DereferenceAndAccessOfSymbolsMustBeSpacedCorrectly : DiagnosticAnalyzer
        {
                ...
                private static readonly LocalizableString Title = new LocalizableResourceString (
                    nameof (SpacingResources.SA1023Title), SpacingResources.ResourceManager,
                ...
                private static readonly LocalizableString MessageFollowed
                    = new LocalizableResourceString (
                        nameof (SpacingResources.SA1023MessageFollowed),
                        SpacingResources.ResourceManager, typeof (SpacingResources));
...
#pragma warning disable SA1202 // Elements should be ordered by access
                ...
                internal static readonly DiagnosticDescriptor DescriptorNotAtBeginningOfLine
                    = new DiagnosticDescriptor (
                        DiagnosticId, Title, MessageNotAtBeginningOfLine,
...
#pragma warning restore SA1202 // Elements should be ordered by access
                ...
                private static void
                HandleAsteriskToken (SyntaxTreeAnalysisContext context, SyntaxToken token)
                {
                        if (token.IsMissing)
                                {
                                        return;
                        ...
                        if (!allowAtLineStart && firstInLine)
                                {
                                        // Dereference symbol '*' should {not appear at the
                                        // beginning of a line}.
...
#pragma warning restore RS1005 // ReportDiagnostic invoked with an unsupported DiagnosticDescriptor
                                ...
                                {
                                        // Dereference symbol '*' should {not be preceded by a
                                        // space}.
                                        var properties = TokenSpacingProperties.RemovePreceding;
                                        context.ReportDiagnostic (
                                            Diagnostic.Create (DescriptorNotPreceded,
                                                               token.GetLocation (), properties));
                                }

                        if (!allowAtLineEnd && lastInLine)
                                {
                                        // Dereference symbol '*' should {not appear at the end of a
                                        // line}.
                                        var properties = TokenSpacingProperties.RemoveFollowing;
#pragma warning disable RS1005 // ReportDiagnostic invoked with an unsupported DiagnosticDescriptor
                               // (https://github.com/dotnet/roslyn-analyzers/issues/4103)
                                        context.ReportDiagnostic (
                                            Diagnostic.Create (DescriptorNotAtEndOfLine,
                                                               token.GetLocation (), properties));
#pragma warning restore RS1005 // ReportDiagnostic invoked with an unsupported DiagnosticDescriptor
                                }
                        else if (!allowTrailingSpace && followedBySpace)
                                {
                                        // Dereference symbol '*' should {not be followed by a
                                        // space}.
                                        var properties = TokenSpacingProperties.RemoveFollowing;
#pragma warning disable RS1005 // ReportDiagnostic invoked with an unsupported DiagnosticDescriptor
                               // (https://github.com/dotnet/roslyn-analyzers/issues/4103)
