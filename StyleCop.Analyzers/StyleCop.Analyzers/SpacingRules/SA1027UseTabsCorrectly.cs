...
namespace StyleCop.Analyzers.SpacingRules
{
        ...
        /// <summary>
        /// The code contains a tab or space character which is not consistent with the current
        /// project settings.
        /// </summary>
                        ...
                        for (int startIndex = 0; startIndex < length; startIndex++)
                                {
                                        if (startIndex == nextExcludedSpan.Start)
                                                {
                                                        ...
                                                        nextExcludedSpan
                                                            = excludedSpanIndex
                                                              < excludedSpans.Length ?
                                                                    excludedSpans[excludedSpanIndex]
                                                            : lastExcludedSpan;
                                                        continue;
                                                }
                                        ...
                                        switch (completeText[startIndex])
                                                ...
                                                case '\r':
                                                case '\n':
                                                        // Handle newlines. We can ignore CR/LF/CRLF
                                                        // issues because we are only tracking
                                                        // column position in a line, and not the
                                        ...
                                        int endIndex;
                                        for (endIndex = startIndex + 1; endIndex < length;
                                             endIndex++)
                                                {
                                        ...
                                        if (useTabs && startIndex == lastLineStart)
                                                {
                                                        // For the case we care about in the
                                                        // following condition (tabAfterSpace is
                                                        // false), spaceCount is the number of
                                        ...
                                        if (trivia.IsKind (SyntaxKind.DisabledTextTrivia))
                                        ...
                                        else if (trivia.IsKind (SyntaxKind.SingleLineCommentTrivia))
                                                {
                                                        if (trivia.ToString ().StartsWith ("////"))
                                                                {
                                                                        // Exclude comments starting
                                                                        // with //// because they
                                                                        // could contain commented
                                                                        // code which contains
                                                                        // string or character
                                                                        // literals, and we don't
                                                                        // want to change the
                                                                        // contents of those
                                                                        // strings.
                                                                        builder.Add (trivia.Span);
                        ...
                        for (int nextIndex = 1; nextIndex < sortedTextSpans.Count; nextIndex++)
                                {
                                        ...
                                        // Since sortedTextSpans is sorted, we already know current
                                        // and next overlap
