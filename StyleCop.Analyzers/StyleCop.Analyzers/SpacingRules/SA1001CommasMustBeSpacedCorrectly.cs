...
namespace StyleCop.Analyzers.SpacingRules
{
        ...
        /// </summary>
        /// <remarks>
        /// <para>A violation of this rule occurs when the spacing around a comma is
        /// incorrect.</para>
        ///
        /// <para>A comma should always be followed by a single space, unless it is the last
        /// character on the line or it is part of a string interpolation alignment component, and a
        /// comma should never be preceded by any whitespace, unless it is the first character on
        /// the line.</para>
        /// </remarks>
        ...
        internal class SA1001CommasMustBeSpacedCorrectly : DiagnosticAnalyzer
        {
                ...
                private static readonly LocalizableString Title = new LocalizableResourceString (
                    nameof (SpacingResources.SA1001Title), SpacingResources.ResourceManager,
                ...
                private static void
                HandleCommaToken (SyntaxTreeAnalysisContext context, SyntaxToken token)
                {
                        if (token.IsMissing)
                                {
                                        return;
                        ...
                        if (token.HasTrailingTrivia)
                        ...
                        else
                                {
                                        ...
                                        if (nextToken.IsKind (SyntaxKind.CommaToken)
                                            || nextToken.IsKind (SyntaxKind.GreaterThanToken)
                                            || nextToken.IsKind (SyntaxKind.CloseBracketToken))
                                                {
                                                        // make an exception for things like
                                                        // typeof(Func<,>), typeof(Func<,,>), and
                                                        // int[,]
                                                        missingFollowingSpace = false;
                                                }
                                }
                        ...
                        if (missingFollowingSpace && !shouldNotHaveFollowingSpace)
                                {
                                        ...
                                        context.ReportDiagnostic (Diagnostic.Create (
                                            Descriptor, token.GetLocation (),
                                            TokenSpacingProperties.InsertFollowing, string.Empty,
                                            "followed"));
                                }
