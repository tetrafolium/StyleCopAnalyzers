...
namespace StyleCop.Analyzers.SpacingRules
{
    ...
    internal class SA1000KeywordsMustBeSpacedCorrectly : DiagnosticAnalyzer {
        ...
        private static readonly ReportDiagnosticCallback<SyntaxTreeAnalysisContext> ReportSyntaxTreeDiagnostic = (ref SyntaxTreeAnalysisContext context, Diagnostic diagnostic) => context.ReportDiagnostic(diagnostic);

        private static readonly ReportDiagnosticCallback<SyntaxNodeAnalysisContext> ReportSyntaxNodeDiagnostic = (ref SyntaxNodeAnalysisContext context, Diagnostic diagnostic) => context.ReportDiagnostic(diagnostic);
        ...
        private static void HandleRequiredSpaceToken(ref SyntaxTreeAnalysisContext context, SyntaxToken token) => HandleRequiredSpaceToken(ReportSyntaxTreeDiagnostic, ref context, token);

        private static void HandleRequiredSpaceToken(ref SyntaxNodeAnalysisContext context, SyntaxToken token) => HandleRequiredSpaceToken(ReportSyntaxNodeDiagnostic, ref context, token);
        ...
        private static void HandleDisallowedSpaceToken(ref SyntaxNodeAnalysisContext context, SyntaxToken token) => HandleDisallowedSpaceToken(ReportSyntaxNodeDiagnostic, ref context, token);
