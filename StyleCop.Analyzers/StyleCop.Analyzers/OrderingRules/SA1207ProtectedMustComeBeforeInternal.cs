...
private static readonly LocalizableString Title = new LocalizableResourceString(
	nameof(OrderingResources.SA1207Title), OrderingResources.ResourceManager, typeof(OrderingResources));
...
private static readonly LocalizableString Description = new LocalizableResourceString(
	nameof(OrderingResources.SA1207Description), OrderingResources.ResourceManager, typeof(OrderingResources));
...
private static readonly ImmutableArray<SyntaxKind> HandledSyntaxKinds = ImmutableArray.Create(
	SyntaxKind.ClassDeclaration, SyntaxKind.DelegateDeclaration, SyntaxKind.EventDeclaration,
	SyntaxKind.EventFieldDeclaration, SyntaxKind.FieldDeclaration, SyntaxKind.IndexerDeclaration,
	SyntaxKind.InterfaceDeclaration, SyntaxKind.MethodDeclaration, SyntaxKind.PropertyDeclaration,
...
private static void HandleDeclaration(SyntaxNodeAnalysisContext context)
{
	...
	foreach (var childToken in childTokens)
	{
		...
		else if (childToken.IsKind(SyntaxKind.ProtectedKeyword))
		{
			if (internalKeywordFound)
			{
				context.ReportDiagnostic(
					Diagnostic.Create(Descriptor, childToken.GetLocation(), "protected", "internal"));
				break;
			}
			else
			{
		...
		{
			context.ReportDiagnostic(
				Diagnostic.Create(Descriptor, childToken.GetLocation(), "private", "protected"));
			break;
		}
	}
}
