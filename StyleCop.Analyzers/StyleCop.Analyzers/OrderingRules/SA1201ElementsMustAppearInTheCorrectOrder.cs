...
namespace StyleCop.Analyzers.OrderingRules
{
        ...
        /// <summary>
        /// An element within a C# code file is out of order in relation to the other elements in
        /// the code.
        /// </summary>
        /// <remarks>
        /// <para>A violation of this rule occurs when the code elements within a file do not follow
        /// a standard ordering scheme.</para>
        ///
        /// <para>To comply with this rule, elements at the file root level or within a namespace
        /// should be positioned in the following order:</para>
        ///
        /// <list type="bullet">
        ...
        /// </list>
        ///
        /// <para>Within a class, struct, or interface, elements should be positioned in the
        ...
        /// </list>
        ///
        /// <para>Complying with a standard ordering scheme based on element type can increase the
        /// readability and maintainability of the file and encourage code reuse.</para>
        ///
        /// <para>When implementing an interface, it is sometimes desirable to group all members of
        /// the interface next to one another. This will sometimes require violating this rule, if
        /// the interface contains elements of different types. This problem can be solved through
        /// the use of partial classes.</para>
        ///
        /// <list type="number">
        /// <item><description>Add the partial attribute to the class, if the class is not already
        /// partial.</description></item>
        /// <item><description>Add a second partial class with the same name. It is possible to
        /// place this in the same file, just below the original class, or within a second
        /// file.</description></item> <item><description>Move the interface inheritance and all
        /// members of the interface implementation to the second part of the
        ...
        internal class SA1201ElementsMustAppearInTheCorrectOrder : DiagnosticAnalyzer
        {
                ...
                HandleMemberList (SyntaxNodeAnalysisContext context,
                                  ImmutableArray<OrderingTrait> elementOrder, int kindIndex,
                        ...
                        for (int i = 0; i < members.Count - 1; i++)
                                {
                                        ...
                                        for (int j = 0; compareKind && j < kindIndex; j++)
                                                {
                                                        switch (elementOrder[j])
                                                                ...
                                                                case OrderingTrait.Readonly:
                                                                        ...
                                                                        // const or readonly, and
                                                                        // all fields have the same
                                                                        // kind.
                                                                        continue;
                                        ...
                                        int index = order.IndexOf (
                                            GetSyntaxKindForOrdering (elementSyntaxKind));

                                        var nextElementSyntaxKind = members [i + 1]
                                                                        .Kind ();
                                        int nextIndex = order.IndexOf (
                                            GetSyntaxKindForOrdering (nextElementSyntaxKind));

                                        if (index > nextIndex)
                                                {
                                                        // [Issue #3160] Added hardening here to
                                                        // make sure that this won't crash when
                                                        // working with invalid code.
                                                        var nextElementMemberName
                                                            = MemberNames.GetValueOrDefault (
                                                                nextElementSyntaxKind, "<unknown>");
                                                        var elementMemberName
                                                            = MemberNames.GetValueOrDefault (
                ...
                private static SyntaxKind
                GetSyntaxKindForOrdering (SyntaxKind syntaxKind)
                {
