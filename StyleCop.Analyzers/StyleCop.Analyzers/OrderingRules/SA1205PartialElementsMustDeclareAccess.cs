...
namespace StyleCop.Analyzers.OrderingRules
{
        ...
        /// </summary>
        /// <remarks>
        /// <para>A violation of this rule occurs when the partial elements does not have an access
        ...
        internal class SA1205PartialElementsMustDeclareAccess : DiagnosticAnalyzer
        {
                ...
                HandleTypeDeclaration (SyntaxNodeAnalysisContext context)
                {
                        ...
                        if (typeDeclarationNode.Modifiers.Any (SyntaxKind.PartialKeyword))
                                {
                                        if (!typeDeclarationNode.Modifiers.Any (
                                            ...
                                            && !typeDeclarationNode.Modifiers.Any (
                                                SyntaxKind.PrivateKeyword))
                                                {
                                                        context.ReportDiagnostic (
                                                            Diagnostic.Create (
                                                                Descriptor,
                                                                typeDeclarationNode.Identifier
                                                                    .GetLocation ()));
                                                }
                                }
                }
