...
namespace StyleCop.Analyzers.DocumentationRules
{
        ...
        /// </summary>
        /// <remarks>
        /// <para>To fix a violation of this rule, review the placeholder documentation for accuracy
        /// and remove the &lt;placeholder&gt; tags.</para>
        /// </remarks>
        ...
        internal class SA1651CodeFixProvider : CodeFixProvider {
                ...
                public override async Task RegisterCodeFixesAsync(CodeFixContext context)
                {
                        foreach (var diagnostic in context.Diagnostics) {
                                if (diagnostic.Properties.ContainsKey(
                                        SA1651DoNotUsePlaceholderElements.NoCodeFixKey)) {
                                ...
                                SyntaxNode syntax = documentRoot.FindNode(
                                ...
                                if (!(syntax is XmlElementSyntax xmlElementSyntax)) {
                                        // We continue even for placeholders if they are empty
                                        // elements (XmlEmptyElementSyntax)
                                        continue;
                                }

                                if (string.IsNullOrWhiteSpace(
                                        xmlElementSyntax.Content.ToString())) {
                                        // The placeholder hasn't been updated yet.
                                        continue;
                                }

                                context.RegisterCodeFix(
                                    CodeAction.Create(DocumentationResources.SA1651CodeFix,
                    ...
                    XmlElementSyntax elementSyntax)
                {
                        ...
                        leadingTrivia
                            = leadingTrivia.AddRange(elementSyntax.StartTag.GetTrailingTrivia());
                        ...
                        var trailingTrivia = content [content.Count - 1]
                                                 .GetTrailingTrivia();
                        trailingTrivia
                            = trailingTrivia.AddRange(elementSyntax.EndTag.GetLeadingTrivia());
                        trailingTrivia
                            = trailingTrivia.AddRange(elementSyntax.EndTag.GetTrailingTrivia());
                        content = content.Replace(content[content.Count - 1],
                ...
                private async Task<Document> GetTransformedDocumentAsync(Document document,
                    XmlElementSyntax elementSyntax, CancellationToken cancellationToken)
                {
                        ...
                        SyntaxNode root = await document.GetSyntaxRootAsync(cancellationToken)
                                              .ConfigureAwait(false);
                        SyntaxNode newRoot
                            = root.ReplaceNode(elementSyntax, RemovePlaceHolder(elementSyntax));
                ...
                private class FixAll : DocumentBasedFixAllProvider {
                            ...
                            ImmutableArray<Diagnostic> diagnostics)
                        {
                                ...
                                var newSyntaxRoot = syntaxRoot.ReplaceNodes(elements,
                                    (original, rewritten) => rewritten.WithAdditionalAnnotations(
                                        NodeToReplaceAnnotation));
                        ...
                        public override SyntaxList<TNode> VisitList<TNode>(SyntaxList<TNode> list)
                        {
                                ...
                                while (index < list.Count) {
                                        ...
                                        if (element.HasAnnotation(NodeToReplaceAnnotation)) {
                                                list = list.ReplaceRange(element,
                                                    RemovePlaceHolder(element as XmlElementSyntax)
                                                        .Cast<TNode>());
                                        } else {
                                                index++;
                                        }
