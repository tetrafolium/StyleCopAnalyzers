...
namespace StyleCop.Analyzers.DocumentationRules
{
        ...
        /// </summary>
        /// <remarks>
        /// <para>To fix a violation of this rule, add and fill-in documentation text within a
        /// &lt;returns&gt; tag describing the value returned from the element.</para>
        /// </remarks>
        ...
        internal class SA1615SA1616CodeFixProvider : CodeFixProvider
        {
            ...
            if (isTask)
                    {
                            ...
                            content = content.Add (
                                XmlSyntaxFactory
                                    .SeeElement (
                                        SyntaxFactory.TypeCref (SyntaxFactory.ParseTypeName (
                                            "global::System.Threading.Tasks.Task")))
                                    .WithAdditionalAnnotations (Simplifier.Annotation));
                            string operationKind
                                = isAsynchronousTestMethod ? "unit test" : "operation";
                            content = content.Add (XmlSyntaxFactory.Text (
                                $" representing the asynchronous {operationKind}."));
            ...
            if (returnsElement != null)
                            ...
                            if (returnsElement is XmlEmptyElementSyntax emptyElement)
                                    {
                                            XmlElementSyntax updatedReturns
                                                = XmlSyntaxFactory
                                                      .Element (XmlCommentHelper.ReturnsXmlTag,
                                                                content)
                                                      .WithLeadingTrivia (
                                                          returnsElement.GetLeadingTrivia ())
                                                      .WithTrailingTrivia (
                                                          returnsElement.GetTrailingTrivia ());
                                            newRoot
                                                = root.ReplaceNode (returnsElement, updatedReturns);
                                    }
                            else
                                    {
                                            XmlElementSyntax updatedReturns
                                                = ((XmlElementSyntax) returnsElement)
                                                      .WithContent (content);
                                            newRoot
                                                = root.ReplaceNode (returnsElement, updatedReturns);
                                    }
            ...
            else
                    {
                            ...
                            returnsElement = XmlSyntaxFactory.Element (
                                XmlCommentHelper.ReturnsXmlTag, content);

                            XmlNodeSyntax leadingNewLine = XmlSyntaxFactory.NewLine (newLineText);

                            // HACK: The formatter isn't working when contents are added to an
                            // existing documentation comment, so we manually apply the indentation
                            // from the last line of the existing comment to each new line of the
                            // generated content.
                            ...
                            if (!exteriorTrivia.Token.IsMissing)
                                    {
                                            leadingNewLine = leadingNewLine.ReplaceExteriorTrivia (
                                                exteriorTrivia);
                                            returnsElement = returnsElement.ReplaceExteriorTrivia (
                                                exteriorTrivia);
                                    }
                                          ...
                                          CancellationToken cancellationToken)
                {
                        foreach (AttributeListSyntax attributeList in methodDeclarationSyntax
                                     .AttributeLists)
                                {
                                        ...
                                        foreach (AttributeSyntax attribute in attributeList
                                                     .Attributes)
                                                {
                                                        if (!(semanticModel
                                                                  .GetSymbolInfo (attribute.Name,
                                                                                  cancellationToken)
