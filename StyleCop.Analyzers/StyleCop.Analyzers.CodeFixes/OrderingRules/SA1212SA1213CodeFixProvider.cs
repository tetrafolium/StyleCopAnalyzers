...
namespace StyleCop.Analyzers.OrderingRules
{
        ...
        internal class SA1212SA1213CodeFixProvider : CodeFixProvider
        {
                                             ...
                                             CancellationToken cancellationToken)
                {
                        ...
                        syntaxRoot = trackedRoot.InsertNodesBefore (trackedFirstAccessor,
                                                                    new[]{ newAccessor });

                        if (HasLeadingBlankLines (secondAccessor))
                                {
                                        trackedFirstAccessor
                                            = syntaxRoot.GetCurrentNode (firstAccesor);
                                        var newFirstAccessor
                                            = trackedFirstAccessor.WithLeadingTrivia (
                                                new[]{ SyntaxFactory.CarriageReturnLineFeed }
                                                    .Concat (firstAccesor.GetFirstToken ()
                                                                 .WithoutLeadingBlankLines ()
                                                                 .LeadingTrivia));
                                        syntaxRoot = syntaxRoot.ReplaceNode (trackedFirstAccessor,
                                                                             newFirstAccessor);
                                }
                        ...
                        if (AccessorsAreOnTheSameLine (firstAccessor, secondAccessor))
                                {
                                        var leadingWhitespace
                                            = firstAccessor.GetLeadingTrivia ().Where (
                                                x => x.IsKind (SyntaxKind.WhitespaceTrivia));
                                        newLeadingTrivia = SyntaxFactory.TriviaList (
                                            TriviaHelper.MergeTriviaLists (
                                                newLeadingTrivia, SyntaxTriviaList.Empty.AddRange (
                                                                      leadingWhitespace)));
                                }
