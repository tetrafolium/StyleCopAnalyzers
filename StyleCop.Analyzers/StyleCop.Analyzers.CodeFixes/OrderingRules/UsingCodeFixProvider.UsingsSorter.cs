...
namespace StyleCop.Analyzers.OrderingRules
{
        ...
        internal sealed partial class UsingCodeFixProvider
        {
                ...
                private class UsingsSorter
                {
                        ...
                        public List<UsingDirectiveSyntax>
                        GetContainedUsings (TreeTextSpan directiveSpan)
                        {
                                ...
                                if (this.namespaceUsings.TryGetValue (directiveSpan,
                                                                      out usingsList))
                                        {
                                                result.AddRange (usingsList);
                                        }
                                ...
                                if (this.systemStaticImports.TryGetValue (directiveSpan,
                                                                          out usingsList))
                                        {
                                                result.AddRange (usingsList);
                                        }
                        ...
                        public SyntaxList<UsingDirectiveSyntax>
                        GenerateGroupedUsings (TreeTextSpan directiveSpan, string indentation,
                                               bool withTrailingBlankLine, bool qualifyNames)
                        {
                                ...
                                usingList.AddRange (
                                    this.GenerateUsings (this.systemUsings, directiveSpan,
                                                         indentation, triviaToMove, qualifyNames));
                                usingList.AddRange (
                                    this.GenerateUsings (this.namespaceUsings, directiveSpan,
                                                         indentation, triviaToMove, qualifyNames));
                                usingList.AddRange (
                                    this.GenerateUsings (this.systemStaticImports, directiveSpan,
                                                         indentation, triviaToMove, qualifyNames));
                                usingList.AddRange (
                                    this.GenerateUsings (this.staticImports, directiveSpan,
                                                         indentation, triviaToMove, qualifyNames));
                                usingList.AddRange (
                                    this.GenerateUsings (this.aliases, directiveSpan, indentation,
                        ...
                        public SyntaxList<UsingDirectiveSyntax>
                        GenerateGroupedUsings (List<UsingDirectiveSyntax> usingsList,
                                               string indentation, bool withTrailingBlankLine,
                                               bool qualifyNames)
                        {
                                var usingList = new List<UsingDirectiveSyntax> ();
                                List<SyntaxTrivia> triviaToMove = new List<SyntaxTrivia> ();
                                ...
                                usingList.AddRange (
                                    this.GenerateUsings (this.systemUsings, usingsList, indentation,
                                                         triviaToMove, qualifyNames));
                                usingList.AddRange (
                                    this.GenerateUsings (this.namespaceUsings, usingsList,
                                                         indentation, triviaToMove, qualifyNames));
                                usingList.AddRange (
                                    this.GenerateUsings (this.systemStaticImports, usingsList,
                                                         indentation, triviaToMove, qualifyNames));
                                usingList.AddRange (
                                    this.GenerateUsings (this.staticImports, usingsList,
                                                         indentation, triviaToMove, qualifyNames));
                        ...
                        private List<UsingDirectiveSyntax>
                        GenerateUsings (
                        ...
                        private List<UsingDirectiveSyntax>
                        GenerateUsings (List<UsingDirectiveSyntax> usingsList, string indentation,
                                        List<SyntaxTrivia> triviaToMove, bool qualifyNames)
                        {
                                List<UsingDirectiveSyntax> result
                                    = new List<UsingDirectiveSyntax> ();
                                ...
                                for (var i = 0; i < usingsList.Count; i++)
                                        {
                                                ...
                                                // strip the file header, if the using is the first
                                                // node in the source file.
                                                List<SyntaxTrivia> leadingTrivia;
                                                if ((i == 0)
                                                    && currentUsing.GetFirstToken ()
                                                           .GetPreviousToken ()
                                                           .IsMissingOrDefault ())
                                                        {
                                                                leadingTrivia
                                                ...
                                                // when there is a directive trivia, add it (and any
                                                // trivia before it) to the triviaToMove collection.
                                                // when there are leading blank lines for the first
                                                // entry, add them to the triviaToMove collection.
                                                ...
                                                // preserve leading trivia (excluding directive
                                                // trivia), indenting each line as appropriate
                                                var newLeadingTrivia
                                                    = leadingTrivia.Except (triviaToMove).ToList ();

                                                // indent the triviaToMove if necessary so it
                                                // behaves correctly later
                                                bool atStartOfLine
                                                    = triviaToMoveCount == 0
                                                      || triviaToMove.Last ().HasBuiltinEndLine ();
                                                for (int m = triviaToMoveCount;
                                                     m < triviaToMove.Count; m++)
                                                        {
                                                                bool currentAtStartOfLine
                                                                    = atStartOfLine;
                                                                atStartOfLine
                                                                    = triviaToMove [m]
                                                                          .HasBuiltinEndLine ();
                                                ...
                                                // strip any leading whitespace on each line (and
                                                // also blank lines)
                                                var k = 0;
                                                ...
                                                for (var j = newLeadingTrivia.Count - 1; j >= 0;
                                                     j--)
                                                        {
                                                ...
                                                // preserve trailing trivia, adding an end of line
                                                // if necessary.
                                                ...
                                                if (!currentTrailingTrivia.Any ()
                                                    || !currentTrailingTrivia.Last ().IsKind (
                                                        ...
                                                        {
                                                                newTrailingTrivia
                                                                    = newTrailingTrivia.Add (
                                                                        SyntaxFactory
                                                                            .CarriageReturnLineFeed);
                                                        }

                                                var processedUsing
                                                    = (qualifyNames ? this.QualifyUsingDirective (
                                                           currentUsing)
                                                       : currentUsing)
                                                          .WithLeadingTrivia (newLeadingTrivia)
                                                          .WithTrailingTrivia (newTrailingTrivia)
                                                          .WithAdditionalAnnotations (
                                                              UsingCodeFixAnnotation);
                                ...
                                if (this.insertBlankLinesBetweenGroups)
                                        {
                                                ...
                                                result[result.Count - 1] = last.WithTrailingTrivia (
                                                    last.GetTrailingTrivia ().Add (
                                                        SyntaxFactory.CarriageReturnLineFeed));
                                        }
                        ...
                        private UsingDirectiveSyntax
                        QualifyUsingDirective (UsingDirectiveSyntax usingDirective)
                        {
                        ...
                        private int
                        CompareUsings (UsingDirectiveSyntax left, UsingDirectiveSyntax right)
                        {
                        ...
                        private bool
                        IsSeparatedStaticSystemUsing (UsingDirectiveSyntax syntax)
                        {
                        ...
                        private bool
                        IsSeparatedSystemUsing (UsingDirectiveSyntax syntax)
                        {
                                if (!this.separateSystemDirectives
                        ...
                        private bool
                        StartsWithSystemUsingDirectiveIdentifier (NameSyntax name)
                        {
                        ...
                        private void
                        ProcessMembers (SyntaxList<MemberDeclarationSyntax> members)
                        {
                                foreach (var namespaceDeclaration in members
                                             .OfType<NamespaceDeclarationSyntax> ())
                                        {
                                                this.ProcessUsingDirectives (
                        ...
                        private void
                        ProcessUsingDirectives (SyntaxList<UsingDirectiveSyntax> usingDirectives)
                        {
                                foreach (var usingDirective in usingDirectives)
                                        {
                                                TreeTextSpan containingSpan
                                                    = this.sourceMap.GetContainingSpan (
                                                        usingDirective);

                                                if (usingDirective.Alias != null)
                                                        {
                                                                this.AddUsingDirective (
                                                                    this.aliases, usingDirective,
                                                                    containingSpan);
                                                        }
                                                ...
                                                else if (this.IsSeparatedSystemUsing (
                                                             usingDirective))
                                                        {
                                                                this.AddUsingDirective (
                                                                    this.systemUsings,
