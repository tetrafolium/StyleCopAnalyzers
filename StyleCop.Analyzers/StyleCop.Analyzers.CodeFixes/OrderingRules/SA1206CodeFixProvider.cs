...
namespace StyleCop.Analyzers.OrderingRules
{
        ...
        internal sealed class SA1206CodeFixProvider : CodeFixProvider
        {
                ...
                /// The relative order within the different kinds <seealso cref="ModifierType"/>
                /// will not be changed.
                /// </summary>
                ...
                FullySortModifiers (SyntaxTokenList modifiers)
                {
                        ...
                        var otherModifiers = modifiers.Where (modifier => GetModifierType (modifier)
                                                                          == ModifierType.Other);
                ...
                /// <summary>
                /// Sorts the modifier list to fix all issues before <paramref
                /// name="modifierToFix"/> and keep the remaining modifiers untouched. The trivia
                /// will be maintained positionally. The relative order within the different kinds
                /// <seealso cref="ModifierType"/> will not be changed.
                /// </summary>
                /// <param name="modifiers">All modifiers from the declaration.</param>
                ...
                private static SyntaxTokenList
                PartiallySortModifiers (SyntaxTokenList modifiers, SyntaxToken modifierToFix)
                {
                        var accessModifiers = modifiers.Where (
                            modifier => GetModifierType (modifier) == ModifierType.Access);
                        var staticModifiers = modifiers.Where (
                        ...
                        // the modifier to fix is of type other, so we need to sort the whole list
                        // of modifier list
                        if (GetModifierType (modifierToFix) == ModifierType.Other)
                                {
                                        beforeIncluding = accessModifiers.Concat (staticModifiers)
                ...
                private static SyntaxTokenList
                AdjustTrivia (IEnumerable<SyntaxToken> newModifiers, SyntaxTokenList oldModifiers)
                {
                ...
                private class FixAll : DocumentBasedFixAllProvider
                {
                        ...
                        protected override string
                            CodeActionTitle => OrderingResources.ModifierOrderCodeFix;
                                ...
                                foreach (var diagnostic in diagnostics)
                                        {
                                                ...
                                                if (GetModifierType (modifierToken)
                                                    == ModifierType.None)
                                                        {
                                                                continue;
                                                        }
                                ...
                                foreach (var member in trackedDiagnosticMembers)
                                        {
                                                ...
                                                var newModifierList = FullySortModifiers (
                                                    memberDeclaration.GetModifiers ());
