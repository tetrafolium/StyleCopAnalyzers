...
namespace StyleCop.Analyzers.OrderingRules
{
        ...
        internal sealed partial class UsingCodeFixProvider : CodeFixProvider
        {
                        ...
                        foreach (Diagnostic diagnostic in context.Diagnostics)
                                {
                                        ...
                                        // do not offer a code fix for SA1200 when there are
                                        // multiple namespaces in the source file
                                        if (isSA1200
                                             ...
                                             CancellationToken cancellationToken)
                {
                        ...
                        var usingsHelper = new UsingsSorter (settings, semanticModel,
                                                             compilationUnit, fileHeader);
                        ...
                        if (usingDirectivesPlacement == UsingDirectivesPlacement.Preserve)
                                {
                                        BuildReplaceMapForNamespaces (usingsHelper, replaceMap,
                                                                      settings.Indentation, false);
                        ...
                        if (usingDirectivesPlacement == UsingDirectivesPlacement.InsideNamespace)
                                {
                                        newSyntaxRoot = AddUsingsToNamespace (
                                ...
                                {
                                        newSyntaxRoot = AddUsingsToCompilationRoot (
                                            newSyntaxRoot, usingsHelper, usingsIndentation,
                                            replaceMap.Any ());
                                }
                                    ...
                                    StyleCopSettings settings)
                {
                        switch (settings.OrderingRules.UsingDirectivesPlacement)
                                ...
                                case UsingDirectivesPlacement.InsideNamespace:
                                        ...
                                        if (namespaceCount == 0)
                                                {
                                                        return UsingDirectivesPlacement
                                                            .OutsideNamespace;
                                                }
                ...
                private static int
                CountNamespaces (SyntaxList<MemberDeclarationSyntax> members)
                {
                        var result = 0;
                        ...
                        foreach (var namespaceDeclaration in members
                                ...
                                {
                                        result
                                            += 1 + CountNamespaces (namespaceDeclaration.Members);
                                }
                ...
                {
                        var usingsPerNamespace
                            = usingsHelper.GetContainedUsings (TreeTextSpan.Empty)
                        ...
                        foreach (var usingList in usingsPerNamespace)
                                {
                                        if (usingList.Count > 0)
                                                {
                                                        // sort the original using declarations on
                                                        // Span.Start, in order to have the correct
                                                        // replace mapping.
                                                        ...
                                                        var modifiedUsings
                                                            = usingsHelper.GenerateGroupedUsings (
                                                                usingList, indentation, false,
                                                                qualifyNames);
                    ...
                    IndentationSettings indentationSettings, TreeTextSpan rootSpan)
                {
                        foreach (var childSpan in rootSpan.Children)
                                {
                                        var originalUsings
                                            = usingsHelper.GetContainedUsings (childSpan);
                                        if (originalUsings.Count > 0)
                                                {
                                                        ...
                                                        var modifiedUsings
                                                            = usingsHelper.GenerateGroupedUsings (
                                                                childSpan, indentation, false,
                                                                qualifyNames
                                                                : false);

                                                        for (var i = 0; i < originalUsings.Count;
                                                             i++)
                                                                {
                                                                        replaceMap.Add (
                                                                            originalUsings[i],
                                        ...
                                        BuildReplaceMapForConditionalDirectives (
                                            usingsHelper, replaceMap, indentationSettings,
                                            childSpan);
                                }
                }

                private static int
                CompareSpanStart (UsingDirectiveSyntax left, UsingDirectiveSyntax right)
                {
                        return left.SpanStart - right.SpanStart;
                }

                private static SyntaxNode
                AddUsingsToNamespace (SyntaxNode newSyntaxRoot, UsingsSorter usingsHelper,
                                      string usingsIndentation, bool hasConditionalDirectives)
                {
                        ...
                        var withTrailingBlankLine = hasConditionalDirectives
                                                    || rootNamespace.Members.Any ()
                                                    || rootNamespace.Externs.Any ();
                                            ...
                                            string usingsIndentation, bool hasConditionalDirectives)
                {
                        ...
                        var withTrailingBlankLine = hasConditionalDirectives
                                                    ...
                                                    || newCompilationUnit.Members.Any ()
                                                    || newCompilationUnit.Externs.Any ();
                ...
                private static SyntaxNode
                StripMultipleBlankLines (SyntaxNode syntaxRoot)
                {
                        ...
                        foreach (var usingDirective in usingDirectives)
                                {
                                        var nextToken
                                            = usingDirective.SemicolonToken.GetNextToken (true);

                                        // start at -1 to compensate for the always present
                                        // end-of-line.
                                        var trailingCount = -1;

                                        // count the blanks lines at the end of the using statement.
                                        foreach (var trivia in usingDirective.SemicolonToken
                ...
                private static ImmutableArray<SyntaxTrivia>
                GetFileHeader (SyntaxNode syntaxRoot)
                {
                        ...
                        for (i = 0; i < firstTokenLeadingTrivia.Count; i++)
                                {
                                        ...
                                        switch (firstTokenLeadingTrivia [i]
                                                ...
                                                case SyntaxKind.MultiLineCommentTrivia:
                                                        fileHeaderBuilder.Add (
                                                            firstTokenLeadingTrivia[i]);
                                                        onBlankLine = false;
                                                        break;
                        ...
                        return hasHeader ? fileHeaderBuilder.ToImmutableArray ()
                            : ImmutableArray.Create<SyntaxTrivia> ();
                }

                private static SyntaxNode
                ReAddFileHeader (SyntaxNode syntaxRoot, ImmutableArray<SyntaxTrivia> fileHeader)
                {
                ...
                private class UsingSyntaxRewriter : CSharpSyntaxRewriter
                {
                                                ...
                                                if (!nextToken.IsKind (SyntaxKind.None))
                                                        {
                                                                var index
                                                                    = TriviaHelper
                                                                          .IndexOfFirstNonBlankLineTrivia (
                                                                              nextToken
                                                                                  .LeadingTrivia);
                                                                if (index != 0)
                                                                        {
                        ...
                        public override SyntaxTrivia
                        VisitTrivia (SyntaxTrivia trivia)
                        {
                ...
                private class FixAll : DocumentBasedFixAllProvider
                {
                                               ...
                                               ImmutableArray<Diagnostic> diagnostics)
                        {
                                ...
                                var forcePreserve = diagnostics.All (
                                    d => d.Id
                                         != SA1200UsingDirectivesMustBePlacedCorrectly
                                                .DiagnosticId);
                                ...
                                var syntaxRoot
                                    = await document
                                          ...
                                          .ConfigureAwait (false);
                                Document newDocument = await GetTransformedDocumentAsync (
                                                           document, syntaxRoot, forcePreserve,
