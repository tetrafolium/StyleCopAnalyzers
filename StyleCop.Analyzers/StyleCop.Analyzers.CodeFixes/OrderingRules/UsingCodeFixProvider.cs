...
namespace StyleCop.Analyzers.OrderingRules
{
        ...
        internal sealed partial class UsingCodeFixProvider : CodeFixProvider {
                ...
                private static readonly List<UsingDirectiveSyntax> EmptyUsingsList
                    = new List<UsingDirectiveSyntax>();
                private static readonly SyntaxAnnotation UsingCodeFixAnnotation
                    = new SyntaxAnnotation(nameof(UsingCodeFixAnnotation));
                private static readonly SymbolDisplayFormat FullNamespaceDisplayFormat
                ...
                = ImmutableArray.Create(SA1200UsingDirectivesMustBePlacedCorrectly.DiagnosticId,
                ...
                public override async Task RegisterCodeFixesAsync(CodeFixContext context)
                {
                        ...
                        foreach (Diagnostic diagnostic in context.Diagnostics) {
                                var isSA1200 = diagnostic.Id
                                    == SA1200UsingDirectivesMustBePlacedCorrectly.DiagnosticId;

                                // do not offer a code fix for SA1200 when there are multiple
                                // namespaces in the source file
                ...
                private static async Task<Document> GetTransformedDocumentAsync(Document document,
                    SyntaxNode syntaxRoot, bool forcePreservePlacement,
                    CancellationToken cancellationToken)
                {
                        ...
                        var semanticModel = await document.GetSemanticModelAsync(cancellationToken)
                                                .ConfigureAwait(false);
                        var usingDirectivesPlacement
                            = forcePreservePlacement ? UsingDirectivesPlacement.Preserve
                            : DeterminePlacement(compilationUnit, settings);

                        var usingsHelper = new UsingsSorter(
                            settings, semanticModel, compilationUnit, fileHeader);
                        var usingsIndentation = DetermineIndentation(
                            compilationUnit, settings.Indentation, usingDirectivesPlacement);

                        // - The strategy is to strip all using directive that are not inside a
                        // conditional directive and replace them later with a sorted list at the
                        // correct spot
                        // - The using directives that are inside a conditional directive are
                        // replaced (in sorted order) on the spot.
                        // - Conditional directives are not moved, as correctly parsing them is too
                        // tricky
                        // - No using directives will be stripped when there are multiple
                        // namespaces. In that case everything is replaced on the spot.
                        List<UsingDirectiveSyntax> stripList;
                        var replaceMap
                            = new Dictionary<UsingDirectiveSyntax, UsingDirectiveSyntax>();

                        // When there are multiple namespaces, do not move using statements outside
                        // of them, only sort.
                        if (usingDirectivesPlacement == UsingDirectivesPlacement.Preserve) {
                                BuildReplaceMapForNamespaces(
                                    usingsHelper, replaceMap, settings.Indentation, false);
                                stripList = new List<UsingDirectiveSyntax>();
                        } else {
                                stripList = BuildStripList(usingsHelper);
                        }

                        BuildReplaceMapForConditionalDirectives(usingsHelper, replaceMap,
                            settings.Indentation, usingsHelper.ConditionalRoot);

                        var usingSyntaxRewriter
                            = new UsingSyntaxRewriter(stripList, replaceMap, fileHeader);
                        ...
                        if (usingDirectivesPlacement == UsingDirectivesPlacement.InsideNamespace) {
                                newSyntaxRoot = AddUsingsToNamespace(newSyntaxRoot, usingsHelper,
                                    usingsIndentation, replaceMap.Any());
                        } else if (usingDirectivesPlacement
                            == UsingDirectivesPlacement.OutsideNamespace) {
                                newSyntaxRoot = AddUsingsToCompilationRoot(newSyntaxRoot,
                                    usingsHelper, usingsIndentation, replaceMap.Any());
                        }
                ...
                private static string DetermineIndentation(CompilationUnitSyntax compilationUnit,
                    ...
                    UsingDirectivesPlacement usingDirectivesPlacement)
                {
                        ...
                        if (usingDirectivesPlacement == UsingDirectivesPlacement.InsideNamespace) {
                                var rootNamespace
                                    = compilationUnit.Members.OfType<NamespaceDeclarationSyntax>()
                                          .First();
                                var indentationLevel = IndentationHelper.GetIndentationSteps(
                                    indentationSettings, rootNamespace);
                                usingsIndentation = IndentationHelper.GenerateIndentationString(
                                    indentationSettings, indentationLevel + 1);
                        } else {
                ...
                private static void BuildReplaceMapForNamespaces(UsingsSorter usingsHelper,
                    ...
                    IndentationSettings indentationSettings, bool qualifyNames)
                {
                        ...
                        foreach (var usingList in usingsPerNamespace) {
                                if (usingList.Count > 0) {
                                        // sort the original using declarations on Span.Start, in
                                        // order to have the correct replace mapping.
                                        usingList.Sort(CompareSpanStart);

                                        var indentationSteps
                                            = IndentationHelper.GetIndentationSteps(
                                                indentationSettings, usingList[0].Parent);
                                        ...
                                        var modifiedUsings = usingsHelper.GenerateGroupedUsings(
                                            usingList, indentation, false, qualifyNames);
                    ...
                    IndentationSettings indentationSettings, TreeTextSpan rootSpan)
                {
                        foreach (var childSpan in rootSpan.Children) {
                                ...
                                if (originalUsings.Count > 0) {
                                        ...
                                        var indentationSteps
                                            = IndentationHelper.GetIndentationSteps(
                                                indentationSettings, originalUsings[0].Parent);
                                        if (originalUsings[0].Parent is
                                        ...
                                        var modifiedUsings = usingsHelper.GenerateGroupedUsings(
                                            childSpan, indentation, false, qualifyNames
                                            : false);

                                        for (var i = 0; i < originalUsings.Count; i++) {
                                                replaceMap.Add(
                                                    originalUsings[i], modifiedUsings[i]);
                                        }
                                }

                                BuildReplaceMapForConditionalDirectives(
                                    usingsHelper, replaceMap, indentationSettings, childSpan);
                        }
                }

                private static int CompareSpanStart(
                    UsingDirectiveSyntax left, UsingDirectiveSyntax right)
                {
                        return left.SpanStart - right.SpanStart;
                }

                private static SyntaxNode AddUsingsToNamespace(SyntaxNode newSyntaxRoot,
                    UsingsSorter usingsHelper, string usingsIndentation,
                    bool hasConditionalDirectives)
                {
                        var rootNamespace = ((CompilationUnitSyntax) newSyntaxRoot)
                                                .Members.OfType<NamespaceDeclarationSyntax>()
                                                .First();
                        var withTrailingBlankLine = hasConditionalDirectives
                            || rootNamespace.Members.Any() || rootNamespace.Externs.Any();

                        var groupedUsings = usingsHelper.GenerateGroupedUsings(
                            TreeTextSpan.Empty, usingsIndentation, withTrailingBlankLine,
                            qualifyNames
                            : false);
                ...
                private static SyntaxNode AddUsingsToCompilationRoot(SyntaxNode newSyntaxRoot,
                    UsingsSorter usingsHelper, string usingsIndentation,
                    bool hasConditionalDirectives)
                {
                        var newCompilationUnit = (CompilationUnitSyntax) newSyntaxRoot;
                        var withTrailingBlankLine = hasConditionalDirectives
                            || newCompilationUnit.AttributeLists.Any()
                            || newCompilationUnit.Members.Any() || newCompilationUnit.Externs.Any();
                ...
                private static SyntaxNode StripMultipleBlankLines(SyntaxNode syntaxRoot)
                {
                        ...
                        var usingDirectives = syntaxRoot.GetAnnotatedNodes(UsingCodeFixAnnotation)
                        ...
                        foreach (var usingDirective in usingDirectives) {
                                ...
                                foreach (var trivia in usingDirective.SemicolonToken.TrailingTrivia
                                             .Reverse()) {
                                ...
                                if ((trailingCount + leadingCount) > 1) {
                                        ...
                                        if (trailingCount > 0) {
                                                var trailingStripCount
                                                    = Math.Min(totalStripCount, trailingCount);

                                                var trailingTrivia
                                                    = usingDirective.SemicolonToken.TrailingTrivia;
                                                replaceMap[usingDirective.SemicolonToken]
                                                    = usingDirective.SemicolonToken
                                                          .WithTrailingTrivia(trailingTrivia.Take(
                                        ...
                                        if (totalStripCount > 0) {
                                                replaceMap[nextToken] = nextToken.WithLeadingTrivia(
                                                    nextToken.LeadingTrivia.Skip(totalStripCount));
                                        }
                                }
                        }
                ...
                private static ImmutableArray<SyntaxTrivia> GetFileHeader(SyntaxNode syntaxRoot)
                {
                        ...
                        return hasHeader ? fileHeaderBuilder
                            .ToImmutableArray()
                            : ImmutableArray.Create<SyntaxTrivia>();
                }

                private static SyntaxNode ReAddFileHeader(
                    SyntaxNode syntaxRoot, ImmutableArray<SyntaxTrivia> fileHeader)
                {
                        ...
                        return syntaxRoot.ReplaceToken(
                            firstToken, firstToken.WithLeadingTrivia(newLeadingTrivia));
                }

                private class UsingSyntaxRewriter : CSharpSyntaxRewriter {
                        private readonly List<UsingDirectiveSyntax> stripList;
                        private readonly Dictionary<UsingDirectiveSyntax, UsingDirectiveSyntax>
                            replaceMap;
                        private readonly ImmutableArray<SyntaxTrivia> fileHeader;
                        private readonly LinkedList<SyntaxToken> tokensToStrip
                            = new LinkedList<SyntaxToken>();

                        public UsingSyntaxRewriter(List<UsingDirectiveSyntax> stripList,
                            Dictionary<UsingDirectiveSyntax, UsingDirectiveSyntax> replaceMap,
                        ...
                        {
                                // The strip list is used to remove using directives that will be
                                // moved.
                                ...
                                // The replacement map is used to replace using declarations in
                                // place in sorted order (inside directive trivia)
                        ...
                        public override SyntaxToken VisitToken(SyntaxToken token)
                        {
                                if (this.tokensToStrip.Contains(token)) {
                                        ...
                                        var index = TriviaHelper.IndexOfFirstNonBlankLineTrivia(
                                            token.LeadingTrivia);
                                        var newLeadingTrivia = (index == -1)
                                            ? SyntaxFactory.TriviaList()
                                            : SyntaxFactory.TriviaList(
                                                token.LeadingTrivia.Skip(index));
                ...
                private class FixAll : DocumentBasedFixAllProvider {
                            ...
                            ImmutableArray<Diagnostic> diagnostics)
                        {
                                ...
                                var forcePreserve = diagnostics.All(d => d.Id
                                        != SA1200UsingDirectivesMustBePlacedCorrectly.DiagnosticId);
                                ...
                                var syntaxRoot
                                    = await document
                                          ...
                                          .ConfigureAwait(false);
                                Document newDocument = await GetTransformedDocumentAsync(document,
                                    syntaxRoot, forcePreserve, fixAllContext.CancellationToken)
                                                           .ConfigureAwait(false);
