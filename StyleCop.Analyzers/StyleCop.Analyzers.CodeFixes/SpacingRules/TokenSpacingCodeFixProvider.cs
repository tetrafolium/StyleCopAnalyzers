        ...
        private static void UpdateReplaceMap(Dictionary<SyntaxToken, SyntaxToken> replaceMap, SyntaxToken token, Diagnostic diagnostic)
        {
            ...
            case TokenSpacingProperties.LocationPreceding:
                ...
                case TokenSpacingProperties.ActionRemove:
                    ...
                    if ((!preserveLayout || !tokenIsFirstInLine)
                        ...
                        for (int i = lastNewLineLeading + 1; i < token.LeadingTrivia.Count; i++) {
                            onlyWhitespace &= token.LeadingTrivia [i]
                        ...
                        for (int i = 0; i < firstNewLineFollowing; i++) {
                            onlyWhitespace &= token.TrailingTrivia [i]
                                                  .IsKind(SyntaxKind.WhitespaceTrivia);
                        }
                        ...
                        SyntaxTriviaList trailingTrivia = triviaList.AddRange(token.TrailingTrivia.WithoutLeadingWhitespace(endOfLineIsWhitespace
                                                                                                                            : false));
                ...
                case TokenSpacingProperties.ActionRemoveImmediate:
                    UpdateReplaceMap(replaceMap, token, t => t.WithLeadingTrivia(t.LeadingTrivia.WithoutTrailingWhitespace(endOfLineIsWhitespace
                                                                                                                           : false)));

                    if (!replaceMap[token].LeadingTrivia.Any()) {
                        UpdateReplaceMap(replaceMap, prevToken, t => t.WithTrailingTrivia(t.TrailingTrivia.WithoutTrailingWhitespace(endOfLineIsWhitespace
                                                                                                                                     : false)));
                    }
            ...
            case TokenSpacingProperties.LocationFollowing:
                ...
                case TokenSpacingProperties.ActionRemove:
                    ...
                    UpdateReplaceMap(replaceMap, nextToken, t => t.WithLeadingTrivia(triviaList.WithoutLeadingWhitespace(endOfLineIsWhitespace
                                                                                                                         : !preserveLayout)));
                    break;
                }
