...
namespace StyleCop.Analyzers.SpacingRules
{
        ...
        internal class TokenSpacingCodeFixProvider : CodeFixProvider
        {
                ...
                private static void
                UpdateReplaceMap (Dictionary<SyntaxToken, SyntaxToken> replaceMap,
                                  SyntaxToken token, Diagnostic diagnostic)
                {
                        string location;
                                                        ...
                                                        if ((!preserveLayout || !tokenIsFirstInLine)
                                                            && triviaList.All (
                                                                             ...
                                                                             .EndOfLineTrivia)))
                                                                {
                                                                 ...
                                                                 && token.IsLastInLine ())
                                                                {
                                                                        /* This block covers the
                                                                         * case where `token` is the
                                                                         * only non-trivia token on
                                                                         * its line. However, the
                                                                         * line may still contain
                                                                         * non-whitespace trivia
                                                                         * which we want the removal
                                                                         * process to preserve. This
                                                                         * code fix only removes the
                                                                         * whitespace surrounding
                                                                         * `token` if it is the only
                                                                         * non-whitespace token on
                                                                         * the line.
                                                                         */
                                        ...
                                        switch (action)
                                                ...
                                                case TokenSpacingProperties.ActionInsert:
                                                        if (!replaceMap.ContainsKey (nextToken))
                                                                {
                                                                        // If the token is already
                                                                        // present in the map and it
                                                                        // has trailing trivia, then
                                                                        // it has already been
                                                                        // processed during an
                                                                        // earlier step in the fix
                                                                        // all process, so no
                ...
                private static void
                UpdateReplaceMap (Dictionary<SyntaxToken, SyntaxToken> replaceMap,
                                  SyntaxToken token, Func<SyntaxToken, SyntaxToken> action)
                {
