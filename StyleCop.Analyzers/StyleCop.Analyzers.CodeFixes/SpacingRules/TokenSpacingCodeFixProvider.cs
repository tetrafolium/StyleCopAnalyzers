...
public override ImmutableArray<string> FixableDiagnosticIds
{
	get;
        ...
        = ImmutableArray.Create(
		  SA1000KeywordsMustBeSpacedCorrectly.DiagnosticId, SA1001CommasMustBeSpacedCorrectly.DiagnosticId,
...
public override Task RegisterCodeFixesAsync(CodeFixContext context)
{
	...
	{
		context.RegisterCodeFix(CodeAction.Create(SpacingResources.TokenSpacingCodeFix,
...
private static void UpdateReplaceMap(Dictionary<SyntaxToken, SyntaxToken> replaceMap, SyntaxToken token,
                                     Diagnostic diagnostic)
{
	...
	case TokenSpacingProperties.LocationPreceding:
		...
		case TokenSpacingProperties.ActionInsert:
			if (!replaceMap.ContainsKey(prevToken))
			{
				UpdateReplaceMap(replaceMap, token,
				                 t => t.WithLeadingTrivia(t.LeadingTrivia.Add(SyntaxFactory.Space)));
			}
		...
		case TokenSpacingProperties.ActionRemove:
			...
			if ((!preserveLayout || !tokenIsFirstInLine) &&
			    triviaList.All(i => i.IsKind(SyntaxKind.WhitespaceTrivia) ||
			...
			else if (tokenIsFirstInLine && token.IsLastInLine())
			{
				...
				{
					onlyWhitespace &= token
					                  .LeadingTrivia [i]
					                  .IsKind(SyntaxKind.WhitespaceTrivia);
				}

				firstNewLineFollowing =
					firstNewLineFollowing == -1 ? token.TrailingTrivia.Count : firstNewLineFollowing;
				...
				if (onlyWhitespace)
				{
					...
					UpdateReplaceMap(replaceMap, token,
					                 t => t.WithLeadingTrivia().WithTrailingTrivia(trailingTrivia));
				}
				else
				{
			...
			else
			{
				SyntaxTriviaList trailingTrivia =
					triviaList.AddRange(token.TrailingTrivia.WithoutLeadingWhitespace(endOfLineIsWhitespace
					                                                                  : false));
		...
		case TokenSpacingProperties.ActionRemoveImmediate:
			UpdateReplaceMap(
				replaceMap, token,
				t => t.WithLeadingTrivia(t.LeadingTrivia.WithoutTrailingWhitespace(endOfLineIsWhitespace
				                                                                   : false)));
			...
			if (!replaceMap[token].LeadingTrivia.Any())
			{
				UpdateReplaceMap(
					...
					t => t.WithTrailingTrivia(t.TrailingTrivia.WithoutTrailingWhitespace(endOfLineIsWhitespace
					                                                                     : false)));
			}
	...
	case TokenSpacingProperties.LocationFollowing:
		...
		case TokenSpacingProperties.ActionInsert:
			...
			{
				// If the token is already present in the map and it has trailing trivia, then it has already
				// been processed during an earlier step in the fix all process, so no additional processing is
				// needed.
				if (!replaceMap.ContainsKey(token) || (replaceMap[token].TrailingTrivia.Count == 0))
				{
					UpdateReplaceMap(replaceMap, token,
					                 t =>
					                 t.WithTrailingTrivia(t.TrailingTrivia.Insert(0, SyntaxFactory.Space)));
				}
			}
		...
		case TokenSpacingProperties.ActionRemove:
			...
			UpdateReplaceMap(replaceMap, nextToken,
			                 t => t.WithLeadingTrivia(triviaList.WithoutLeadingWhitespace(endOfLineIsWhitespace
			                                                                              : !preserveLayout)));
			break;
		}
