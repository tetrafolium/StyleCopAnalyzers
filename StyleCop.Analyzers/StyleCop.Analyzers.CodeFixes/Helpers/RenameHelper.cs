        ...
        internal static class RenameHelper
        {
                                   ...
                                   string newName, CancellationToken cancellationToken)
                {
                        var annotatedRoot = root.ReplaceToken (
                            declarationToken, declarationToken.WithAdditionalAnnotations (
                        ...
                        var newSolution = await Renamer
                                              .RenameSymbolAsync (annotatedSolution, symbol,
                                                                  newName, null, cancellationToken)
                                              .ConfigureAwait (false);
                                           ...
                                           CancellationToken cancellationToken)
                {
                        if (symbol.Kind == SymbolKind.NamedType)
                                {
                                        ...
                                        // If the symbol is a class or struct, the name can't be the
                                        // same as any of its members.
                                        if (typeKind == TypeKind.Class
                                            || typeKind == TypeKind.Struct)
                                                {
                                                        var members = (symbol as INamedTypeSymbol)
                                                            ?.GetMembers (name);
                                                        if (members.HasValue
                                                            && !members.Value.IsDefaultOrEmpty)
                                                                {
                                                                        return false;
                                                                }
                                                }
                        ...
                        if (symbol.Kind == SymbolKind.TypeParameter)
                                {
                                        // If the symbol is a type parameter, the name can't be the
                                        // same as any type parameters of the containing type.
                                        if (containingSymbol?.ContainingSymbol is INamedTypeSymbol
                                                parentSymbol
                                            && parentSymbol.TypeParameters.Any (t => t.Name
                                                                                     == name))
                                                {
                                                        return false;
                                                }
                        ...
                        if (containingSymbol is INamespaceOrTypeSymbol
                                ...
                                {
                                        if (containingNamespaceOrTypeSymbol.Kind
                                            == SymbolKind.Namespace)
                                                {
                                                        // Make sure to use the compilation
                                                        // namespace so interfaces in referenced
                                                 ...
                                                 == SymbolKind.NamedType)
                                                {
                                                        ...
                                                        // If the containing type is a class or
                                                        // struct, the name can't be the same as the
                                                        // name of the containing type.
                                                        if ((typeKind == TypeKind.Class
                                                             || typeKind == TypeKind.Struct)
                                                            && containingNamespaceOrTypeSymbol.Name
                                                                   == name)
                                                                {
                                                                        return false;
                                                                }
                                        ...
                                        // The name can't be the same as the name of an other member
                                        // of the same type. At this point no special consideration
                                        // is given to overloaded methods.
                        ...
                        else if (containingSymbol.Kind == SymbolKind.Method)
                                {
                                        ...
                                        if (methodSymbol.Parameters.Any (i => i.Name == name)
                                            || methodSymbol.TypeParameters.Any (i => i.Name
                                                                                     == name))
                                                {
                                                        return false;
                                                }
                                        ...
                                        while (outermostMethod.ContainingSymbol.Kind
                                               == SymbolKind.Method)
                                                {
                                                        outermostMethod
                                                            = (IMethodSymbol)
                                                                  outermostMethod.ContainingSymbol;
                                                        if (outermostMethod.Parameters.Any (
                                                                i => i.Name == name)
                                                        ...
                                                        SyntaxNode syntaxNode
                                                            = await syntaxReference
                                                                  .GetSyntaxAsync (
                                                                      cancellationToken)
                                                                  .ConfigureAwait (false);
                                                        LocalNameFinder localNameFinder
                                                            = new LocalNameFinder (name);
                ...
                public static SyntaxNode
                GetParentDeclaration (SyntaxToken token)
                {
