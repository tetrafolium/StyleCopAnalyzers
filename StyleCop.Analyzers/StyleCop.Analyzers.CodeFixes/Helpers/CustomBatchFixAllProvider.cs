                        ...
                        if (fixAllContext.Document != null)
                                {
                                        ...
                                        return await this
                                            .GetFixAsync (documentsAndDiagnosticsToFixMap,
                                                          fixAllContext)
                                            .ConfigureAwait (false);
                                }
                        else
                                {
                                        var projectsAndDiagnosticsToFixMap
                                            = await this
                                                  .GetProjectDiagnosticsToFixAsync (fixAllContext)
                                                  .ConfigureAwait (false);
                                        return await this
                                            .GetFixAsync (projectsAndDiagnosticsToFixMap,
                                                          fixAllContext)
                                            .ConfigureAwait (false);
                                }
                }
        ...
        if (projectsAndDiagnosticsToFixMap != null && projectsAndDiagnosticsToFixMap.Any ())
                {
                        ...
                        foreach (var project in projectsAndDiagnosticsToFixMap.Keys)
                                {
                                        ...
                                        fixOperations.Add (this.AddProjectFixesAsync (
                                            project, diagnostics, fixesBag[index].Add,
                                            fixAllContext));
                                }
                        ...
                        if (fixesBag.Any (fixes => fixes.Count > 0))
                                {
                                        return await this
                                            .TryGetMergedFixAsync (fixesBag.SelectMany (i => i),
                                                                   fixAllContext)
                                            .ConfigureAwait (false);
                                }
                }
...
public virtual Task
AddProjectFixesAsync (Project project, IEnumerable<Diagnostic> diagnostics,
                      Action<CodeAction> addFix, FixAllContext fixAllContext)
{
...
public virtual async Task<CodeAction>
TryGetMergedFixAsync (IEnumerable<CodeAction> batchOfFixes, FixAllContext fixAllContext)
{
        ...
        if (!batchOfFixes.Any ())
                {
                        throw new ArgumentException ($"{nameof(batchOfFixes)} cannot be empty.",
                                                     nameof (batchOfFixes));
                }
        ...
        var newSolution
            = await this
                  .TryMergeFixesAsync (solution, batchOfFixes, fixAllContext.CancellationToken)
                  .ConfigureAwait (false);
        if (newSolution != null && newSolution != solution)
                {
                        var title = this.GetFixAllTitle (fixAllContext);
                        return CodeAction.Create (
                            title, cancellationToken => Task.FromResult (newSolution));
                }
...
GetFixAllTitle (FixAllContext fixAllContext)
{
...
public virtual async Task<Solution>
TryMergeFixesAsync (Solution oldSolution, IEnumerable<CodeAction> codeActions,
        ...
        foreach (var codeAction in codeActions)
                {
                        ...
                        foreach (var operation in operations)
                                {
                                        ...
                                        if (singleApplyChangesOperation != null)
                                                {
                                                        // Already had an ApplyChangesOperation;
                        ...
                        var documentIdsWithChanges
                            = solutionChanges.GetProjectChanges ().SelectMany (
                                p => p.GetChangedDocuments ());
                        ...
                        foreach (var documentId in documentIdsWithChanges)
                                {
                                        ...
                                        if (changedDocumentsMap.TryGetValue (documentId,
                                                                             out existingDocument))
                                                {
                                                        if (existingDocument != null)
                                                                {
                                                                        changedDocumentsMap
                                                                            [documentId]
                                                                            = null;
                                                                        var documentsToMerge
                                                                            = new List<Document> ();
                                                                        documentsToMerge.Add (
                                                                            existingDocument);
                                                                        documentsToMerge.Add (
                                                                            document);
                                                                        documentsToMergeMap
                                                                            = documentsToMergeMap ?? new Dictionary<
                                                                                DocumentId,
                                                                                List<Document> > ();
                                                                        documentsToMergeMap
                                                                            [documentId]
                                        ...
                                        mergeTasks[i] = Task.Run (async () => {
                                                var appliedChanges
                                                    = (await documentsToMerge [0]
                                                           .GetTextChangesAsync (oldDocument,
                                                                                 cancellationToken)
                                                           .ConfigureAwait (false))
                                                          .ToList ();
                        ...
                        foreach (var kvp in mergedDocuments)
                                {
                                        cancellationToken.ThrowIfCancellationRequested ();
                                        currentSolution
                                            = currentSolution.WithDocumentText (kvp.Key, kvp.Value);
                                }
                }
                                 ...
                                 CancellationToken cancellationToken)
{
        ...
        foreach (var change in await newDocument
                     .GetTextChangesAsync (oldDocument, cancellationToken)
                     .ConfigureAwait (false))
                {
                        cancellationToken.ThrowIfCancellationRequested ();
                        while (cumulativeChangeIndex < cumulativeChanges.Count
                               && cumulativeChanges[cumulativeChangeIndex].Span.End
                                      < change.Span.Start)
                                {
                                        cancellationToken.ThrowIfCancellationRequested ();

                                        // Existing change that does not overlap with the current
                                        // change in consideration
                                        successfullyMergedChanges.Add (
                                            cumulativeChanges[cumulativeChangeIndex]);
                        ...
                        if (cumulativeChangeIndex < cumulativeChanges.Count)
                                {
                                        ...
                                        if (!cumulativeChange.Span.IntersectsWith (change.Span))
                                                {
                                                        // The current change in consideration does
                                                        // not intersect with any existing change
                                        ...
                                        else
                                                {
                                                        if (change.Span != cumulativeChange.Span
                                                            || change.NewText
                                                                   != cumulativeChange.NewText)
                                                                {
                                                                        // The current change in
                                                                        // consideration overlaps an
                                                                        // existing change but the
                                                                        // changes are not
                                                                        // identical. Bail out merge
                                                                        // efforts and return the
                                                                        // original
                                                                        // 'cumulativeChanges'.
                                ...
                                {
                                        // The current change in consideration does not intersect
                                        // with any existing change
                                        successfullyMergedChanges.Add (change);
                                }
                }
