...
namespace StyleCop.Analyzers.Helpers
{
        ...
        internal partial class CustomBatchFixAllProvider : FixAllProvider {
                ...
                = new CustomBatchFixAllProvider();

                public override async Task<CodeAction> GetFixAsync(FixAllContext fixAllContext)
                {
                        if (fixAllContext.Document != null) {
                                var documentsAndDiagnosticsToFixMap
                                    = await this.GetDocumentDiagnosticsToFixAsync(fixAllContext)
                                          .ConfigureAwait(false);
                                return await this
                                    .GetFixAsync(documentsAndDiagnosticsToFixMap, fixAllContext)
                                    .ConfigureAwait(false);
                        } else {
                                var projectsAndDiagnosticsToFixMap
                                    = await this.GetProjectDiagnosticsToFixAsync(fixAllContext)
                                          .ConfigureAwait(false);
                                return await this
                    ...
                    FixAllContext fixAllContext)
                {
                        if (documentsAndDiagnosticsToFixMap != null
                            && documentsAndDiagnosticsToFixMap.Any()) {
                                ...
                                var documents
                                    = documentsAndDiagnosticsToFixMap.Keys.ToImmutableArray();
                                ...
                                for (int index = 0; index < documents.Length; index++) {
                                        ...
                                        fixOperations.Add(this.AddDocumentFixesAsync(document,
                                            documentsAndDiagnosticsToFixMap[document],
                                            fixesBag[index].Add, fixAllContext));
                                }
                                ...
                                if (fixesBag.Any(fixes => fixes.Count > 0)) {
                                        return await this
                                            .TryGetMergedFixAsync(
                                                fixesBag.SelectMany(i => i), fixAllContext)
                                            .ConfigureAwait(false);
                                }
                        }
                ...
                public async virtual Task AddDocumentFixesAsync(Document document,
                    ImmutableArray<Diagnostic> diagnostics, Action<CodeAction> addFix,
                    FixAllContext fixAllContext)
                {
                ...
                fixerTasks[i] = Task.Run(async () =>
                {
                    ...
                    var context = new CodeFixContext(
                        ...
                        // TODO: Wrap call to ComputeFixesAsync() below in
                        // IExtensionManager.PerformFunctionAsync() so that a buggy extension that
                        // throws can't bring down the host?
                        var task = fixAllContext.CodeFixProvider.RegisterCodeFixesAsync(context)
                            ?? SpecializedTasks.CompletedTask;
                        await task.ConfigureAwait(false);

                        cancellationToken.ThrowIfCancellationRequested();
                        localFixes.RemoveAll(action => action.EquivalenceKey
    ...
    FixAllContext fixAllContext)
{
        if (projectsAndDiagnosticsToFixMap != null && projectsAndDiagnosticsToFixMap.Any()) {
                ...
                foreach (var project in projectsAndDiagnosticsToFixMap.Keys) {
                        ...
                        fixesBag[index] = new List<CodeAction>();
                        fixOperations.Add(this.AddProjectFixesAsync(
                            project, diagnostics, fixesBag[index].Add, fixAllContext));
                }
                ...
                if (fixesBag.Any(fixes => fixes.Count > 0)) {
                        return await this
                            .TryGetMergedFixAsync(fixesBag.SelectMany(i => i), fixAllContext)
                            .ConfigureAwait(false);
                }
        }
...
public virtual async Task<CodeAction> TryGetMergedFixAsync(
    IEnumerable<CodeAction> batchOfFixes, FixAllContext fixAllContext)
{
        ...
        if (!batchOfFixes.Any()) {
                throw new ArgumentException(
                    $"{nameof(batchOfFixes)} cannot be empty.", nameof(batchOfFixes));
        }

        var solution = fixAllContext.Solution;
        var newSolution
            = await this.TryMergeFixesAsync(solution, batchOfFixes, fixAllContext.CancellationToken)
                  .ConfigureAwait(false);
...
public virtual Task<ImmutableDictionary<Document, ImmutableArray<Diagnostic>>>
...
public virtual Task<ImmutableDictionary<Project, ImmutableArray<Diagnostic>>>
...
public virtual async Task<Solution> TryMergeFixesAsync(
    Solution oldSolution, IEnumerable<CodeAction> codeActions, CancellationToken cancellationToken)
{
        ...
        foreach (var codeAction in codeActions) {
                ...
                ImmutableArray<CodeActionOperation> operations
                    = await codeAction.GetPreviewOperationsAsync(cancellationToken)
                          .ConfigureAwait(false);
                ...
                foreach (var documentId in documentIdsWithChanges) {
                        ...
                        if (changedDocumentsMap.TryGetValue(documentId, out existingDocument)) {
                                if (existingDocument != null) {
                                        ...
                                        documentsToMergeMap
                                            = documentsToMergeMap ?? new Dictionary<DocumentId,
                                                List<Document>>();
                                        documentsToMergeMap[documentId] = documentsToMerge;
                                } else {
                                        documentsToMergeMap [documentId]
                                            .Add(document);
                                }
                        } else {
                ...
                for (int i = 0; i < documentsToMergeArray.Length; i++) {
                        ...
                        mergeTasks[i] = Task.Run(async() => {
                                ...
                                foreach (var document in documentsToMerge.Skip(1)) {
                                        cancellationToken.ThrowIfCancellationRequested();
                                        appliedChanges
                                            = await TryAddDocumentMergeChangesAsync(oldDocument,
                                                document, appliedChanges, cancellationToken)
                                                  .ConfigureAwait(false);
                                }
...
/// <summary>
/// Try to merge the changes between <paramref name="newDocument"/> and <paramref
/// name="oldDocument"/> into <paramref name="cumulativeChanges"/>. If there is any conflicting
/// change in <paramref name="newDocument"/> with existing <paramref name="cumulativeChanges"/>,
/// then the original <paramref name="cumulativeChanges"/> are returned. Otherwise, the newly merged
...
private static async Task<List<TextChange>> TryAddDocumentMergeChangesAsync(Document oldDocument,
    Document newDocument, List<TextChange> cumulativeChanges, CancellationToken cancellationToken)
{
        var successfullyMergedChanges = new List<TextChange>();

        int cumulativeChangeIndex = 0;
        foreach (var change in await newDocument.GetTextChangesAsync(oldDocument, cancellationToken)
                     .ConfigureAwait(false)) {
                cancellationToken.ThrowIfCancellationRequested();
                while (cumulativeChangeIndex < cumulativeChanges.Count
                    && cumulativeChanges[cumulativeChangeIndex].Span.End < change.Span.Start) {
                        ...
                        // Existing change that does not overlap with the current change in
                        // consideration
                ...
                if (cumulativeChangeIndex < cumulativeChanges.Count) {
                        ...
                        if (!cumulativeChange.Span.IntersectsWith(change.Span)) {
                                // The current change in consideration does not intersect with any
                                // existing change
                                successfullyMergedChanges.Add(change);
                        } else {
                                if (change.Span != cumulativeChange.Span
                                    || change.NewText != cumulativeChange.NewText) {
                                        // The current change in consideration overlaps an existing
                                        // change but the changes are not identical. Bail out merge
                                        // efforts and return the original 'cumulativeChanges'.
                                        return cumulativeChanges;
                                } else {
                                        // The current change in consideration is identical to an
                                        // existing change
                                        successfullyMergedChanges.Add(change);
                                        cumulativeChangeIndex++;
                                }
                        ...
                        // The current change in consideration does not intersect with any existing
                        // change
                        successfullyMergedChanges.Add(change);
                }
        }
