...
public override async Task<CodeAction> GetFixAsync(FixAllContext fixAllContext)
{
	if (fixAllContext.Document != null)
	{
		var documentsAndDiagnosticsToFixMap =
			await this.GetDocumentDiagnosticsToFixAsync(fixAllContext).ConfigureAwait(false);
	...
	else
	{
		var projectsAndDiagnosticsToFixMap =
			await this.GetProjectDiagnosticsToFixAsync(fixAllContext).ConfigureAwait(false);
	...
	FixAllContext fixAllContext)
{
	if (documentsAndDiagnosticsToFixMap != null && documentsAndDiagnosticsToFixMap.Any())
	{
		...
		for (int index = 0; index < documents.Length; index++)
		{
			...
			fixOperations.Add(this.AddDocumentFixesAsync(document, documentsAndDiagnosticsToFixMap[document],
		...
		{
			return await this.TryGetMergedFixAsync(fixesBag.SelectMany(i => i), fixAllContext)
...
public async virtual Task AddDocumentFixesAsync(Document document, ImmutableArray<Diagnostic> diagnostics,
                                                           ...
                                                           FixAllContext fixAllContext)
{
	...
	var newSolution =
		await this.TryMergeFixesAsync(solution, batchOfFixes, fixAllContext.CancellationToken).ConfigureAwait(false);
...
public virtual Task<ImmutableDictionary<Document, ImmutableArray<Diagnostic> > > GetDocumentDiagnosticsToFixAsync(
	FixAllContext fixAllContext)
{
	return FixAllContextHelper.GetDocumentDiagnosticsToFixAsync(fixAllContext);
}

public virtual Task<ImmutableDictionary<Project, ImmutableArray<Diagnostic> > > GetProjectDiagnosticsToFixAsync(
	FixAllContext fixAllContext)
{
	return FixAllContextHelper.GetProjectDiagnosticsToFixAsync(fixAllContext);
}

public virtual async Task<Solution> TryMergeFixesAsync(Solution oldSolution, IEnumerable<CodeAction> codeActions,
                                                       CancellationToken cancellationToken)
{
	...
	foreach (var codeAction in codeActions)
	{
		...
		ImmutableArray<CodeActionOperation> operations =
			await codeAction.GetPreviewOperationsAsync(cancellationToken).ConfigureAwait(false);
	...
	if (documentsToMergeMap != null)
	{
		...
		for (int i = 0; i < documentsToMergeArray.Length; i++)
		{
			...
			mergeTasks[i] = Task.Run(async() => {
						var appliedChanges = (await documentsToMerge [0]
						...
						foreach (var document in documentsToMerge.Skip(1))
						{
						        cancellationToken.ThrowIfCancellationRequested();
						        appliedChanges =
								await TryAddDocumentMergeChangesAsync(oldDocument, document, appliedChanges, cancellationToken)
								.ConfigureAwait(false);
						}
...
/// <summary>
/// Try to merge the changes between <paramref name="newDocument"/> and <paramref name="oldDocument"/> into <paramref
/// name="cumulativeChanges"/>. If there is any conflicting change in <paramref name="newDocument"/> with existing
/// <paramref name="cumulativeChanges"/>, then the original <paramref name="cumulativeChanges"/> are returned.
...
/// <param name="cumulativeChanges">Existing merged changes from other batch fixes into which newDocument changes are
/// being merged.</param> <param name="cancellationToken">Cancellation token.</param> <returns>A <see cref="Task"/>
/// representing the asynchronous operation.</returns>
private static async Task<List<TextChange> > TryAddDocumentMergeChangesAsync(Document oldDocument, Document newDocument,
