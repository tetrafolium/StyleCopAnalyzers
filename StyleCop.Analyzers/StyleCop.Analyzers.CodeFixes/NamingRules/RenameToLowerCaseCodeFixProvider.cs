...
namespace StyleCop.Analyzers.NamingRules
{
        ...
        /// <summary>
        /// Implements a code fix for diagnostics which are fixed by renaming a symbol to start with
        /// a lower case letter.
        /// </summary>
        /// <remarks>
        /// <para>To fix a violation of this rule, change the name of the field or variable so that
                        ...
                        foreach (var diagnostic in context.Diagnostics)
                                {
                                        ...
                                        if (baseName.Length == 0)
                                                {
                                                        // only offer a code fix if the name does
                                                        // not consist of only underscores.
                                                        continue;
                                                }

                                        baseName
                                            = char.ToLower (baseName[0]) + baseName.Substring (1);
