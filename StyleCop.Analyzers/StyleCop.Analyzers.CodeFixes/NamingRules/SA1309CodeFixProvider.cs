...
namespace StyleCop.Analyzers.NamingRules
{
        ...
        /// </summary>
        /// <remarks>
        /// <para>To fix a violation of this rule, remove the underscore from the beginning of the
        /// field name, or place the item within a <c>NativeMethods</c> class if appropriate.</para>
        /// </remarks>
                        ...
                        foreach (var diagnostic in context.Diagnostics)
                                {
                                        ...
                                        if (!string.IsNullOrEmpty (token.ValueText))
                                                {
                                                        var newName = token.ValueText.TrimStart (
                                                            new[]{ '_' });

                                                        if (!SyntaxFacts.IsValidIdentifier (
                                                                newName))
                                                                {
                                                                        // The proposed name was not
                                                                        // legal, so no code fix
                                                                        // will be offered.
                                                                        continue;
                                                                }
                                                        ...
                                                        context.RegisterCodeFix (
                                                            CodeAction.Create (
                                                                string.Format (
                                                                    NamingResources.RenameToCodeFix,
                                                                    newName),
