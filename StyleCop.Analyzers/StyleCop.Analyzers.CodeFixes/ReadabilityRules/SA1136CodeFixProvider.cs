...
public override Task RegisterCodeFixesAsync(CodeFixContext context)
{
	...
	{
		context.RegisterCodeFix(CodeAction.Create(ReadabilityResources.SA1136CodeFix,
                                                                ...
                                                                CancellationToken cancellationToken)
{
	...
	var enumMemberDeclaration =
		(EnumMemberDeclarationSyntax) syntaxRoot.FindNode(diagnostic.Location.SourceSpan);
	...
	var indentation =
		IndentationHelper.GenerateWhitespaceTrivia(settings.Indentation, parentIndentationSteps + 1);

	// combine all trivia between the separator and the enum member and place them after the separator, followed
	// by a new line.
	var enumMemberDeclarationFirstToken = enumMemberDeclaration.GetFirstToken();
	var sharedTrivia = TriviaHelper.MergeTriviaLists(precedingSeparatorToken.TrailingTrivia,
	...
	var newSyntaxRoot =
		syntaxRoot.ReplaceTokens(replacements.Keys, (original, rewritten) => replacements[original]);
