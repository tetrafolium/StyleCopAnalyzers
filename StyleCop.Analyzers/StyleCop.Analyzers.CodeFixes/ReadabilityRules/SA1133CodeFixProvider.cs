...
public override Task RegisterCodeFixesAsync(CodeFixContext context)
{
	...
	{
		context.RegisterCodeFix(CodeAction.Create(ReadabilityResources.SA1133CodeFix,
...
private static List<AttributeListSyntax> GetNewAttributeList(AttributeListSyntax attributeList,
                                                             SyntaxTrivia indentationTrivia)
{
	...
	{
		var newAttributes =
			SyntaxFactory.SingletonSeparatedList(attributeList
			                                     .Attributes [i]
		...
		newAttributeList = (i == 0) ? newAttributeList.WithLeadingTrivia(attributeList.GetLeadingTrivia())
...
protected override string CodeActionTitle => ReadabilityResources.SA1133CodeFix;
                                                                ...
                                                                ImmutableArray<Diagnostic> diagnostics)
{
	...
	foreach (var attributeList in nodes)
	{
		...
		var indentationTrivia =
			IndentationHelper.GenerateWhitespaceTrivia(settings.Indentation, indentationSteps);
		newRoot = newRoot.ReplaceNode(newRoot.GetCurrentNode(attributeList),
