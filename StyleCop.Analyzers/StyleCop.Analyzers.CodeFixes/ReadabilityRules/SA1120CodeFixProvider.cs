...
namespace StyleCop.Analyzers.ReadabilityRules
{
        ...
        internal class SA1120CodeFixProvider : CodeFixProvider
        {
                ...
                private static bool
                TriviaHasLeadingContentOnLine (SyntaxNode root, SyntaxTrivia commentTrivia)
                {
                        if (commentTrivia.SpanStart == 0)
                                {
                                        // It is impossible to have leading content at the start of
                                        // the file.
                                        return false;
                                }
                ...
                private static bool
                TriviaHasTrailingContentOnLine (SyntaxNode root, SyntaxTrivia commentTrivia)
                {
                        if (commentTrivia.Span.End == root.Span.End)
                                {
                                        // It is impossible to have trailing content at the end of
                                        // the file.
                                        return false;
                                }
