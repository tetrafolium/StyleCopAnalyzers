...
public override async Task RegisterCodeFixesAsync(CodeFixContext context)
{
	foreach (var diagnostic in context.Diagnostics)
	{
		...
		context.RegisterCodeFix(CodeAction.Create(ReadabilityResources.SA1130CodeFix,
...
private static async Task<bool> CanFixAsync(CodeFixContext context, Diagnostic diagnostic)
{
	...
	var anonymousMethod = (AnonymousMethodExpressionSyntax)
...
private static SyntaxNode ReplaceWithLambda(SemanticModel semanticModel,
                                            AnonymousMethodExpressionSyntax anonymousMethod)
{
	...
	if (parameterList == null)
	{
		...
		List<ParameterSyntax> parameters =
			GenerateUniqueParameterNames(semanticModel, anonymousMethod, argumentList);

		var newList = (parameters.Count > 0)
		                  ? SyntaxFactory.SeparatedList(
			parameters, Enumerable.Repeat(ParameterListSeparator, parameters.Count - 1))
		                  : SyntaxFactory.SeparatedList<ParameterSyntax>();

		parameterList = SyntaxFactory.ParameterList(newList).WithLeadingTrivia(
	...
	if (parameterList.Parameters.Count == 1)
	{
		...
		var leadingTrivia =
			parameterList.OpenParenToken.LeadingTrivia.Concat(parameterList.OpenParenToken.TrailingTrivia)
		...
		lambdaExpression = SyntaxFactory.SimpleLambdaExpression(anonymousMethod.AsyncKeyword, parameterSyntax,
	...
	else
	{
		var parameterListSyntax =
			...
			.WithTrailingTrivia(parameterList.GetTrailingTrivia().WithoutTrailingWhitespace().Add(
						    SyntaxFactory.ElasticSpace));
		lambdaExpression = SyntaxFactory.ParenthesizedLambdaExpression(
			anonymousMethod.AsyncKeyword, parameterListSyntax, arrowToken, anonymousMethod.Body);
	}
	...
	SemanticModel semanticModel, AnonymousMethodExpressionSyntax anonymousMethod)
{
	...
	var parameterList =
		SA1130UseLambdaSyntax.GetDelegateParameterList(originalSymbolInfo.Symbol, argumentIndex);
	return parameterList.Parameters.Select(p => p.Identifier.ToString()).ToImmutableArray();
}

private static ImmutableArray<string> GetEqualsArgumentList(SemanticModel semanticModel,
                                                            AnonymousMethodExpressionSyntax anonymousMethod)
{
...
? GetAssignmentArgumentList(SemanticModel semanticModel, AnonymousMethodExpressionSyntax anonymousMethod)
{
	...
	SemanticModel semanticModel, AnonymousMethodExpressionSyntax anonymousMethod)
{
	...
	return !(((IMethodSymbol) enclosingSymbol).ReturnType is INamedTypeSymbol returnType)
	               ? ImmutableArray<string>.Empty
	               : returnType.DelegateInvokeMethod.Parameters.Select(ps => ps.Name).ToImmutableArray();
}
...
private static ParameterListSyntax RemoveType(ParameterListSyntax parameterList)
{
	return parameterList.WithParameters(SyntaxFactory.SeparatedList(
						    parameterList.Parameters.Select(x => RemoveType(x)), parameterList.Parameters.GetSeparators()));
}
...
private static ParameterSyntax RemoveType(ParameterSyntax parameterSyntax)
{
	...
	if (parameterSyntax.Type != null)
	{
		syntax = syntax.WithLeadingTrivia(
			parameterSyntax.Type.GetLeadingTrivia().Concat(parameterSyntax.Type.GetTrailingTrivia()));
	}
...
{
	// If one of the following conditions is false the code won't compile, but we want to check for it anyway
	// and not make it worse by applying this code fix.
	return parameterSyntax.AttributeLists.Count == 0 && parameterSyntax.Default == null &&
