...
namespace StyleCop.Analyzers.ReadabilityRules
{
        ...
        {
                private static readonly SyntaxToken ParameterListSeparator
                ...
                RegisterCodeFixesAsync (CodeFixContext context)
                {
                        foreach (var diagnostic in context.Diagnostics)
                                {
                                        if (!(await CanFixAsync (context, diagnostic)
                ...
                CanFixAsync (CodeFixContext context, Diagnostic diagnostic)
                {
                        var syntaxRoot
                        ...
                        var anonymousMethod
                            = (AnonymousMethodExpressionSyntax) syntaxRoot.FindNode (
                                   ...
                                   AnonymousMethodExpressionSyntax anonymousMethod)
                {
                        ...
                        if (parameterList == null)
                                {
                                        ...
                                        switch (anonymousMethod.Parent.Kind ())
                                                ...
                                                case SyntaxKind.Argument:
                                                        argumentList
                                                            = GetMethodInvocationArgumentList (
                                                ...
                                                case SyntaxKind.ReturnStatement:
                                                        argumentList
                                                            = GetMemberReturnTypeArgumentList (
                                                                semanticModel, anonymousMethod);
                                        ...
                                        List<ParameterSyntax> parameters
                                            = GenerateUniqueParameterNames (
                                                semanticModel, anonymousMethod, argumentList);

                                        var newList
                                            = (parameters.Count > 0)
                                                  ? SyntaxFactory.SeparatedList (
                                                      parameters,
                                                      Enumerable.Repeat (ParameterListSeparator,
                                                                         parameters.Count - 1))
                                ...
                                {
                                        parameterList = parameterList.WithLeadingTrivia (
                                            anonymousMethod.DelegateKeyword.TrailingTrivia);
                        ...
                        if (parameterList.Parameters.Count == 1)
                                {
                                        var parameterSyntax
                                            = RemoveType (parameterList.Parameters[0]);

                                        var trailingTrivia
                                            = parameterSyntax.GetTrailingTrivia ()
                                                  .Concat (
                                                      parameterList.CloseParenToken.LeadingTrivia)
                                                  .Concat (
                                                      parameterList.CloseParenToken.TrailingTrivia
                                                          .WithoutTrailingWhitespace ())
                                                  .Concat (new[]{ SyntaxFactory.ElasticSpace });
                                        var leadingTrivia
                                            = parameterList.OpenParenToken.LeadingTrivia
                                                  .Concat (
                                                      parameterList.OpenParenToken.TrailingTrivia)
                                                  .Concat (parameterSyntax.GetLeadingTrivia ());

                                        parameterSyntax
                                            = parameterSyntax.WithLeadingTrivia (leadingTrivia)
                                                  .WithTrailingTrivia (trailingTrivia);

                                        lambdaExpression = SyntaxFactory.SimpleLambdaExpression (
                                            anonymousMethod.AsyncKeyword, parameterSyntax,
                                            arrowToken, anonymousMethod.Body);
                                }
                        else
                                {
                                        var parameterListSyntax
                                            = RemoveType (parameterList)
                                                  .WithTrailingTrivia (
                                                      parameterList.GetTrailingTrivia ()
                                                          .WithoutTrailingWhitespace ()
                                                          .Add (SyntaxFactory.ElasticSpace));
                                        lambdaExpression
                                            = SyntaxFactory.ParenthesizedLambdaExpression (
                                                anonymousMethod.AsyncKeyword, parameterListSyntax,
                                                arrowToken, anonymousMethod.Body);
                                }
                                                 ...
                                                 AnonymousMethodExpressionSyntax anonymousMethod)
                {
                        ...
                        var parameterList = SA1130UseLambdaSyntax.GetDelegateParameterList (
                                       ...
                                       AnonymousMethodExpressionSyntax anonymousMethod)
                {
                        ...
                        var variableDeclaration
                            = (VariableDeclarationSyntax) equalsValueClauseSyntax.Parent.Parent;

                        var symbol = semanticModel.GetSymbolInfo (variableDeclaration.Type);
                        var namedTypeSymbol = (INamedTypeSymbol) symbol.Symbol;
                        return namedTypeSymbol.DelegateInvokeMethod.Parameters
                            .Select (ps => ps.Name)
                            .ToImmutableArray ();
                }

                private static ImmutableArray<string>
                    ? GetAssignmentArgumentList (SemanticModel semanticModel,
                                                 AnonymousMethodExpressionSyntax anonymousMethod)
                {
                        var assignmentExpressionSyntax
                            = (AssignmentExpressionSyntax) anonymousMethod.Parent;
                                                 ...
                                                 AnonymousMethodExpressionSyntax anonymousMethod)
                {
                        ...
                        return !(((IMethodSymbol) enclosingSymbol)
                                     .ReturnType is INamedTypeSymbol returnType)
                                   ? ImmutableArray<string>.Empty
                                   : returnType.DelegateInvokeMethod.Parameters
                                         .Select (ps => ps.Name)
                                         .ToImmutableArray ();
                }
                        ...
                        foreach (var argumentName in argumentNames)
                                {
                                        ...
                                        while (semanticModel
                                                   .LookupSymbols (anonymousMethod.SpanStart, name
                                                                   : newName)
                                                   .Length
                                               > 0)
                                                {
                                        ...
                                        parameters.Add (
                                            SyntaxFactory
                                                .Parameter (SyntaxFactory.Identifier (newName))
                                                .WithType (null));
                                }
                ...
                RemoveType (ParameterListSyntax parameterList)
                {
                        return parameterList.WithParameters (SyntaxFactory.SeparatedList (
                            parameterList.Parameters.Select (x => RemoveType (x)),
                            parameterList.Parameters.GetSeparators ()));
                }

                private static ParameterSyntax
                RemoveType (ParameterSyntax parameterSyntax)
                {
                        var syntax = parameterSyntax.WithType (null);

                        if (parameterSyntax.Type != null)
                                {
                                        syntax = syntax.WithLeadingTrivia (
                                            parameterSyntax.Type.GetLeadingTrivia ().Concat (
                                                parameterSyntax.Type.GetTrailingTrivia ()));
                                }

                        return syntax
                            .WithTrailingTrivia (
                                syntax.GetTrailingTrivia ().WithoutTrailingWhitespace ())
                            .WithLeadingTrivia (syntax.GetLeadingTrivia ().WithoutWhitespace ());
                }

                private static bool
                IsValid (ParameterSyntax parameterSyntax)
                {
                        // If one of the following conditions is false the code won't compile, but
                        // we want to check for it anyway and not make it worse by applying this
                        // code fix.
                ...
                private class FixAll : DocumentBasedFixAllProvider
                {
                        ...
                        protected override string
                            CodeActionTitle => ReadabilityResources.SA1130CodeFix;
                                               ...
                                               ImmutableArray<Diagnostic> diagnostics)
                        {
                                ...
                                foreach (var diagnostic in diagnostics)
                                        {
                                                var node = (AnonymousMethodExpressionSyntax)
                                                               syntaxRoot.FindNode (
                                ...
                                return syntaxRoot.ReplaceNodes (
                                    nodes, (originalNode, rewrittenNode) => {
                                            var newNode
                                                = ReplaceWithLambda (semanticModel, rewrittenNode);
