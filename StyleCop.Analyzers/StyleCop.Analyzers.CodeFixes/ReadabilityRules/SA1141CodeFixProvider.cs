        ...
        internal class SA1141CodeFixProvider : CodeFixProvider
        {
                                             ...
                                             CancellationToken cancellationToken)
                {
                        ...
                        if (node.IsKind (SyntaxKind.SimpleMemberAccessExpression))
                                {
                                        // get the invocation node when processing
                                        // ValueTuple.Create, as that needs to be replaced.
                                        node = node.Parent;
                                }
                ...
                GetReplacementNode (SemanticModel semanticModel, SyntaxNode node)
                {
                        switch (node)
                                {
                                        ...
                                        if (IsValueTuple (semanticModel, typeArgument))
                                                {
                                                        var tupleTypeSyntax = (TypeSyntax)
                                                            GetReplacementNode (semanticModel,
                                                                                typeArgument);
                                                        tupleElements = tupleElements.Add (
                                                            SyntaxFactoryEx.TupleElement (
                                                                tupleTypeSyntax));
                                                }
                                        else
                                                {
                ...
                private static bool
                IsValueTuple (SemanticModel semanticModel, TypeSyntax typeSyntax)
                {
