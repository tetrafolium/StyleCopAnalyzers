...
namespace StyleCop.Analyzers.ReadabilityRules
{
        ...
        internal class IndentationCodeFixProvider : CodeFixProvider
        {
                ...
                private static bool
                TryGetTextChange (Diagnostic diagnostic, SyntaxNode syntaxRoot,
                                  out TextChange textChange)
                {
                        ...
                        if (trivia == default)
                                {
                                        // The warning was reported on a token because the line is
                                        // not indented
                                        originalSpan = new TextSpan (
                                            diagnostic.Location.SourceSpan.Start, 0);
                                }
                        else
                                {
                ...
                private class FixAll : DocumentBasedFixAllProvider
                {
                                               ...
                                               ImmutableArray<Diagnostic> diagnostics)
                        {
                                ...
                                foreach (var diagnostic in diagnostics)
                                        {
                                                TextChange textChange;
                                                if (TryGetTextChange (diagnostic, syntaxRoot,
                                                                      out textChange))
                                                        {
