...
namespace StyleCop.Analyzers.ReadabilityRules
{
        ...
        internal class SA1135CodeFixProvider : CodeFixProvider
        {
                ...
                GetReplacementNode (SemanticModel semanticModel, UsingDirectiveSyntax node,
                                    CancellationToken cancellationToken)
                {
                ...
                GetReplacementName (TypeSyntax symbolNameSyntax, TypeSyntax nameSyntax)
                {
                                           ...
                                           GenericNameSyntax genericNameSyntax)
                {
                        ...
                        TypeArgumentListSyntax newTypeArgumentList
                            = GetReplacementTypeArgumentList (symbolGenericNameSyntax,
                                                              genericNameSyntax);
                                                ...
                                                GenericNameSyntax genericNameSyntax)
                {
                        ...
                        for (var i = 0; i < genericNameSyntax.TypeArgumentList.Arguments.Count; i++)
                                {
                                        var argument
                                            = genericNameSyntax.TypeArgumentList.Arguments[i];

                                        if (!argument.IsKind (SyntaxKind.PredefinedType))
                                                {
                                                        var symbolArgument
                                                            = symbolGenericNameSyntax
                                                                  .TypeArgumentList.Arguments[i];

                                                        var replacementArgument
                                                            = GetReplacementName (symbolArgument,
                                                                                  argument)
                                                                  .WithLeadingTrivia (
