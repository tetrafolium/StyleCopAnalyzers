        ...
        internal class SA1134CodeFixProvider : CodeFixProvider
        {
                ...
                RegisterCodeFixesAsync (CodeFixContext context)
                {
                        ...
                        foreach (var diagnostic in context.Diagnostics)
                                {
                                        // Do not offer the code fix if the error is found at an
                                        // invalid node (like IncompleteMemberSyntax)
                                             ...
                                             CancellationToken cancellationToken)
                {
                        ...
                        var attributeListSyntax = (AttributeListSyntax) syntaxRoot.FindNode (
                            diagnostic.Location.SourceSpan);

                        // use the containing type to determine the indentation level, anything else
                        // is less reliable.
                        ...
                        if (diagnostic.Properties.ContainsKey (
                                SA1134AttributesMustNotShareLine.FixWithNewLineBeforeKey))
                                {
                                        ...
                                        tokensToReplace[prevToken] = prevToken.WithTrailingTrivia (
                                            prevToken.TrailingTrivia.WithoutTrailingWhitespace ()
