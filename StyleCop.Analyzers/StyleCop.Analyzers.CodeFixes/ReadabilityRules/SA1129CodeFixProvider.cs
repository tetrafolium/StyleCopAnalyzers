...
namespace StyleCop.Analyzers.ReadabilityRules
{
        ...
        internal class SA1129CodeFixProvider : CodeFixProvider
        {
                ...
                GetReplacementNode (SyntaxNode node, SemanticModel semanticModel,
                                    CancellationToken cancellationToken)
                {
                ...
                /// <returns><see langword="true"/> if the syntax matches the type; <see
                /// langword="false"/> otherwise.</returns>
                private static bool
                IsType<T> (INamedTypeSymbol namedTypeSymbol)
                {
                ...
                IsDefaultParameterValue (ObjectCreationExpressionSyntax expression)
                {
                        if (expression.Parent.Parent is ParameterSyntax parameterSyntax)
                                {
                                        return parameterSyntax.Parent.Parent is
                ...
                /// <summary>
                /// Determines whether a given enumeration symbol contains a member with value
                /// <c>0</c>.
                /// </summary>
                /// <param name="namedTypeSymbol">The symbol.</param>
                /// <param name="foundMemberName">Will be set to the string name of the member, if
                /// one is found.</param> <returns><see langword="true"/> if the syntax is an
                /// enumeration with a value of <c>0</c>; <see langword="false"/>
                /// otherwise.</returns>
                private static bool
                IsEnumWithDefaultMember (INamedTypeSymbol namedTypeSymbol,
                ...
                /// <summary>
                /// Gets a qualified member access expression for the given <paramref
                /// name="typeSyntax"/>.
                /// </summary>
                ...
                private static SyntaxNode
                ConstructMemberAccessSyntax (TypeSyntax typeSyntax, string memberName)
                {
                ...
                private class FixAll : DocumentBasedFixAllProvider
                {
                                               ...
                                               ImmutableArray<Diagnostic> diagnostics)
                        {
                                ...
                                return syntaxRoot.ReplaceNodes (
                                    nodes, (originalNode, rewrittenNode) => GetReplacementNode (
