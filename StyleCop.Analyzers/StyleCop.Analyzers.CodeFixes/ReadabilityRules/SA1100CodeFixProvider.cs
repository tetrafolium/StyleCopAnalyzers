...
namespace StyleCop.Analyzers.ReadabilityRules
{
        ...
        /// <summary>
        /// This class provides a code fix for <see
        /// cref="SA1100DoNotPrefixCallsWithBaseUnlessLocalImplementationExists"/>.
        /// </summary>
        /// <remarks>
        /// <para>To fix a violation of this rule, change the <c>base.</c> prefix to
        /// <c>this.</c>.</para>
        /// </remarks>
        ...
        internal class SA1100CodeFixProvider : CodeFixProvider
        {
                ...
                = ImmutableArray.Create (
                    SA1100DoNotPrefixCallsWithBaseUnlessLocalImplementationExists.DiagnosticId);
                ...
                private static BaseExpressionSyntax
                GetBaseExpressionNode (SyntaxNode root, TextSpan sourceSpan)
                {
                        return (BaseExpressionSyntax) root.FindToken (sourceSpan.Start).Parent;
                }

                private static ThisExpressionSyntax
                RewriteBaseAsThis (BaseExpressionSyntax token)
                {
                                             ...
                                             CancellationToken cancellationToken)
                {
                        ...
                        SyntaxNode newSyntaxRoot
                            = root.ReplaceNode (node, RewriteBaseAsThis (node));
                ...
                private class FixAll : DocumentBasedFixAllProvider
                {
                        ...
                        protected override string
                            CodeActionTitle => ReadabilityResources.SA1100CodeFix;
                                               ...
                                               ImmutableArray<Diagnostic> diagnostics)
                        {
                                ...
                                List<BaseExpressionSyntax> nodesToReplace
                                    = new List<BaseExpressionSyntax> (diagnostics.Length);
                                foreach (var diagnostic in diagnostics)
                                        {
                                                nodesToReplace.Add (GetBaseExpressionNode (
                                                    syntaxRoot, diagnostic.Location.SourceSpan));
                                        }

                                return syntaxRoot.ReplaceNodes (
                                    nodesToReplace,
                                    (originalNode,
                                     rewrittenNode) => RewriteBaseAsThis (rewrittenNode));
                        }
                }
        }
}
