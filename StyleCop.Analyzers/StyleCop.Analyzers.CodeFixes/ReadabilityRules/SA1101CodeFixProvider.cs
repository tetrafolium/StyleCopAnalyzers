...
namespace StyleCop.Analyzers.ReadabilityRules
{
        ...
        /// </summary>
        /// <remarks>
        /// <para>To fix a violation of this rule, insert the <c>this.</c> prefix before the call to
        /// the class member.</para>
        /// </remarks>
        ...
        internal class SA1101CodeFixProvider : CodeFixProvider {
                ...
                private static Task<Document> GetTransformedDocumentAsync(
                    Document document, SyntaxNode root, SimpleNameSyntax node)
                {
                ...
                private class FixAll : DocumentBasedFixAllProvider {
                        ...
                        protected override string
                            CodeActionTitle => ReadabilityResources.SA1101CodeFix;
                            ...
                            ImmutableArray<Diagnostic> diagnostics)
                        {
                                ...
                                foreach (Diagnostic diagnostic in diagnostics) {
                                        if (!(syntaxRoot.FindNode(diagnostic.Location.SourceSpan,
                                                false, true) is SimpleNameSyntax node)
                                            || node.IsMissing) {
                                                continue;
                                        }
                                ...
                                return syntaxRoot.ReplaceNodes(nodesNeedingQualification,
                                    (originalNode,
                                        rewrittenNode) => SyntaxFactory
                                                              .MemberAccessExpression(
                                                                  SyntaxKind
                                                                      .SimpleMemberAccessExpression,
                                                                  ThisExpressionSyntax,
                                                                  (SimpleNameSyntax)
                                                                      rewrittenNode.WithoutTrivia()
                                                                          .WithoutFormatting())
