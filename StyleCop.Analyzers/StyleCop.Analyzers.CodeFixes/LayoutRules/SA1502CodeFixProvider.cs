...
namespace StyleCop.Analyzers.LayoutRules
{
        ...
        internal class SA1502CodeFixProvider : CodeFixProvider
        {
                ...
                private Document
                CreateCodeFix (Document document, IndentationSettings indentationSettings,
                               Diagnostic diagnostic, SyntaxNode syntaxRoot)
                {
                        ...
                        switch (node.Kind ())
                                ...
                                case SyntaxKind.AccessorList:
                                        newSyntaxRoot
                                            = this.RegisterPropertyLikeDeclarationCodeFix (
                                                syntaxRoot,
                ...
                private SyntaxNode
                ReformatElement (SyntaxNode syntaxRoot, SyntaxNode element,
                        ...
                        if (parentEndLine == blockStartLine)
                                {
                                        var newTrailingTrivia
                                            = parentLastToken.TrailingTrivia
                        ...
                        if (startOfContentToken != closeBraceToken)
                                {
                                        var newStartOfContentTokenLeadingTrivia
                                            = startOfContentToken.LeadingTrivia
                                                  .WithoutTrailingWhitespace ()
                                                  .Add (SyntaxFactory.Whitespace (
                                                      contentIndentationString));
                        ...
                        if (endOfContentToken != openBraceToken)
                                {
                                        var newEndOfContentTokenTrailingTrivia
                                            = endOfContentToken.TrailingTrivia
                                                  .WithoutTrailingWhitespace ()
                                                  .Add (SyntaxFactory.CarriageReturnLineFeed);

                                        // check if the token already exists (occurs when there is
                                        // only one token in the block)
                ...
                private SyntaxToken
                FormatBraceToken (SyntaxToken braceToken, string indentationString)
                {
                        ...
                        if ((newBraceTokenTrailingTrivia.Count == 0)
                            || !newBraceTokenTrailingTrivia.Last ().IsKind (
                                ...
                                {
                                        newBraceTokenTrailingTrivia
                                            = newBraceTokenTrailingTrivia.Add (
