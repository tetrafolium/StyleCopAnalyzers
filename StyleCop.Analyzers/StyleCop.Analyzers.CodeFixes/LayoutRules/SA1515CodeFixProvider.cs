...
public override Task RegisterCodeFixesAsync(CodeFixContext context)
{
	...
	{
		context.RegisterCodeFix(CodeAction.Create(LayoutResources.SA1515CodeFix,
	...
	foreach (var singleLineComment in commentTrivias)
	{
		...
		while (index >= 0)
		{
			...
			default:
				triviaList = triviaList.ReplaceRange(
					triviaList[index], new[] {triviaList[index], SyntaxFactory.CarriageReturnLineFeed});
		...
		if (index == -1)
		{
			triviaList = triviaList.ReplaceRange(triviaList[0],
			                                     new[] {SyntaxFactory.CarriageReturnLineFeed, triviaList[0]});
		}
	}
