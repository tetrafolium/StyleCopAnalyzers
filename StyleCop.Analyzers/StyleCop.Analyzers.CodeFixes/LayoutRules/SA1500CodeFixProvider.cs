...
namespace StyleCop.Analyzers.LayoutRules
{
        ...
        /// cref="SA1500BracesForMultiLineStatementsMustNotShareLine"/>.
        /// </summary>
        ...
        internal class SA1500CodeFixProvider : CodeFixProvider
        {
                        ...
                        foreach (var braceToken in braceTokens)
                                {
                                        ...
                                        var indentationSteps = DetermineIndentationSteps (
                                            indentationSettings, braceToken);

                                        var previousToken = braceToken.GetPreviousToken ();

                                        if (IsAccessorWithSingleLineBlock (previousToken,
                                                                           braceToken))
                                                {
                                                ...
                                                {
                                                        // Check if we need to apply a fix before
                                                        // the brace
                                                        if (LocationHelpers
                                                                .GetLineSpan (previousToken)
                                                                .StartLinePosition.Line
                                                            == braceLine)
                                                                {
                                                                        if (!braceTokens.Contains (
                                                                                previousToken))
                                                                                {
                                                                                        var sharedTrivia
                                                                                            = braceReplacementToken
                                                        ...
                                                        // Check if we need to apply a fix after the
                                                        // brace. No fix is needed when:
                                                        // - The closing brace is followed by a
                                                        // semi-colon or closing paren
                                                        // - The closing brace is the last token in
                                                        // the file
                ...
                private static bool
                IsAccessorWithSingleLineBlock (SyntaxToken previousToken, SyntaxToken braceToken)
                {
                ...
                private static bool
                IsValidFollowingToken (SyntaxToken nextToken)
                {
                ...
                private static bool
                ContainsStartOfLine (SyntaxToken token, int startLine)
                {
                ...
                GetTokenStartLinePosition (SyntaxToken token)
                {
                                ...
                                SyntaxToken originalToken, SyntaxToken replacementToken)
                {
                        if (tokenReplacements.ContainsKey (originalToken))
                                {
                                        // This will only happen when a single keyword (like else)
                                        // has invalid brace tokens before and after it.
                                        tokenReplacements[originalToken]
                                            = tokenReplacements [originalToken]
                                                  .WithTrailingTrivia (
                                                      replacementToken.TrailingTrivia);
                                }
                        else
                                {
