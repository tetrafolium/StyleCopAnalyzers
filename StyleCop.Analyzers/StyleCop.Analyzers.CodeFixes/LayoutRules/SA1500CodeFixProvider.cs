...
public override Task RegisterCodeFixesAsync(CodeFixContext context)
{
	...
	{
		context.RegisterCodeFix(CodeAction.Create(LayoutResources.SA1500CodeFix,
                                                                ...
                                                                CancellationToken cancellationToken)
{
	...
	var newSyntaxRoot = syntaxRoot.ReplaceTokens(tokenReplacements.Keys, (originalToken, rewrittenToken) =>
	                                             tokenReplacements[originalToken]);
	return document.WithSyntaxRoot(newSyntaxRoot);
}

private static Dictionary<SyntaxToken, SyntaxToken> GenerateBraceFixes(IndentationSettings indentationSettings,
	...
	foreach (var braceToken in braceTokens)
	{
		...
		if (IsAccessorWithSingleLineBlock(previousToken, braceToken))
		{
			var newTrailingTrivia =
				previousToken.TrailingTrivia.WithoutTrailingWhitespace().Add(SyntaxFactory.Space);

			AddReplacement(tokenReplacements, previousToken,
			               previousToken.WithTrailingTrivia(newTrailingTrivia));

			braceReplacementToken =
				braceReplacementToken.WithLeadingTrivia(braceToken.LeadingTrivia.WithoutLeadingWhitespace());
		}
		else
		{
			...
			if (LocationHelpers.GetLineSpan(previousToken).StartLinePosition.Line == braceLine)
			{
				if (!braceTokens.Contains(previousToken))
				{
					...
					AddReplacement(tokenReplacements, previousToken,
					               previousToken.WithTrailingTrivia(previousTokenNewTrailingTrivia));
				}

				braceReplacementToken = braceReplacementToken.WithLeadingTrivia(
					IndentationHelper.GenerateWhitespaceTrivia(indentationSettings, indentationSteps));
			}
			...
			var nextTokenLine = nextToken.IsKind(SyntaxKind.None)
			                    ? -1
			                    : LocationHelpers.GetLineSpan(nextToken).StartLinePosition.Line;
			var isMultiDimensionArrayInitializer =
				braceToken.IsKind(SyntaxKind.OpenBraceToken) &&
				braceToken.Parent.IsKind(SyntaxKind.ArrayInitializerExpression) &&
				braceToken.Parent.Parent.IsKind(SyntaxKind.ArrayInitializerExpression);
			    ...
			    !isMultiDimensionArrayInitializer)
			{
				...
				var newTrailingTrivia = braceReplacementToken.TrailingTrivia.WithoutTrailingWhitespace()
				                        .AddRange(sharedTrivia)
				                        .Add(SyntaxFactory.CarriageReturnLineFeed);
				...
				if (!braceTokens.Contains(nextToken))
				{
					...
					AddReplacement(tokenReplacements, nextToken,
					               nextToken.WithLeadingTrivia(IndentationHelper.GenerateWhitespaceTrivia(
											   indentationSettings, newIndentationSteps)));
				}
...
private static void AddReplacement(Dictionary<SyntaxToken, SyntaxToken> tokenReplacements,
                                   SyntaxToken originalToken, SyntaxToken replacementToken)
{
	if (tokenReplacements.ContainsKey(originalToken))
	{
		...
		tokenReplacements[originalToken] = tokenReplacements [originalToken]
