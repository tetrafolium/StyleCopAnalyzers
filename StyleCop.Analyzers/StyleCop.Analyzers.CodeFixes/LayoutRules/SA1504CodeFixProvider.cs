...
namespace StyleCop.Analyzers.LayoutRules {
        ...
        internal class SA1504CodeFixProvider : CodeFixProvider {
                ...
                public override async Task RegisterCodeFixesAsync(CodeFixContext context) {
                        ...
                        foreach (var diagnostic in context.Diagnostics) {
                                ...
                                foreach (var accessor in accessorList.Accessors) {
                                        // only offer single line fix for accessors without
                                        // attributes
                                        ...
                                        // only offer single line fix for accessors with at most a
                                        // single statement
                                        ...
                                        if (accessor.Body.Statements.Count == 1) {
                                                // only offer single line fix when the statement is
                                                // on a single line
                                        ...
                                        // only offer single line fix if there are no interfering
                                        // trivia
                                ...
                                if (canOfferSingleLineFix) {
                                        context.RegisterCodeFix(
                                            CodeAction.Create(
                                                ...
                                                cancellationToken =>
                                                    GetTransformedDocumentForSingleLineAsync(
                                ...
                                context.RegisterCodeFix(
                                    CodeAction.Create(
                                        ...
                                        cancellationToken =>
                                            GetTransformedDocumentForMutipleLinesAsync(
                ...
                GetTransformedDocumentForSingleLineAsync(Document document, Diagnostic diagnostic,
                        ...
                        var newSyntaxRoot =
                            syntaxRoot.ReplaceNodes(replacements.Keys, (original, maybeRewritten) =>
                ...
                private static BlockSyntax ReformatBodyAsSingleLine(BlockSyntax body) {
                        ...
                        if (body.Statements.Count > 0) {
                                ...
                                newStatements =
                                    SyntaxFactory.List<StatementSyntax>().Add(reformattedStatement);
                        } else {
                ...
                private static async Task<Document>
                GetTransformedDocumentForMutipleLinesAsync(Document document, Diagnostic diagnostic,
                                                           CancellationToken cancellationToken) {
                        var syntaxRoot = await document.GetSyntaxRootAsync(cancellationToken)
                                             .ConfigureAwait(false);
                        var settings = SettingsHelper.GetStyleCopSettings(
                        ...
                        foreach (var accessor in accessorList.Accessors) {
                                var reformattedAccessor =
                                    ReformatAccessorAsMultipleLines(settings.Indentation, accessor);
                                if (accessor != accessorList.Accessors.Last()) {
                                        // insert an empty line between accessors
                                        reformattedAccessor =
                                            reformattedAccessor.WithTrailingTrivia(
                                                reformattedAccessor.GetTrailingTrivia().Add(
                ...
                ReformatAccessorAsMultipleLines(IndentationSettings indentationSettings,
                        ...
                        var newAccessor =
                            accessor
                                .WithModifiers(ReformatModifiersAsMultipleLines(accessor.Modifiers,
                                                                                indentation))
                                .WithKeyword(ReformatKeywordAsMultipleLines(
                                    accessor.Keyword, indentation, accessor.Modifiers.Count == 0))
                                .WithBody(ReformatBodyAsMultipleLines(accessor.Body, indentation,
                ...
                ReformatModifiersAsMultipleLines(SyntaxTokenList modifiers,
                        ...
                        reformattedModifiers[0] =
                            reformattedModifiers [0]
                                .WithLeadingTrivia(
                                    reformattedModifiers[0].LeadingTrivia.Insert(0, indentation));

                        return SyntaxFactory.TokenList(reformattedModifiers);
                }

                private static SyntaxToken ReformatKeywordAsMultipleLines(SyntaxToken keyword,
                                                                          SyntaxTrivia indentation,
                ...
                ReformatBodyAsMultipleLines(BlockSyntax body, SyntaxTrivia indentation,
                        ...
                        if (body.Statements.Count > 0) {
                                ...
                                newStatements =
                                    SyntaxFactory.List<StatementSyntax>().AddRange(statements);
                                ...
                                var triviaBetweenBraces = TriviaHelper.MergeTriviaLists(
                                    ...
                                    body.CloseBraceToken.LeadingTrivia);
                                reformattedOpenBraceTrailingTrivia =
                                    ReformatTriviaListNoTrailingSpace(triviaBetweenBraces);
                                newStatements = body.Statements;
