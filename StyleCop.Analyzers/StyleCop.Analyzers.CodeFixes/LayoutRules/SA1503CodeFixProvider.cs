...
namespace StyleCop.Analyzers.LayoutRules
{
        ...
        internal class SA1503CodeFixProvider : CodeFixProvider
        {
                                        ...
                                        context.RegisterCodeFix (
                                            CodeAction.Create (
                                                ...
                                                cancellationToken => GetTransformedDocumentAsync (
                                                    context.Document, syntaxRoot, node,
                                                    cancellationToken),
                ...
                private static bool
                ContainsConditionalDirectiveTrivia (SyntaxNode node)
                {
                                ...
                                foreach (Diagnostic diagnostic in diagnostics)
                                        {
                                                if (!(syntaxRoot.FindNode (
                                                        diagnostic.Location.SourceSpan, false, true)
                                                          is StatementSyntax node)
                                                    || node.IsMissing)
                                                        {
                                                                continue;
                                                ...
                                                // If the parent of the statement contains a
                                                // conditional directive, stuff will be really hard
                                                // to fix correctly, so don't offer a code fix.
                                                if (ContainsConditionalDirectiveTrivia (
                                                        node.Parent))
                                                        {
                                                                continue;
                                                        }
