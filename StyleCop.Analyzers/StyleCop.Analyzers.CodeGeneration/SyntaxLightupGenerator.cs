...
private void GenerateSyntaxWrapper(in GeneratorExecutionContext context, SyntaxData syntaxData,
                                   NodeData nodeData)
{
				      ...
				      : SyntaxFactory.VariableDeclaration(
					      ...
					      : SyntaxFactory.SingletonSeparatedList(SyntaxFactory.VariableDeclarator(
											     ...
											     : SyntaxFactory.EqualsValueClause(SyntaxFactory.LiteralExpression(
																       ...
																       SyntaxFactory.Literal("Microsoft.CodeAnalysis.CSharp.Syntax." + nodeData.Name))))))));
	...
	foreach (var field in nodeData.Fields)
	{
		...
		FieldDeclarationSyntax fieldAccessor = SyntaxFactory.FieldDeclaration(
			...
			: SyntaxFactory.VariableDeclaration(
				...
				variables
				: SyntaxFactory.SingletonSeparatedList(SyntaxFactory.VariableDeclarator(field.AccessorName))));
		...
		members = members.Add(SyntaxFactory.FieldDeclaration(
					...
					SyntaxFactory.IdentifierName(concreteBase),
						      ...
						      : SyntaxFactory.SingletonSeparatedList(
							      SyntaxFactory.VariableDeclarator(field.WithAccessorName)))));
	}
	...
	members = members.Add(
		...
		.FieldDeclaration(
			...
			: SyntaxFactory.VariableDeclaration(
				...
				: SyntaxFactory.SingletonSeparatedList(SyntaxFactory.VariableDeclarator("node"))))
	...
	foreach (var field in nodeData.Fields)
	{
		...
		if (field.IsWrappedSeparatedSyntaxList(syntaxData, out var elementNode))
		{
			...
			helperName = SyntaxFactory.GenericName(
				identifier
				: SyntaxFactory.Identifier("CreateSeparatedSyntaxListPropertyAccessor"), typeArgumentList
		...
		else
		{
			...
			helperName = SyntaxFactory.GenericName(
				identifier
				: SyntaxFactory.Identifier("CreateSyntaxPropertyAccessor"), typeArgumentList
				: SyntaxFactory.TypeArgumentList(SyntaxFactory.SeparatedList(new[] {
						SyntaxFactory.IdentifierName(concreteBase),
						SyntaxFactory.ParseTypeName(field.GetAccessorResultType(syntaxData)),
					})));
		...
		// ReturnTypeAccessor = LightupHelpers.CreateSyntaxPropertyAccessor<StatementSyntax,
		...
		staticCtorStatements =
			staticCtorStatements.Add(SyntaxFactory.ExpressionStatement(SyntaxFactory.AssignmentExpression(
											   SyntaxKind.SimpleAssignmentExpression, left
											   : SyntaxFactory.IdentifierName(field.AccessorName), right
											   : SyntaxFactory.InvocationExpression(
												   expression
												   : SyntaxFactory.MemberAccessExpression(SyntaxKind.SimpleMemberAccessExpression, expression
	...
	foreach (var field in nodeData.Fields)
	{
		...
		if (field.IsWrappedSeparatedSyntaxList(syntaxData, out var elementNode))
		{
			...
			helperName = SyntaxFactory.GenericName(
				identifier
				: SyntaxFactory.Identifier("CreateSeparatedSyntaxListWithPropertyAccessor"), typeArgumentList
				: SyntaxFactory.TypeArgumentList(SyntaxFactory.SeparatedList<TypeSyntax>(new[] {
						SyntaxFactory.IdentifierName(concreteBase),
						SyntaxFactory.IdentifierName(elementNode.WrapperName),
					})));
		...
		else
		{
			...
			helperName = SyntaxFactory.GenericName(
				identifier
				: SyntaxFactory.Identifier("CreateSyntaxWithPropertyAccessor"), typeArgumentList
				: SyntaxFactory.TypeArgumentList(SyntaxFactory.SeparatedList(new[] {
						SyntaxFactory.IdentifierName(concreteBase),
						SyntaxFactory.ParseTypeName(field.GetAccessorResultType(syntaxData)),
					})));
		...
		// WithReturnTypeAccessor = LightupHelpers.CreateSyntaxWithPropertyAccessor<StatementSyntax,
		// TypeSyntax>(WrappedType, nameof(ReturnType));
		staticCtorStatements =
			staticCtorStatements.Add(SyntaxFactory.ExpressionStatement(SyntaxFactory.AssignmentExpression(
											   SyntaxKind.SimpleAssignmentExpression, left
											   : SyntaxFactory.IdentifierName(field.WithAccessorName), right
											   : SyntaxFactory.InvocationExpression(
												   expression
												   : SyntaxFactory.MemberAccessExpression(SyntaxKind.SimpleMemberAccessExpression, expression
												                                          : SyntaxFactory.IdentifierName("LightupHelpers"),
	...
	foreach (var field in nodeData.Fields)
	{
		...
		if (field.IsOverride)
		{
			...
			else
			{
				...
				{
					// this.SyntaxNode.OpenParenToken()
					returnExpression = SyntaxFactory.InvocationExpression(expression
		...
		else if (field.IsWrappedSeparatedSyntaxList(syntaxData, out var elementNode))
		{
			...
			returnExpression = SyntaxFactory.InvocationExpression(
				expression
				: SyntaxFactory.IdentifierName(field.AccessorName), argumentList
				: SyntaxFactory.ArgumentList(SyntaxFactory.SingletonSeparatedList(SyntaxFactory.Argument(
													  SyntaxFactory.MemberAccessExpression(SyntaxKind.SimpleMemberAccessExpression, expression
													                                       : SyntaxFactory.ThisExpression(), name
													                                       : SyntaxFactory.IdentifierName("SyntaxNode"))))));
		...
		else if (syntaxData.TryGetNode(field.Type) is {} fieldNodeType)
		{
			...
			if (fieldNodeType.WrapperName is not null)
			{
				...
				returnExpression = SyntaxFactory.CastExpression(
					type
					: SyntaxFactory.IdentifierName(fieldNodeType.WrapperName), expression
					: returnExpression);
			}
		}
		else
		...
		PropertyDeclarationSyntax property = SyntaxFactory.PropertyDeclaration(
			...
			: SyntaxFactory.AccessorList(SyntaxFactory.SingletonList(SyntaxFactory.AccessorDeclaration(
	...
	for (var baseNode = syntaxData.TryGetNode(nodeData.BaseName); baseNode?.WrapperName is not null;
		...
		ConversionOperatorDeclarationSyntax wrapperConversion = SyntaxFactory.ConversionOperatorDeclaration(
			...
			: SyntaxFactory.ParameterList(SyntaxFactory.SingletonSeparatedList(
							      SyntaxFactory.Parameter(attributeLists
							                              : default, modifiers
							                              : default, type
							                              : SyntaxFactory.IdentifierName(baseNode.WrapperName), identifier
							                              : SyntaxFactory.Identifier("node"), @default
							                              : null))),
			body
			: SyntaxFactory.Block(SyntaxFactory.ReturnStatement(SyntaxFactory.CastExpression(
										    type
										    : SyntaxFactory.IdentifierName(nodeData.WrapperName), expression
										    : SyntaxFactory.MemberAccessExpression(SyntaxKind.SimpleMemberAccessExpression, expression
	...
	//     if (!IsInstance(node))
	//     {
	//         throw new InvalidCastException($"Cannot cast '{node.GetType().FullName}' to
	//         '{WrappedTypeName}'");
	//     }
	//
	...
	var nodeConversion = SyntaxFactory.ConversionOperatorDeclaration(
		...
		: SyntaxFactory.Block(
			...
			SyntaxFactory.ReturnStatement(SyntaxFactory.ObjectCreationExpression(
							      type
							      : SyntaxFactory.IdentifierName(nodeData.WrapperName), argumentList
							      : SyntaxFactory.ArgumentList(SyntaxFactory.SingletonSeparatedList(SyntaxFactory.Argument(
																	SyntaxFactory.CastExpression(type
																	                             : SyntaxFactory.IdentifierName(concreteBase), expression
																	                             : SyntaxFactory.IdentifierName("node"))))),
							      initializer
							      : null))),
		expressionBody
	     ...
	     baseNode = syntaxData.TryGetNode(baseNode.BaseName))
	{
		...
		members = members.Add(SyntaxFactory.ConversionOperatorDeclaration(
					      ...
					      : SyntaxFactory.Token(SyntaxKind.OperatorKeyword), type
					      : SyntaxFactory.IdentifierName(baseNode.WrapperName), parameterList
					      : SyntaxFactory.ParameterList(SyntaxFactory.SingletonSeparatedList(
										    SyntaxFactory.Parameter(attributeLists
										                            : default, modifiers
										                            : default, type
										                            : SyntaxFactory.IdentifierName(nodeData.WrapperName), identifier
										                            : SyntaxFactory.Identifier("wrapper"), @default
										                            : null))),
					      body
					      : SyntaxFactory.Block(SyntaxFactory.ReturnStatement(SyntaxFactory.InvocationExpression(
													  expression
													  : SyntaxFactory.MemberAccessExpression(SyntaxKind.SimpleMemberAccessExpression, expression
	...
	foreach (var field in nodeData.Fields)
	{
		...
		ExpressionSyntax returnExpression = SyntaxFactory.InvocationExpression(
			expression
			: SyntaxFactory.IdentifierName(field.WithAccessorName), argumentList
			: SyntaxFactory.ArgumentList(SyntaxFactory.SeparatedList(new[] {
		...
		members = members.Add(SyntaxFactory.MethodDeclaration(
					      attributeLists
					      : default, modifiers
					      : SyntaxFactory.TokenList(SyntaxFactory.Token(SyntaxKind.PublicKeyword)), returnType
					      : SyntaxFactory.IdentifierName(nodeData.WrapperName), explicitInterfaceSpecifier
					      : null, identifier
					      : SyntaxFactory.Identifier("With" + field.Name), typeParameterList
					      : null, parameterList
					      : SyntaxFactory.ParameterList(SyntaxFactory.SingletonSeparatedList(
										    SyntaxFactory.Parameter(attributeLists
										                            : default, modifiers
					      ...
					      : SyntaxFactory.Block(SyntaxFactory.ReturnStatement(SyntaxFactory.ObjectCreationExpression(
													  ...
													  : SyntaxFactory.IdentifierName(nodeData.WrapperName), argumentList
													  : SyntaxFactory.ArgumentList(
														  SyntaxFactory.SingletonSeparatedList(SyntaxFactory.Argument(returnExpression))),
													  initializer
													  : null))),
					      expressionBody
					      : null, semicolonToken
	...
	var wrapperStruct = SyntaxFactory.StructDeclaration(
		...
		: SyntaxFactory.BaseList(SyntaxFactory.SingletonSeparatedList<BaseTypeSyntax>(
						 SyntaxFactory.SimpleBaseType(SyntaxFactory.GenericName(
										      identifier
										      : SyntaxFactory.Identifier("ISyntaxWrapper"), typeArgumentList
	...
	var wrapperNamespace = SyntaxFactory.NamespaceDeclaration(
		...
		.Add(SyntaxFactory.UsingDirective(
			     SyntaxFactory.ParseName("Microsoft.CodeAnalysis.CSharp.Syntax"))),
		members
                                         ...
                                         ImmutableArray<NodeData> wrapperTypes)
{
	...
	var staticCtorStatements =
		...
		.Add(SyntaxFactory.LocalDeclarationStatement(SyntaxFactory.VariableDeclaration(
								     ...
								     : SyntaxFactory.SingletonSeparatedList(SyntaxFactory.VariableDeclarator(
														    identifier
														    : SyntaxFactory.Identifier("csharpCodeAnalysisAssembly"), argumentList
														    : null, initializer
														    : SyntaxFactory.EqualsValueClause(SyntaxFactory.MemberAccessExpression(
																			      ...
																			      : SyntaxFactory.InvocationExpression(SyntaxFactory.MemberAccessExpression(
																									   ...
																									   : SyntaxFactory.TypeOfExpression(
																										   SyntaxFactory.IdentifierName("CSharpSyntaxNode")),
																									   name
	...
	foreach (var node in wrapperTypes.OrderBy(node => node.Name, StringComparer.OrdinalIgnoreCase))
	{
		...
		if (node.Name == nameof(CommonForEachStatementSyntax))
		{
			...
			// the CommonForEachStatementSyntax type isn't found at runtime, we fall back to using this type
			// instead.
			//
			...
			staticCtorStatements = staticCtorStatements.Add(
				SyntaxFactory.LocalDeclarationStatement(SyntaxFactory.VariableDeclaration(
										...
										: SyntaxFactory.SingletonSeparatedList(SyntaxFactory.VariableDeclarator(
															       identifier
															       : SyntaxFactory.Identifier("forEachStatementSyntaxType"), argumentList
															       : null, initializer
															       : SyntaxFactory.EqualsValueClause(SyntaxFactory.BinaryExpression(
																					 ...
																					 : SyntaxFactory.InvocationExpression(
																						 ...
																						 : SyntaxFactory.MemberAccessExpression(
																							 SyntaxKind.SimpleMemberAccessExpression, expression
																							 : SyntaxFactory.IdentifierName("csharpCodeAnalysisAssembly"), name
																							 : SyntaxFactory.IdentifierName("GetType")),
																						 argumentList
																						 : SyntaxFactory.ArgumentList(SyntaxFactory.SingletonSeparatedList(
																					 ...
																					 : SyntaxFactory.InvocationExpression(
																						 ...
																						 : SyntaxFactory.ArgumentList(SyntaxFactory.SingletonSeparatedList(
																										      SyntaxFactory.Argument(SyntaxFactory.MemberAccessExpression(
																														     SyntaxKind.SimpleMemberAccessExpression, expression
																														     : SyntaxFactory.IdentifierName(node.WrapperName), name
																														     : SyntaxFactory.IdentifierName("FallbackWrappedTypeName")))))))))))));
...
public SyntaxData(in GeneratorExecutionContext context, XDocument document)
{
	...
	foreach (var element in document.XPathSelectElement("/Tree[@Root='SyntaxNode']")
	         .XPathSelectElements("PredefinedNode|AbstractNode|Node"))
	{
...
public NodeData(in GeneratorExecutionContext context, XElement element)
{
	...
	this.ExistingType =
		context.Compilation.GetTypeByMetadataName($"Microsoft.CodeAnalysis.CSharp.Syntax.{this.Name}")
	...
	this.Fields = element.XPathSelectElements("descendant::Field")
	              .Select(field => new FieldData(this, field))
	              .ToImmutableArray();
}
...
public bool IsWrappedSeparatedSyntaxList(SyntaxData syntaxData, [ NotNullWhen(true) ] out NodeData? element)
{
	if (this.Type.StartsWith("SeparatedSyntaxList<") && this.Type.EndsWith(">"))
	{
		var elementTypeName = this.Type.Substring(
			"SeparatedSyntaxList<".Length, this.Type.Length - "SeparatedSyntaxList<".Length - ">".Length);
