...
private void GenerateOperationInterface(in GeneratorExecutionContext context, InterfaceData node)
{
	...
	members = members.Add(SyntaxFactory.FieldDeclaration(
				      ...
				      : default,
				      modifiers
				      : SyntaxFactory.TokenList(SyntaxFactory.Token(SyntaxKind.InternalKeyword), SyntaxFactory.Token(SyntaxKind.ConstKeyword)),
				      declaration
				      : SyntaxFactory.VariableDeclaration(
					      ...
					      : SyntaxFactory.SingletonSeparatedList(SyntaxFactory.VariableDeclarator(
											     identifier
											     : SyntaxFactory.Identifier("WrappedTypeName"),
											     argumentList
											     : null,
											     initializer
											     : SyntaxFactory.EqualsValueClause(SyntaxFactory.LiteralExpression(SyntaxKind.StringLiteralExpression, SyntaxFactory.Literal("Microsoft.CodeAnalysis.Operations." + node.InterfaceName))))))));
	...
	foreach (var property in node.Properties) {
		...
		members = members.Add(SyntaxFactory.FieldDeclaration(
					...
					property.AccessorResultType,
						      ...
						      variables
						      : SyntaxFactory.SingletonSeparatedList(SyntaxFactory.VariableDeclarator(property.AccessorName)))));
	}
	...
	foreach (var property in node.Properties) {
		...
		if (property.IsDerivedOperationArray) {
			...
			helperName = SyntaxFactory.GenericName(
				identifier
				: SyntaxFactory.Identifier("CreateOperationListPropertyAccessor"),
				typeArgumentList
				: SyntaxFactory.TypeArgumentList(SyntaxFactory.SingletonSeparatedList<TypeSyntax>(SyntaxFactory.IdentifierName("IOperation"))));
		} else {
			// CreateOperationPropertyAccessor<IOperation, IMethodSymbol>
			helperName = SyntaxFactory.GenericName(
				identifier
				: SyntaxFactory.Identifier("CreateOperationPropertyAccessor"),
				typeArgumentList
				: SyntaxFactory.TypeArgumentList(SyntaxFactory.SeparatedList(
									 new[] {
						SyntaxFactory.IdentifierName("IOperation"),
		...
		staticCtorStatements = staticCtorStatements.Add(SyntaxFactory.ExpressionStatement(SyntaxFactory.AssignmentExpression(
					...
					SyntaxFactory.Argument(SyntaxFactory.InvocationExpression(
								       ...
								       argumentList
								       : SyntaxFactory.ArgumentList(SyntaxFactory.SingletonSeparatedList(SyntaxFactory.Argument(SyntaxFactory.IdentifierName(property.Name)))))),
				}))))));
	}
	...
	members = members.Add(SyntaxFactory.ConstructorDeclaration(
				      ...
				      identifier
				      : SyntaxFactory.Identifier(node.WrapperName),
				      parameterList
				      : SyntaxFactory.ParameterList(),
				      initializer
				      : null,
				      body
				      : SyntaxFactory.Block(staticCtorStatements),
				      expressionBody
				      : null));
	...
	members = members.Add(SyntaxFactory.PropertyDeclaration(
				      ...
				      : null,
				      identifier
				      : SyntaxFactory.Identifier("WrappedOperation"),
				      accessorList
				      : null,
				      expressionBody
	...
	foreach (var property in node.Properties) {
		if (property.IsSkipped) {
			...
			if (property.IsPublicProperty) {
				...
				members = members.Add(SyntaxFactory.PropertyDeclaration(
							      ...
							      : null,
							      expressionBody
							      : SyntaxFactory.ArrowExpressionClause(SyntaxFactory.ThrowExpression(SyntaxFactory.ObjectCreationExpression(
																	  type
		...
		var propertyType = property.NeedsWrapper ? SyntaxFactory.IdentifierName(property.Type + "Wrapper")
		    : property.AccessorResultType;
		...
		if (property.NeedsWrapper) {
			...
			convertedResult = SyntaxFactory.InvocationExpression(
				...
				argumentList
				: SyntaxFactory.ArgumentList(SyntaxFactory.SingletonSeparatedList(SyntaxFactory.Argument(evaluatedAccessor))));
		} else {
					      ...
					      : SyntaxFactory.ArrowExpressionClause(SyntaxFactory.InvocationExpression(
											    ...
											    : SyntaxFactory.ArgumentList(SyntaxFactory.SingletonSeparatedList(SyntaxFactory.Argument(
																				      SyntaxFactory.MemberAccessExpression(
																					      SyntaxKind.SimpleMemberAccessExpression,
																					      expression
																					      : SyntaxFactory.IdentifierName("wrapper"),
																					      name
																					      : SyntaxFactory.IdentifierName("WrappedOperation"))))))),
					      semicolonToken
					      : SyntaxFactory.Token(SyntaxKind.SemicolonToken)));
	...
	members = members.Add(SyntaxFactory.MethodDeclaration(
				      ...
				      : null,
				      identifier
				      : SyntaxFactory.Identifier("FromOperation"),
				      typeParameterList
				      : null,
				      parameterList
				      : SyntaxFactory.ParameterList(SyntaxFactory.SingletonSeparatedList(SyntaxFactory.Parameter(
					      ...
					      SyntaxFactory.IfStatement(
						      ...
						      : SyntaxFactory.PrefixUnaryExpression(
							      ...
							      : SyntaxFactory.InvocationExpression(
								      ...
								      argumentList
								      : SyntaxFactory.ArgumentList(SyntaxFactory.SingletonSeparatedList(SyntaxFactory.Argument(SyntaxFactory.IdentifierName("operation")))))),
						      statement
						      : SyntaxFactory.Block(
	...
	members = members.Add(SyntaxFactory.MethodDeclaration(
				      ...
				      : null,
				      identifier
				      : SyntaxFactory.Identifier("IsInstance"),
				      typeParameterList
				      : null,
				      parameterList
				      : SyntaxFactory.ParameterList(SyntaxFactory.SingletonSeparatedList(SyntaxFactory.Parameter(
	...
	var wrapperStruct = SyntaxFactory.StructDeclaration(
		...
		: default,
		modifiers
		: SyntaxTokenList.Create(SyntaxFactory.Token(SyntaxKind.InternalKeyword)).Add(SyntaxFactory.Token(SyntaxKind.ReadOnlyKeyword)),
		identifier
		: SyntaxFactory.Identifier(node.WrapperName),
		typeParameterList
		: null,
		baseList
		: SyntaxFactory.BaseList(SyntaxFactory.SingletonSeparatedList<BaseTypeSyntax>(SyntaxFactory.SimpleBaseType(SyntaxFactory.IdentifierName("IOperationWrapper")))),
		constraintClauses
		: default,
		members
		: members);
	var wrapperNamespace = SyntaxFactory.NamespaceDeclaration(
		name
		: SyntaxFactory.ParseName("StyleCop.Analyzers.Lightup"),
		externs
		: default,
		usings
...
private void GenerateOperationWrapperHelper(in GeneratorExecutionContext context, ImmutableArray<InterfaceData> wrapperTypes)
{
	...
	var wrappedTypes = SyntaxFactory.FieldDeclaration(
		...
		: SyntaxFactory.VariableDeclaration(
			...
			variables
			: SyntaxFactory.SingletonSeparatedList(SyntaxFactory.VariableDeclarator(SyntaxFactory.Identifier("WrappedTypes")))));
																	       ...
																	       SyntaxFactory.MemberAccessExpression(
																		       ...
																		       : SyntaxFactory.InvocationExpression(
																			       SyntaxFactory.MemberAccessExpression(
																				       SyntaxKind.SimpleMemberAccessExpression,
																				       expression
																				       : SyntaxFactory.TypeOfExpression(SyntaxFactory.IdentifierName("SyntaxNode")),
																				       name
					...
					SyntaxFactory.Argument(
						SyntaxFactory.InvocationExpression(
							...
							: SyntaxFactory.MemberAccessExpression(
								SyntaxKind.SimpleMemberAccessExpression,
								expression
								: SyntaxFactory.IdentifierName("codeAnalysisAssembly"),
								name
								: SyntaxFactory.IdentifierName("GetType")),
							argumentList
							: SyntaxFactory.ArgumentList(SyntaxFactory.SingletonSeparatedList(SyntaxFactory.Argument(
	...
	var staticCtor = SyntaxFactory.ConstructorDeclaration(
		...
		: SyntaxFactory.TokenList(SyntaxFactory.Token(SyntaxKind.StaticKeyword)),
		identifier
		: SyntaxFactory.Identifier("OperationWrapperHelper"),
		parameterList
		: SyntaxFactory.ParameterList(),
		initializer
		: null,
	...
	var getWrappedType = SyntaxFactory.MethodDeclaration(
		...
		: null,
		identifier
		: SyntaxFactory.Identifier("GetWrappedType"),
		typeParameterList
		: null,
		parameterList
		: SyntaxFactory.ParameterList(SyntaxFactory.SingletonSeparatedList(SyntaxFactory.Parameter(
											   ...
											   : SyntaxFactory.IdentifierName("Type"),
											   identifier
											   : SyntaxFactory.Identifier("wrapperType"),
											   @default
											   : null))),
		constraintClauses
		: default,
			...
			SyntaxFactory.IfStatement(
				...
				SyntaxFactory.Argument(
					...
					: SyntaxFactory.DeclarationExpression(
						...
						: SyntaxFactory.IdentifierName("Type"),
						designation
						: SyntaxFactory.SingleVariableDesignation(SyntaxFactory.Identifier("wrappedType")))),
			}))),
				statement
				: SyntaxFactory.Block(
	...
	var wrapperHelperClass = SyntaxFactory.ClassDeclaration(
		...
		: default,
		modifiers
		: SyntaxTokenList.Create(SyntaxFactory.Token(SyntaxKind.InternalKeyword)).Add(SyntaxFactory.Token(SyntaxKind.StaticKeyword)),
		identifier
		: SyntaxFactory.Identifier("OperationWrapperHelper"),
		typeParameterList
		: null,
...
private void GenerateOperationKindEx(in GeneratorExecutionContext context, ImmutableArray<InterfaceData> wrapperTypes)
{
					      ...
					      : SyntaxFactory.VariableDeclaration(
						      ...
						      : SyntaxFactory.SingletonSeparatedList(SyntaxFactory.VariableDeclarator(
												     ...
												     : SyntaxFactory.EqualsValueClause(SyntaxFactory.CastExpression(
																	       ...
																	       : SyntaxFactory.IdentifierName("OperationKind"),
																	       expression
																	       : SyntaxFactory.LiteralExpression(SyntaxKind.NumericLiteralExpression, SyntaxFactory.Literal($"0x{operationKind.value:x}", operationKind.value)))))))));
	}
	...
	var operationKindExClass = SyntaxFactory.ClassDeclaration(
		...
		: SyntaxTokenList.Create(SyntaxFactory.Token(SyntaxKind.InternalKeyword)).Add(SyntaxFactory.Token(SyntaxKind.StaticKeyword)),
		identifier
		: SyntaxFactory.Identifier("OperationKindEx"),
		typeParameterList
		: null,
		baseList
		: null,
