...
internal class CodeFixEquivalenceGroup {
        ...
        internal ImmutableDictionary<ProjectId, ImmutableArray<Diagnostic>>
            ...
            CancellationToken cancellationToken) {
                        ...
                        foreach (var diagnostic in projectDiagnostics.Value) {
                                ...
                                if (diagnostic.Location.IsInSource) {
                                        ...
                                        if (!projectDocumentDiagnostics.TryGetValue(
                                                ...
                                                projectDocumentDiagnostics.Add(sourcePath,
                                                                               diagnosticsInFile);
                                        }
                ...
                ImmutableDictionary<ProjectId,
                    ...
                    documentDiagnosticsToFix = relevantDocumentDiagnostics.ToImmutableDictionary(
                        i => i.Key, i => i.Value.ToImmutableDictionary(
                                        j => j.Key, j => j.Value.ToImmutableArray(),
                ...
                foreach (var equivalenceKey in equivalenceKeys) {
                        groups.Add(new CodeFixEquivalenceGroup(
                            equivalenceKey, solution, fixAllProvider, codeFixProvider,
        ...
        internal async Task<ImmutableArray<CodeActionOperation>> GetOperationsAsync(
            CancellationToken cancellationToken) {
                ...
                var context =
                    new FixAllContext(document, this.CodeFixProvider, FixAllScope.Solution,
                                      this.CodeFixEquivalenceKey, diagnosticIds,
