...
/// <summary>
/// StyleCopTester is a tool that will analyze a solution, find diagnostics in it and will print out
...
internal static class Program
{
        private static void Main (string[] args)
        {
                ...
                // Since Console apps do not have a SynchronizationContext, we're leveraging the
                // built-in support in WPF to pump the messages via the Dispatcher. See the
        ...
        MainAsync (string[] args, CancellationToken cancellationToken)
        {
                ...
                if (args.Length < 1)
                ...
                else
                        {
                                ...
                                var properties = new Dictionary<string, string>{
                                        // This property ensures that XAML files will be compiled in
                                        // the
                                        // current
                                        // AppDomain rather than a separate one. Any tasks isolated
                                        // in
                                        // AppDomains or tasks that create AppDomains will likely
                                        // not work
                                        // due
                                ...
                                Console.WriteLine (
                                    $"Loaded solution in {stopwatch.ElapsedMilliseconds}ms");
                                ...
                                if (args.Contains ("/stats"))
                                        {
                                                ...
                                                Console.WriteLine ("Number of projects:\t\t"
                                                                   + csharpProjects.Count);
                                                Console.WriteLine ("Number of documents:\t\t"
                                                                   + csharpProjects.Sum (
                                                                       x => x.DocumentIds.Count));

                                                var statistics
                                                    = await GetAnalyzerStatisticsAsync (
                                ...
                                bool testDocuments
                                    = args.Contains ("/editperf")
                                      || args.Any (arg => arg.StartsWith ("/editperf:"));
                                if (testDocuments)
                                        {
                                                Func<string, bool> documentMatch = _ => true;
                                                string matchArg = args.FirstOrDefault (
                                                    arg => arg.StartsWith ("/editperf:"));
                                                if (matchArg != null)
                                                        {
                                                                ...
                                                                documentMatch
                                                                    = documentPath => expression.IsMatch (
                                                                        documentPath);
                                                        }

                                                int iterations = 10;
                                                string iterationsArg = args.FirstOrDefault (
                                                    arg => arg.StartsWith ("/edititer:"));
                                                if (iterationsArg != null)
                                                        {
                                                                iterations = int.Parse (
                                                                    iterationsArg.Substring (
                                                                        "/edititer:".Length));
                                                        }

                                                var projectPerformance
                                                    = new Dictionary<ProjectId, double> ();
                                                var documentPerformance = new Dictionary<
                                                    DocumentId, DocumentAnalyzerPerformance> ();
                                                ...
                                                var slowestFiles
                                                    = documentPerformance
                                                          .OrderBy (
                                                              pair => pair.Value.EditsPerSecond)
                                                ...
                                                foreach (var projectGroup in slowestFiles)
                                                        {
                                                                ...
                                                                foreach (var pair in projectGroup
                                                                             .Take (5))
                                                                        {
                                                                                var document
                                ...
                                foreach (var group in allDiagnostics.GroupBy (i => i.Id).OrderBy (
                                             i => i.Key, StringComparer.OrdinalIgnoreCase))
                                        {
                                                ...
                                                // Print out analyzer diagnostics like AD0001 for
                                                // analyzer exceptions
                                ...
                                string logArgument
                                    = args.FirstOrDefault (x => x.StartsWith ("/log:"));
                                if (logArgument != null)
                                        {
                                                string fileName = logArgument.Substring (
                                                    logArgument.IndexOf (':') + 1);
                                                WriteDiagnosticResults (
                                                    diagnostics
                                                        .SelectMany (
                                                            i => i.Value.Select (
                                                                j => Tuple.Create (i.Key, j)))
                                                        .ToImmutableArray (),
                                                    fileName);
                                        }
                                ...
                                if (args.Contains ("/codefixes"))
                                        {
                                                await TestCodeFixesAsync (stopwatch, solution,
                                                                          allDiagnostics,
                                                                          cancellationToken)
                                                    .ConfigureAwait (true);
                                        }
                                ...
                                if (args.Contains ("/fixall"))
                                        {
                                                await TestFixAllAsync (stopwatch, solution,
                                                                       diagnostics, applyChanges,
                                                                       cancellationToken)
                                                    .ConfigureAwait (true);
                                        }
                        }
        ...
        TestDocumentPerformanceAsync (ImmutableArray<DiagnosticAnalyzer> analyzers, Project project,
                                      DocumentId documentId, int iterations, bool force,
                ...
                if (force)
                        {
                                foreach (var analyzer in analyzers)
                                        {
                                                foreach (var diagnostic in analyzer
                                                             .SupportedDiagnostics)
                                                        {
                                                                // make sure the analyzers we are
                                                                // testing are enabled
                ...
                for (int i = 0; i < iterations; i++)
                        {
                                ...
                                await compilationWithAnalyzers
                                    .GetAnalyzerSemanticDiagnosticsAsync (
                                        compilation.GetSemanticModel (tree), null,
                                        cancellationToken)
                                    .ConfigureAwait (false);
                        }

                return new DocumentAnalyzerPerformance (iterations
                                                        / stopwatch.Elapsed.TotalSeconds);
        }

        private static void
        WriteDiagnosticResults (ImmutableArray<Tuple<ProjectId, Diagnostic> > diagnostics,
                                string fileName)
        {
                         ...
                         bool applyChanges, CancellationToken cancellationToken)
        {
                ...
                foreach (var codeFixer in codeFixers)
                        {
                                equivalenceGroups.AddRange (await CodeFixEquivalenceGroup
                                                                .CreateAsync (codeFixer,
        ...
        private static async Task
        TestCodeFixesAsync (Stopwatch stopwatch, Solution solution,
                ...
                foreach (var item in diagnostics)
                        {
                                foreach (var codeFixer in codeFixers.GetValueOrDefault (
                                        ...
                                        {
                                                fixes.AddRange (
                                                    await GetFixesAsync (solution, codeFixer, item,
                                                                         cancellationToken)
                                                        .ConfigureAwait (false));
                                        }
                        }
                ...
                Parallel.ForEach (fixes, fix => {
                        try
                        ...
                        catch (Exception ex)
                                {
                                        ...
                                        {
                                                WriteLine (
                                                    $"The fix '{fix.Title}' threw an exception:",
                                                    ConsoleColor.Yellow);
        ...
        private static Task<Statistic>
        GetAnalyzerStatisticsAsync (IEnumerable<Project> projects,
                                    CancellationToken cancellationToken)
        {
        ...
        private static async Task<Statistic>
        GetAnalyzerStatisticsAsync (Document document, CancellationToken cancellationToken)
        {
                SyntaxTree tree
                ...
                var tokensAndNodes = root.DescendantNodesAndTokensAndSelf (descendIntoTrivia
                                                                           : true);
        ...
        private static IEnumerable<DiagnosticAnalyzer>
        FilterAnalyzers (IEnumerable<DiagnosticAnalyzer> analyzers, string[] args)
        {
                ...
                foreach (var analyzer in analyzers)
                        {
                                ...
                                else if (analyzer.SupportedDiagnostics.Any (
                                             y => ids.Contains (y.Id)))
                                        {
                                                yield return analyzer;
                                        }
        ...
        private static ImmutableArray<DiagnosticAnalyzer>
        GetAllAnalyzers ()
        {
                                ...
                                if (type.IsSubclassOf (codeFixProviderType) && !type.IsAbstract)
                                        {
                                                ...
                                                foreach (var diagnosticId in codeFixProvider
                                                             .FixableDiagnosticIds)
                                                        {
                                                                providers.AddToInnerList (
        ...
        GetAnalyzerDiagnosticsAsync (Solution solution,
                                     ImmutableArray<DiagnosticAnalyzer> analyzers, bool force,
                                     CancellationToken cancellationToken)
        {
        ...
        /// <summary>
        /// Returns a list of all analyzer diagnostics inside the specific project. This is an
        ...
        /// <param name="force"><see langword="true"/> to force the analyzers to be enabled;
        /// otherwise, <see langword="false"/> to use the behavior configured for the specified
        /// <paramref name="project"/>.</param> <param name="cancellationToken">The cancellation
        ...
        GetProjectAnalyzerDiagnosticsAsync (ImmutableArray<DiagnosticAnalyzer> analyzers,
                                            Project project, bool force,
                                            CancellationToken cancellationToken)
        {
                var supportedDiagnosticsSpecificOptions
                    = new Dictionary<string, ReportDiagnostic> ();
